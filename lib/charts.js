...
(function (root, factory) {
  ...
  function forecast(days, opts) {
    ...
    const key = m => (m.dash ? '<svg width="18" height="10" aria-hidden="true"><line x1="0" y1="5" x2="18" y2="5" '
      + 'stroke="' + m.colour + '" stroke-width="2.5" stroke-dasharray="6 3"/></svg>'
      : '<i class="key" style="background:' + m.colour + '"></i>');
    const legend = single || opts.legend === false ? '' : '<div class="legend">' + ms.map(m => '<span>' + key(m)
      + esc(m.name) + (m.sign < 0 ? ' (cost)' : '') + '</span>').join('') + '</div>';
  ...
  function tipHtml(t) {
    ...
    return '<div class="fc-h">' + esc(t.head) + '</div>' + cols
      + t.rows.map(r => '<div class="fc-r' + (r[3] ? ' on' : '') + '"><span>'
        ...
        + [r[1], r[2], r[5]].filter(v => v !== undefined).map(v => '<b' + (/^-|^[A-Z]{3} -/.test(String(v))
  ...
  return { bankReceipts, waterfall, categoryBars, policyCompare, availability, forecast, forecastItems,
    forecastHover, forecastAt, forecastStep, forecastBrush, forecastRange, spend, showTip, feeColour, MEASURES,
