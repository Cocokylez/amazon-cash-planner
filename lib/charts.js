...
(function (root, factory) {
  ...
  const MEASURES = [
    ...
    /* dashed: it is worked out from the others, and it often runs right
       alongside advertising - the dashes let the line under it show */
    { key: 'after', name: 'After fees and ads', colour: 'var(--mark-4)', sign: 1, dash: '9 5' },
  ];
  const DOW = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const dayName = d => DOW[new Date(toT(d)).getUTCDay()] + ' ' + mmmd(d);
  /* A day's value of one line. Days carry either the four headline figures
     (Simple.forecastFor's daily) or any set of lines in `v` (Simple.seriesFor). */
  const valueOf = (e, key) => {
    if (!e || !e.covered) return null;
    if (e.v) return e.v[key] == null ? 0 : e.v[key];
    return key === 'after' ? e.netSales - e.fees - e.advertising : e[key];
  };
  const picked = (opts, S) => {
    const want = opts.metrics || (opts.metric ? [opts.metric] : []);
    const ms = S.filter(x => want.indexOf(x.key) >= 0);
    return ms.length ? ms : [S[0]];
  };
  const share = (a, b) => (a == null || !b || b <= 0 ? null : a / b);
  ...
  /* What is drawn: one entry per day (the running total to it) or per week
     ...
     item.vs[key] is the plotted value of each picked line; item.rows is the
     reading, one per line: [name, amount, amount to date | undefined,
     picked, colour, share of opts.pctOf | undefined]. */
  function forecastItems(days, opts) {
    opts = opts || {};
    const S = opts.series || MEASURES;
    const ms = picked(opts, S);
    const on = x => ms.indexOf(x) >= 0;
    const monday = opts.mondayOf || (d => d);
    const pk = opts.pctOf || null;
    const items = [];
    if (opts.mode === 'weekly') {
      const byWeek = new Map();
      for (const e of days) {
        const k = monday(e.date);
        const w = byWeek.get(k) || { days: 0, covered: 0, sums: {}, from: e.date, to: e.date };
        w.days++;
        w.to = e.date;
        if (e.covered) {
          w.covered++;
          for (const x of S) w.sums[x.key] = (w.sums[x.key] || 0) + valueOf(e, x.key);
          if (pk && !S.some(x => x.key === pk)) w.sums[pk] = (w.sums[pk] || 0) + valueOf(e, pk);
        }
        byWeek.set(k, w);
      }
      for (const [k, w] of byWeek) {
        ...
        items.push({ vs, gap: !w.covered, from: w.from, to: w.to,
          ...
          rows: S.map(x => [x.name, w.covered ? w.sums[x.key] * x.sign : null, undefined, on(x), x.colour,
            pk ? (w.covered ? share(w.sums[x.key], w.sums[pk]) : null) : undefined]),
          label: mmmd(k) });
      }
    } else {
      const run = {};
      const keys = S.map(x => x.key).concat(pk && !S.some(x => x.key === pk) ? [pk] : []);
      for (const e of days) {
        for (const k of keys) run[k] = (run[k] || 0) + (valueOf(e, k) || 0);
        const vs = {};
        for (const x of ms) vs[x.key] = run[x.key];
        items.push({ vs, gap: !e.covered, from: e.date, to: e.date, head: dayName(e.date),
          note: e.covered ? '' : 'No forecast downloaded for this day',
          rows: S.map(x => [x.name, e.covered ? valueOf(e, x.key) * x.sign : null,
            run[x.key] * x.sign, on(x), x.colour, pk ? share(run[x.key], run[pk]) : undefined]),
          label: mmmd(e.date) });
      }
    }
  ...
  /* The chart. opts: series (default the four tiles), metrics (the keys
     shown), mode ('running' | 'weekly'), currency, mondayOf, pctOf (a key:
     the reading adds each line's share of it), zoom ({from, to}: only these
     dates are drawn, the running totals still counted from the start), id
     (which chart, for zooming), legend (false: the page draws its own). */
  function forecast(days, opts) {
    ...
    const all = forecastItems(days, opts);
    const ms = all.measures;
    let items = all.items;
    const z = opts.zoom;
    if (z && z.from && z.to) {
      const a = items.findIndex(it => it.to >= z.from);
      let b = -1;
      for (let i = items.length - 1; i >= 0; i--) if (items[i].from <= z.to) { b = i; break; }
      if (a >= 0 && b >= a) items = items.slice(a, b + 1);
    }
    const zoomed = items.length < all.items.length;
    const single = ms.length === 1;
    const ends = !weekly;           // each running line is named at its end

    const W = 1072, H = 340, padL = 64, padR = ends ? 200 : 18, padT = 30, padB = 34;
    const plotW = W - padL - padR, plotH = H - padT - padB;
    const vals = [];
    for (const it of items) for (const x of ms) if (it.vs[x.key] != null) vals.push(it.vs[x.key]);
    if (!vals.length) vals.push(0);
    /* Zoomed in on running totals, the axis fits what is on screen rather
       than starting at zero - that is what zooming in is for. */
    const withZero = weekly || !zoomed;
    const vHi = withZero ? Math.max(0, ...vals) : Math.max(...vals);
    const vLo = withZero ? Math.min(0, ...vals) : Math.min(...vals);
    const step = niceMax(Math.max(1, vHi - vLo) / 4);
    const lo = Math.floor(vLo / step) * step;
    let hi = Math.ceil(vHi / step) * step;
    if (hi <= lo) hi = lo + step;
    ...
    /* money with its sign, $50k, as the tiles and the reading show it */
    const num = c => Money.fmtShort(c, cur) || '';
    let grid = '';
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (band * (j - i + 1) > 170) {
        bands += '<text class="ch-gap" x="' + (padL + band * (i + j + 1) / 2) + '" y="' + (padT + plotH - 10)
          + '" text-anchor="middle">No forecast downloaded</text>';
      }
      i = j;
    }
    ...
    let marks = '', labels = '';
    if (weekly) {
      /* side by side in each week, in the order of the lines */
      const group = Math.min(COL_MAX * ms.length, band - 10);
      const w = Math.max(2, (group - GAP * (ms.length - 1)) / ms.length);
      items.forEach((it, i) => {
        ms.forEach((m, k) => {
          const v = it.vs[m.key];
          if (!v) return;
          const top = Math.min(Y(v), base), h = Math.abs(Y(v) - base);
      ...
      /* the running totals: one line each; across days with no forecast a
         line runs level and dashed. One line alone gets a light fill. */
      for (const m of ms) {
        let area = '', line = '', flat = '';
        items.forEach((it, i) => {
          const p = X(i).toFixed(1) + ',' + Y(it.vs[m.key]).toFixed(1);
          if (i === 0) { line = 'M' + p; area = 'M' + X(0).toFixed(1) + ',' + base.toFixed(1) + 'L' + p; return; }
          if (it.gap) {
        ...
        area += 'L' + X(items.length - 1).toFixed(1) + ',' + base.toFixed(1) + 'Z';
        const last = items.length - 1;
        marks += (single || m.key === 'netSales' ? '<path d="' + area + '" fill="' + m.colour + '" opacity=".10"/>' : '')
          + '<path d="' + line + '" fill="none" stroke="' + m.colour + '" stroke-width="2" '
          + 'stroke-linejoin="round" stroke-linecap="round"'
          + (m.dash && !single ? ' stroke-dasharray="' + m.dash + '"' : '') + '/>'
          + (flat ? '<path d="' + flat + '" fill="none" stroke="' + (single ? 'var(--mark-neutral)' : m.colour)
            + '" stroke-width="2" stroke-dasharray="5 4"' + (single ? '' : ' opacity=".6"') + '/>' : '')
          + '<circle class="fc-dot" cx="' + X(last) + '" cy="' + Y(items[last].vs[m.key]) + '" r="4" fill="' + m.colour + '"/>';
      }
      /* Every line named at its end, pushed apart where they meet, so two
         lines that run on top of each other still read as two. */
      const last = items.length - 1;
      const lab = ms.map(m => ({ m, v: items[last].vs[m.key], y: Y(items[last].vs[m.key] || 0) }))
        .sort((a, b) => a.y - b.y);
      const GAPY = 17, top = padT + 4, bottom = padT + plotH;
      for (let i = 0; i < lab.length; i++) {
        lab[i].y = Math.max(lab[i].y, i ? lab[i - 1].y + GAPY : top);
      }
      for (let i = lab.length - 1; i >= 0; i--) {
        lab[i].y = Math.min(lab[i].y, i === lab.length - 1 ? bottom : lab[i + 1].y - GAPY);
      }
      const lx = X(last) + 12;
      for (const l of lab) {
        const ly = Y(l.v || 0);
        const name = l.m.name.length > 19 ? l.m.name.slice(0, 18) + '…' : l.m.name;
        labels += (Math.abs(ly - l.y) > 3 ? '<path d="M' + (X(last) + 5) + ',' + ly.toFixed(1) + 'L' + (lx - 2) + ','
          + l.y.toFixed(1) + '" stroke="' + l.m.colour + '" stroke-width="1" fill="none" opacity=".7"/>' : '')
          + '<text class="ch-lab" x="' + lx + '" y="' + (l.y + 4).toFixed(1) + '">' + esc(name)
          + ' <tspan class="ch-labv">' + esc(num((l.v || 0) * l.m.sign)) + '</tspan>'
          + '<title>' + esc(l.m.name) + '</title></text>';
      }
    }

    /* the reading: a guide line and a dot per line, moved by forecastHover;
       and the band a drag draws, to zoom into */
    const hover = '<rect class="fc-brush" x="0" y="' + padT + '" width="0" height="' + plotH
      + '" style="display:none"/>'
      + '<g class="fc-cursor" style="display:none">'
      + '<line class="fc-guide" x1="0" y1="' + padT + '" x2="0" y2="' + (padT + plotH) + '" stroke-width="1"/>'
      + (weekly ? '' : ms.map((m, k) => '<circle class="fc-dot" data-k="' + k + '" r="4.5" cx="0" cy="0" '
        + 'fill="' + m.colour + '"/>').join(''))
      + '</g>';

    const data = { id: opts.id || '', W, band, padL, padT, plotH, weekly, currency: cur, pct: !!opts.pctOf,
      items: items.map((it, i) => ({ x: +X(i).toFixed(1), from: it.from, to: it.to,
    ...
    const svg = '<svg class="chart" viewBox="0 0 ' + W + ' ' + H + '" style="max-width:' + W + 'px" '
      + 'aria-hidden="true">'
      + '<defs><pattern id="fc-gap" width="6" height="6" patternUnits="userSpaceOnUse" '
      + 'patternTransform="rotate(45)"><rect width="1.5" height="6" style="fill:var(--hatch)"/></pattern></defs>'
      + bands + grid
      + '<line class="ch-base" x1="' + padL + '" y1="' + base + '" x2="' + (W - padR) + '" y2="' + base + '"/>'
      + marks + labels + hover + xl + '</svg>';

    const key = m => '<i class="swatch' + (weekly ? '' : ' swatch--line') + (m.dash && !weekly ? ' swatch--dashed' : '')
      + '" style="--c:' + m.colour + '"></i>';
    const gaps = items.some(it => it.gap);
    const legend = (single && !gaps) || opts.legend === false ? '' : '<div class="legend">'
      + (single ? '' : ms.map(m => '<span>' + key(m) + esc(m.name) + '</span>').join(''))
      + (gaps ? '<span><i class="swatch swatch--hatch"></i>No forecast downloaded</span>' : '') + '</div>';

    return '<div class="fchart" tabindex="0" role="group" data-chart="' + esc(opts.id || '') + '" aria-label="'
      + esc(ms.map(m => m.name).join(', ') + ', ' + (weekly ? 'week by week' : 'running total')
      + '. Arrow keys read each ' + (weekly ? 'week' : 'day') + '; plus and minus zoom.')
      + '" data-fc="' + esc(JSON.stringify(data)) + '">'
  ...
  /* One reading box, used by every chart that has one: a heading, rows of
     [name, value, value2?, bold?, colour?, value3?], column heads, a note. */
  function tipHtml(t) {
    const valsOf = r => [r[1], r[2], r[5]].filter(v => v !== undefined);
    const n = Math.max(1, t.cols ? t.cols.length : 0, ...t.rows.map(r => valsOf(r).length));
    let g = t.cols ? '<span></span>' + t.cols.map(c => '<span class="h">' + esc(c) + '</span>').join('') : '';
    for (const r of t.rows) {
      const vals = valsOf(r);
      const on = r[3] ? ' on' : '';
      g += '<span class="nm' + on + '">' + (r[4] ? '<i style="background:' + r[4] + '"></i>' : '') + esc(r[0]) + '</span>'
        + vals.map(v => '<span class="v' + (/^-|^[A-Z]{3} -/.test(String(v)) ? ' neg' : '') + on + '">'
          + esc(v == null ? '—' : v) + '</span>').join('')
        + '<span></span>'.repeat(n - vals.length);
    }
    return '<div class="fc-h">' + esc(t.head) + '</div>'
      + '<div class="fc-g" style="grid-template-columns:minmax(0,1fr) repeat(' + n + ',auto)">' + g + '</div>'
  ...
  function forecastHover(el, i) {
    ...
    const fmt = c => (c == null ? null : Money.fmt(c, { currency: data.currency }));
    const pc = f => (f == null ? null : (Math.round(f * 1000) / 10).toFixed(1) + '%');
    const cols = (data.weekly ? ['This week'] : ['That day', 'To date']).concat(data.pct ? ['Of sales'] : []);
    tip.innerHTML = tipHtml({ head: it.head, cols: cols.length > 1 ? cols : null,
      rows: it.rows.map(r => [r[0], fmt(r[1]), r[2] === undefined ? undefined : fmt(r[2]), r[3], r[4],
        r[5] === undefined ? undefined : pc(r[5])]), note: it.note });
    tip.hidden = false;
  ...
  /* Which item is under the pointer at clientX: null outside the plot, or
     the nearest end with clamp. */
  function forecastAt(el, clientX, clamp) {
    ...
    if (clamp) return Math.max(0, Math.min(data.items.length - 1, i));
    return i < 0 || i >= data.items.length ? null : i;
  }

  /* The band a drag draws across items a..b, or none. Browser only. */
  function forecastBrush(el, a, b) {
    const data = el && fcData(el);
    const r = el && el.querySelector('.fc-brush');
    if (!data || !r) return;
    if (a == null || b == null) { r.style.display = 'none'; return; }
    const lo = Math.min(a, b), hi = Math.max(a, b);
    const x1 = data.items[lo].x - data.band / 2, x2 = data.items[hi].x + data.band / 2;
    r.setAttribute('x', x1.toFixed(1));
    r.setAttribute('width', (x2 - x1).toFixed(1));
    r.style.display = '';
  }

  /* The dates items a..b stand for. */
  function forecastRange(el, a, b) {
    const data = el && fcData(el);
    if (!data || a == null || b == null || !data.items.length) return null;
    const lo = Math.max(0, Math.min(a, b)), hi = Math.min(data.items.length - 1, Math.max(a, b));
    return { from: data.items[lo].from, to: data.items[hi].to };
  }
  ...
  const FEE_COLOURS = {
    'FBA fulfillment fees': 'var(--fee-fba)', 'Referral fee': 'var(--fee-referral)',
    'Monthly inventory storage fee': 'var(--fee-storage)', 'Aged inventory surcharge': 'var(--fee-aged)',
    'Closing fee': 'var(--fee-closing)', 'Per-item selling fee': 'var(--fee-peritem)',
    'Returns processing fee for Apparel and Shoes': 'var(--fee-returns)',
    'Advertising': 'var(--series-ads)', 'Left after fees and ads': 'var(--series-left)',
  };
  const SPARE = ['var(--fee-spare-1)', 'var(--fee-spare-2)', 'var(--fee-spare-3)', 'var(--fee-spare-4)'];
  function feeColour(name, i) { return FEE_COLOURS[name] || SPARE[(i || 0) % SPARE.length]; }

  function spend(rows, opts) {
    opts = opts || {};
    const cur = opts.currency || 'USD';
    const fmt = c => Money.fmt(c, { currency: cur }) || '';
    ...
    if (opts.share && !big) {
      ...
      out += '<line x1="' + x100 + '" y1="' + (padT - 6) + '" x2="' + x100 + '" y2="' + (H - 2)
        + '" stroke="var(--text-2)" stroke-width="1.5" stroke-dasharray="3 3"/>'
        + '<text class="ch-axis" x="' + x100 + '" y="' + (padT - 10) + '" text-anchor="' + (shareMax > 1.15
          ? 'middle' : 'end') + '">100% of net sales</text>';
    }
    const pick = r => (opts.pickable ? ' data-sku="' + esc(r.label) + '"' : '');
    rows.forEach((r, i) => {
      const y = padT + i * rowH + (rowH - barH) / 2;
      if (opts.pickable) {
        out += '<rect class="sp-row' + (opts.picked === r.label ? ' on' : '') + '" x="0" y="' + (padT + i * rowH + 1)
          + '" width="' + W + '" height="' + (rowH - 2) + '" rx="6"' + pick(r) + '/>';
      }
      const k = scale(r);
      const cost = costOf(r);
      ...
      if (!big) {
        ...
        out += '<text class="ch-val" x="6" y="' + (y + barH / 2 - (r.sub ? 3 : -5)) + '"' + pick(r) + '>' + esc(name)
          + '<title>' + esc(r.label) + '</title></text>'
          + (r.sub ? '<text class="ch-axis" x="6" y="' + (y + barH / 2 + 14) + '" style="font-size:12px"' + pick(r) + '>'
            + esc(r.sub) + '</text>' : '');
      }
      let x = barX;
      if (opts.share && !(r.netSales > 0)) {
        ...
        for (const s of segs) {
          ...
          out += '<rect class="sp-seg" x="' + x.toFixed(1) + '" y="' + y + '" width="' + Math.max(0.5, w - 1).toFixed(1)
            + '" height="' + barH + '" rx="2" fill="' + s.c + '" data-tip="' + tipFor(s.name) + '"' + pick(r) + '/>';
          if (big && w > 70) {
  ...
  /* ── 8. the same, drawn in HTML (Ledger): one bar of every dollar of net
     sales, and a row per product ─────────────────────────────────────────

     The reading for a piece: every piece of that row, the one under the
     pointer in bold - the same reading spend() gives. */
  const SHORT = { 'FBA fulfillment fees': 'FBA fulfillment', 'Referral fee': 'Referral',
    'Monthly inventory storage fee': 'Storage', 'Aged inventory surcharge': 'Aged inventory',
    'Closing fee': 'Closing', 'Per-item selling fee': 'Per-item',
    'Returns processing fee for Apparel and Shoes': 'Returns', 'Left after fees and ads': 'Left' };
  const shortName = n => SHORT[n] || n;
  function pieces(r) {
    const cost = r.parts.reduce((s, p) => s + Math.max(0, p.amount || 0), 0);
    const left = (r.netSales || 0) - cost;
    const segs = r.parts.filter(p => p.amount > 0).map((p, j) => ({ name: p.name, amount: p.amount,
      c: feeColour(p.name, j) }));
    if (left > 0) segs.push({ name: 'Left after fees and ads', amount: left, c: feeColour('Left after fees and ads') });
    return { cost, left, segs };
  }
  function readingOf(r, cur) {
    const fmt = c => Money.fmt(c, { currency: cur }) || '';
    const pc = (a, b) => (!b || b <= 0 ? '—' : (Math.round(a / b * 1000) / 10).toFixed(1) + '%');
    const { left } = pieces(r);
    return { head: r.label + (r.sub ? ' · ' + r.sub : ''), cols: ['Amount', 'Of net sales'],
      rows: [['Net sales', fmt(r.netSales || 0), '100%', false, 'var(--series-net)']]
        .concat(r.parts.filter(p => p.amount).map((p, j) => [p.name, fmt(-p.amount), pc(p.amount, r.netSales),
          false, feeColour(p.name, j)]))
        .concat([['Left after fees and ads', fmt(left), pc(left, r.netSales), false,
          feeColour('Left after fees and ads')]]) };
  }
  const tipAttr = (reading, name) => ' data-tip="' + esc(JSON.stringify(Object.assign({}, reading, {
    rows: reading.rows.map(x => x.slice(0, 3).concat([x[0] === name]).concat(x.slice(4))) }))) + '"';

  /* Out of every dollar of net sales: each fee, advertising and what is
     left, as one bar; names and shares under the pieces wide enough to
     carry them. Costs past the sales fill the bar and say so. */
  function shareBar(row, opts) {
    opts = opts || {};
    const cur = opts.currency || 'USD';
    if (!row || !((row.netSales || 0) > 0 || row.parts.some(p => p.amount))) {
      return emptyState('Nothing to show', 'No sales or fees for these dates.');
    }
    const { cost, left, segs } = pieces(row);
    const whole = Math.max(row.netSales || 0, cost) || 1;
    const reading = readingOf(row, cur);
    const pc = a => (row.netSales > 0 ? (Math.round(a / row.netSales * 1000) / 10).toFixed(1) + '%' : '');
    const flex = s => (s.amount / whole * 1000).toFixed(1);
    const label = segs.map(s => shortName(s.name) + ' ' + pc(s.amount)).join(', ');
    return '<div class="tipzone"><div class="stacked-bar" role="img" aria-label="' + esc(label) + '">'
      + segs.map(s => '<span class="stacked-bar__seg" style="flex:' + flex(s) + ' 1 0;--c:' + s.c + '"'
        + tipAttr(reading, s.name) + '></span>').join('')
      + '</div><div class="stacked-bar-labels" aria-hidden="true">'
      + segs.map(s => '<span style="flex:' + flex(s) + ' 1 0">'
        + (s.amount / whole >= 0.05 ? esc(shortName(s.name)) + '<b>' + esc(pc(s.amount)) + '</b>' : '')
        + '</span>').join('')
      + '</div>'
      + (left < 0 && row.netSales > 0 ? '<div class="meta" style="margin-top:8px;color:var(--negative)">Costs are '
        + esc(pc(cost)) + ' of net sales: ' + esc(Money.fmt(-left, { currency: cur }) || '')
        + ' more than the sales.</div>' : '')
      + '<div class="fc-tip" hidden></div></div>';
  }

  /* A row per product: its costs laid along its own net sales (share) or
     along one money scale (amounts), with a dashed mark where its net sales
     end. rows: [{ label, sub, netSales, parts }]; opts: currency, share,
     picked (the product the chart above follows), ariaLabel. */
  function productRows(rows, opts) {
    opts = opts || {};
    const cur = opts.currency || 'USD';
    const fmt = c => Money.fmt(c, { currency: cur }) || '';
    const pc = (a, b) => (!b || b <= 0 ? '—' : (Math.round(a / b * 1000) / 10).toFixed(1) + '%');
    rows = (rows || []).filter(r => r && ((r.netSales || 0) > 0 || r.parts.some(p => p.amount)));
    if (!rows.length) return emptyState('Nothing to show', 'No sales or fees for these dates.');
    const all = rows.map(r => Object.assign({ r }, pieces(r)));
    const shareMax = Math.min(2, Math.max(1, ...all.map(x => (x.r.netSales > 0 ? x.cost / x.r.netSales : 1))));
    const moneyMax = Math.max(1, ...all.map(x => Math.max(x.r.netSales || 0, x.cost)));
    let out = '';
    for (const x of all) {
      const r = x.r;
      const reading = readingOf(r, cur);
      const extent = Math.max(r.netSales || 0, x.cost);
      let barW, mark;
      if (opts.share) {
        barW = r.netSales > 0 ? Math.min(extent / r.netSales, shareMax) / shareMax : 0;
        mark = 1 / shareMax;
      } else {
        barW = extent / moneyMax;
        mark = (r.netSales || 0) / moneyMax;
      }
      const on = opts.picked === r.label;
      const track = opts.share && !(r.netSales > 0)
        ? '<span class="product-row__none"' + tipAttr(reading, '') + '>No sales on these dates — fees '
          + esc(fmt(x.cost)) + '</span>'
        : '<span class="product-row__bar" style="width:' + (barW * 100).toFixed(2) + '%">'
          + x.segs.map(s => '<span style="flex:' + s.amount + ' 1 0;--c:' + s.c + '"' + tipAttr(reading, s.name)
            + '></span>').join('')
          + '</span>' + (r.netSales > 0 ? '<span class="product-row__marker" style="left:' + (mark * 100).toFixed(2)
            + '%"></span>' : '');
      out += '<button type="button" class="product-row" role="option" aria-selected="' + on + '" data-sku="'
        + esc(r.label) + '" title="Follow ' + esc(r.label) + ' in the chart above">'
        + '<span class="product-row__id"><b>' + esc(r.label) + '</b>' + (r.sub ? '<small>' + esc(r.sub) + '</small>' : '')
        + '</span><span class="product-row__track">' + track + '</span>'
        + '<span class="product-row__nums"><span>' + (r.netSales > 0 ? 'costs ' + esc(pc(x.cost, r.netSales)) : 'no sales')
        + '</span><b' + (x.left < 0 ? ' class="is-neg"' : '') + '>' + esc(x.left < 0 ? 'loses ' + fmt(-x.left)
          : 'keeps ' + fmt(x.left)) + '</b></span></button>';
    }
    return '<div class="tipzone"><div class="product-list" role="listbox" aria-label="'
      + esc(opts.ariaLabel || 'Products') + '">' + out + '</div><div class="fc-tip" hidden></div></div>';
  }
  ...
  return { bankReceipts, waterfall, categoryBars, policyCompare, availability, forecast, forecastItems,
    forecastHover, forecastAt, forecastStep, forecastBrush, forecastRange, spend, shareBar, productRows, showTip,
    feeColour, shortName, tipHtml, MEASURES,
    emptyState, colPath, barPath, niceMax };
});
