...
(function (root, factory) {
  ...
  function create(opts) {
    ...
    const useCap = opts.use || (typeof globalThis !== 'undefined'
      && globalThis.claude && globalThis.claude.use
      ? globalThis.claude.use.bind(globalThis.claude) : null);
    ...
    async function connect() {
      deviceId();
      if (!useCap) {
        set('unavailable', { reason: 'This copy of the app is running outside claude.ai, so there '
          + 'is no account to sync with. Imports are saved on this device only.' });
        return false;
      }
    ...
    async function pushState(localState, force) {
      ...
      const payload = {};
      for (const k of STATE_KEYS) payload[k] = localState[k];
    ...
    function mergeState(mine, theirs, preferLocal) {
      ...
      const ACCUMULATED = ['balanceSnapshots', 'deferredSnapshots', 'settlements',
        'bankDeposits', 'forecastRuns', 'requestPlans', 'cashCommitments'];
  ...
  return { create, encode, decode, fingerprint, splitChunks, safeKey,
    CHUNK, MAX_CHUNKS_PER_BLOB, STATE_KEYS };
});
