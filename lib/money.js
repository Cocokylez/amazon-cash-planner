...
(function (root, factory) {
  ...
  /* The currency's own sign, which reads faster than its code: $ for US
     dollars, CA$ and MX$ so a Canadian or Mexican dollar is never taken for
     one. A currency with no sign that cannot be mistaken keeps its code. */
  const SYMBOLS = { USD: '$', CAD: 'CA$', MXN: 'MX$', AUD: 'A$', SGD: 'S$', BRL: 'R$',
    GBP: '\u00a3', EUR: '\u20ac', JPY: '\u00a5', INR: '\u20b9' };
  const symbolOf = code => SYMBOLS[code || displayCurrency] || ((code || displayCurrency) + ' ');
  function fmt(c, opts) {
  ...
  return {
    ...
    cents, cents0,
    fmt, fmtDec, fmtShort, pct, fmtPct, fmtRate, group, setCurrency, symbolOf,
  };
});
