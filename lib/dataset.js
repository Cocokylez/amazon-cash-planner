...
(function (root, factory) {
  ...
  function readiness(ctx) {
    ...
    /* optional: a feature that needs the transaction history. The app is
       built around Amazon's forecast; history is an extra someone may never
       want, so what depends on it is not counted as missing. Import a
       transactions file and those features count - and work - again. */
    const item = (id, label, ready, needs, screen, optional) =>
      ({ id, label, ready, needs: needs || [], screen, optional: !!optional });
    ...
    return [
      ...
      item('actual-expenses', 'Actual fees you have been charged', has.actual,
        has.actual ? [] : [need('the Payments date-range transaction CSV', 'data')], 'expenses', true),
      ...
      item('payout-timing', 'When a payout would be released', has.actual && has.balance,
        [].concat(has.actual ? [] : [need('the transaction CSV, to measure release timing', 'data')],
          has.balance ? [] : [need('your current Amazon balances', 'data')]), 'forecast', true),
  ...
  const blocking = list => list.filter(r => !r.ready && !r.optional);
  /* What the count is out of: everything, less optional features not in use. */
  const counted = list => list.filter(r => !r.optional || r.ready);
  const ready = list => list.filter(r => r.ready);

  return { build, forecastBranch, actualBranch, readiness, blocking, ready, counted, overlapOf };
});
