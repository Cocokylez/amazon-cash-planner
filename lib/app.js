...
(function () {
  ...
  function salesRange() {
    ...
    const loadedTo = forecastOf(scopedPreviews(), from, from).loadedTo;
    const to = state.salesTo && state.salesTo >= from ? state.salesTo
      : (loadedTo && loadedTo >= from ? loadedTo : horizonEnd());
    return { from, to };
  }
  ...
  screens.sales = function () {
    ...
    /* the dates */
    let html = '<section class="card" id="salesdates"><div class="frow f3" style="align-items:end">'
      + '<div class="field"><label class="lbl">From</label>'
      + '<div style="font-size:var(--t-h3);font-weight:600;padding:6px 0">Tomorrow, ' + esc(fmtDay(from)) + '</div></div>'
      + '<div class="field"><label class="lbl" for="s-to">Up to</label>'
      + '<input type="date" id="s-to" value="' + esc(to) + '" min="' + esc(from) + '"></div>'
      + '<div class="field"><label class="lbl">Quick</label><div class="btnrow" style="gap:6px;margin:0">'
      + [7, 14, 30, 56, 120].map(n => '<button class="chip" data-sdays="' + n + '"'
        + (CSV.addDays(state.today, n) === to ? ' aria-pressed="true"' : '') + '>'
        + (n === 56 ? '8 weeks' : n === 120 ? '4 months' : n + ' days') + '</button>').join('')
      + '</div></div></div>'
      + '<div class="meta">' + f.days + ' days \u00b7 '
  ...
  function buildBrief() {
    ...
    {
      const from = horizonStart();
      const to = state.salesTo && state.salesTo >= from ? state.salesTo : horizonEnd();
  ...
  async function runSchedule(trigger) {
    ...
    try {
      for (let n = 0; n < windows.length; n++) {
        ...
        if (r.ok) { run.done++; inARow = 0; } else {
          run.failed++;
          run.failures.push({ from: w.from, to: w.to, why: r.why });
          if (r.stop) { run.note = r.why; run.status = 'stopping'; break; }
          if (++inARow >= 3) {
        ...
        saveRun();
        paintSchedule();
        if (n < windows.length - 1) await pause(cfg.gapSeconds * 1000);
      }
    } finally {
  ...
  async function fetchWindow(w) {
    ...
    return waitForJob(job.jobId, w);
  }

  async function waitForJob(jobId, w) {
    const run = state.scheduleRun;
  ...
  function scheduleCard() {
    ...
    let html = '<section class="card" id="schedulecard"><div class="card-h">' + ico('download')
      ...
      + 'morning; then week by week to about four months, refreshed once a week. One report at a time, '
      + cfg.gapSeconds + ' seconds apart. If Amazon asks you to sign in or complete a check, it stops, tells '
      + 'you, and carries on once you have. Replaced versions are let go, so storage stays level.</div>'
    ...
    if (run) {
      ...
      const words = {
        running: 'Downloading: ' + run.done + ' of ' + run.total + ' done'
          + (run.current ? ' · now ' + fmtShortDay(run.current.from)
  ...
  document.addEventListener('click', e => {
    ...
    if (sd) { state.salesTo = CSV.addDays(state.today, +sd.getAttribute('data-sdays')); render(); return; }
    if (e.target.closest('[data-sget]')) {
      const el = document.getElementById('s-to');
      openGetWindow({ from: horizonStart(), to: (el && el.value) || state.salesTo || horizonEnd() });
      return;
    }
  ...
  document.addEventListener('change', e => {
    ...
    if (id === 's-to') {
      ...
      if (v && v >= horizonStart()) { state.salesTo = v; render(); }
      return;
    }
