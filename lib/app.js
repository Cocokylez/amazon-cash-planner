...
(function () {
  ...
  function money(cents) {
    ...
    const txt = M.fmt(cents) || '';
    /* The currency code is set beside the figure, smaller, as v3 has it. */
    const code = state.filters.currency || 'USD';
    return '<span class="cur">' + esc(code) + '</span>' + esc(txt.replace(/^[^\d\-\u2212(]+/, ''));
  }
  ...
  function leadArrivalCard(br) {
    ...
    const measured = state.bankDeposits.length > 0;
    let html = '<section class="card"><div class="card-h"><h2>Expected bank arrival</h2>'
      + (r && r.low ? tag(measured ? 'MEASURED' : 'ASSUMPTION') : '') + '</div>';
    if (r && r.low) {
      const lo = state.policy.bankTransitDaysLow, hi = state.policy.bankTransitDaysHigh;
      html += '<div class="bigdate" style="margin-top:14px">' + esc(weekday(r.low)) + ', '
        ...
        + (measured ? 'Measured from ' + state.bankDeposits.length + ' recorded deposit'
          + (state.bankDeposits.length === 1 ? '' : 's')
          : 'Transit days entered by hand. Add bank deposits to measure them.') + '</div>';
    } else {
  ...
  function availabilityCard(run, reqDate) {
    ...
    html += '<div class="meta">' + esc(fmtShortDay(first)) + ' \u2013 ' + esc(fmtShortDay(lastD))
      ...
      + Charts.availability(pts, { today: state.today, selected: reqDate, gaps,
  ...
  function comparisonCard(account, reqDate) {
    ...
    let html = '<section class="card" id="comparecard"><div class="card-h">'
      ...
      + (nextS ? '<span class="meta" style="margin-left:0">Compared through '
        + esc(fmtDay(CSV.addDays(state.today, 55))) + '</span>' : '') + '</div>'
  ...
  function accountSelect() {
    return '<span class="hctl"><label class="lbl" for="h-acct">Account</label>'
      + '<select id="h-acct">' + accountOptions(state.filters.account || 'Standard Orders')
      + '</select></span>';
  }
  ...
  function topControls() {
    ...
    if (s === 'dashboard' || s === 'forecast' || s === 'expenses' || s === 'profit') {
      html += accountSelect();
    }
  ...
  function dataStatus(readyN, totalN) {
    ...
    return '<span class="ds-t">Data status</span>' + tx + fc + bl + dep
      + (ds.anyData ? '<span class="ds-r" style="color:var(--ink-3);font-size:var(--t-tag)">'
        + readyN + ' of ' + totalN + ' features working</span>' : '');
  }
  ...
  function render() {
    ...
    $('#view').innerHTML = body + dateBasisNote();
