...
(function () {
  ...
  const state = {
    screen: 'sales', tab: null,
    salesFrom: null,           // the first date shown on the forecast screen (null: tomorrow)
    salesTo: null,             // the "up to" date on the forecast screen
    salesMetrics: ['netSales'], // which tiles the forecast chart shows - one to all four
    show: loadShow(),          // Settings: which of the extra inputs appear, and More tools
    moreOpen: loadMoreOpen(),  // the sidebar's More tools list, open or closed
    theme: loadTheme(),        // Settings: System, Light or Dark
    installing: null,          // the version being installed, while the app closes to install it
    skuSort: 'cost',           // costs by product: 'cost' | 'pct' | 'sales'
    skuShare: true,            // ...bars as a share of each product's sales, or amounts
    skuAll: false,             // ...the top 20, or every product
    skuPick: null,             // ...the product charted over time
    zoom: {},                  // each chart's zoom: { chartId: { from, to } }
    cloudBarHidden: false,     // "only on this computer" bar, put off until next launch
    schedule: null,            // the morning download's settings and record (travels with the figures)
    scheduleRun: loadRun(),    // this computer's morning download, while and after it runs
    removedImports: [],        // files let go on any computer, so no other computer brings them back
    chartShow: {},             // the lines shown on each fees-over-time chart
    chartMode: {},             // ...running total or week by week
    salesMode: 'running',      // the chart: 'running' total, or 'weekly'
    ledger: null,
    previews: [],
    balanceSnapshots: [],
    deferredSnapshots: [],
    ...
    getWin: null,              // the "Get Amazon data" window, while it is open
    notices: loadNotices(),    // what happened: downloads, imports, errors - newest first
    noticeOpen: false,
    jobSeen: null,             // each job's last status, to notice when it changes
    noticedUpdate: null,       // the update version already announced
  ...
  /* The app does one job: Amazon's forecast of sales and fees, from tomorrow
     to the date you pick. Two screens. The cash-planning screens it had
     before are kept, working and tested, but out of the navigation - see
     HIDDEN_SCREENS - so nothing was thrown away. */
  const SCREENS = [
    { id: 'sales', group: 'Forecast', name: 'Sales & fees forecast', title: 'Sales & fees forecast',
      short: 'Forecast', icon: 'forecast',
      blurb: 'Amazon\u2019s estimate of your sales and fees, from tomorrow to the date you pick' },
    { id: 'data', group: 'Forecast', name: 'Downloads', title: 'Downloads',
      short: 'Downloads', icon: 'download',
      blurb: 'Get Amazon\u2019s forecast, and the files you have' },
  ];
  ...
  /* Everything 5.0 took out of the navigation, back within reach: the list
     opens from "More tools" in the sidebar. Your own figures holds the inputs
     Amazon's downloads do not carry; Settings decides which of them show. */
  HIDDEN_SCREENS.push({ id: 'inputs', group: 'Inputs', name: 'Your own figures', title: 'Your own figures',
    short: 'Figures', icon: 'data',
    blurb: 'What Amazon\u2019s downloads do not carry: balance, payout rules, deposits, costs, ad billing' });
  const APP_SCREENS = [
    { id: 'settings', group: 'App', name: 'Settings', title: 'Settings', short: 'Settings', icon: 'settings',
      blurb: 'What the app shows, and the guide' },
  ];
  /* Screens that read the forecast alone, or nothing: no period control,
     no transaction-date note. */
  const PLAIN = new Set(['sales', 'data', 'inputs', 'settings']);
  const screenById = id => SCREENS.concat(HIDDEN_SCREENS, APP_SCREENS).find(s => s.id === id) || SCREENS[0];

  /* Settings. Every extra input is on unless turned off; turning one off
     only hides it - nothing entered is deleted. Kept on this computer. The
     key is written out here because this runs while `state` is being built. */
  /* Appearance: the same as Windows, light, or dark. Kept on this computer.
     The shell is told too, so the title bar's buttons and the loading window
     match the page. */
  const THEMES = [['system', 'System'], ['light', 'Light'], ['dark', 'Dark']];
  const APP_ICON = '<span class="app-icon app-icon--lg" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" '
    + 'stroke="#fff" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M5 7h14" opacity=".55"/>'
    + '<path d="M5 12h14" opacity=".55"/><path d="M5 17h14" opacity=".55"/><path d="M5 16.5 10 12l3 2.5L19 8"/></svg></span>';
  /* Written out rather than read from THEMES: this runs while `state` is
     being built, before THEMES exists. */
  function loadTheme() {
    let t = null;
    try { t = localStorage.getItem('fba-theme'); } catch (e) { t = null; }
    return t === 'light' || t === 'dark' ? t : 'system';
  }
  function applyTheme(t) {
    if (!THEMES.some(x => x[0] === t)) t = 'system';
    state.theme = t;
    if (typeof document !== 'undefined') document.documentElement.setAttribute('data-theme', t);
    try { localStorage.setItem('fba-theme', t); } catch (e) { /* this session only */ }
    if (typeof window !== 'undefined' && window.desktopShell && window.desktopShell.theme) {
      window.desktopShell.theme(t).catch(() => { /* the page still changes */ });
    }
  }

  function loadShow() {
    const d = { balance: true, rules: true, deposits: true, costs: true, ads: true, more: true };
    try { Object.assign(d, JSON.parse(localStorage.getItem('fba-show-v1') || '{}')); } catch (e) { /* defaults */ }
    return d;
  }
  function saveShow() {
    try { localStorage.setItem('fba-show-v1', JSON.stringify(state.show)); } catch (e) { /* kept for this session */ }
  }
  function loadMoreOpen() {
    try { return localStorage.getItem('fba-more-open') === '1'; } catch (e) { return false; }
  }
  function saveMoreOpen() {
    try { localStorage.setItem('fba-more-open', state.moreOpen ? '1' : '0'); } catch (e) { /* this session */ }
  }
  const SHOW_ITEMS = [
    { key: 'balance', name: 'Current Amazon balance', state: 'Current Amazon balance',
      where: 'Seller Central \u2192 Payments page. It\u2019s on the page but in no downloadable report.' },
    { key: 'rules', name: 'Payout rules', state: 'Payout rules',
      where: 'Your account\u2019s payout schedule, also on the Payments page.' },
    { key: 'deposits', name: 'Bank deposits', state: 'Bank deposit history',
      where: 'Your bank statement. Amazon only knows when it sent the money.' },
    { key: 'costs', name: 'Product costs', state: 'Product costs',
      where: 'What you pay your supplier. Amazon never has this.' },
    { key: 'ads', name: 'Advertising billing', state: 'Advertising billing',
      where: 'Whether ads come out of your payout or go on a card. Amazon\u2019s forecast does not say.' },
  ];
  ...
  function amount(v, opts) {
    ...
    /* $1,234.56, -$318.51: the currency's sign, as people write money. With no
       currency known, that is said rather than guessed at. */
    return '<span class="' + cls.trim() + '">' + esc(state.filters.currency
      ? M.fmt(cents, { plus: opts.plus, currency: state.filters.currency })
      : 'Unknown currency ' + M.fmt(cents, { plus: opts.plus, bare: true })) + '</span>'
  ...
  function money(cents) {
    ...
    return '<span' + (cents < 0 ? ' class="neg"' : '') + '>' + (cents < 0 ? '-' : '')
      + '<span class="cur">' + esc(M.symbolOf(code).trim()) + '</span>'
      + esc(M.fmt(Math.abs(cents), { bare: true }) || '') + '</span>';
  }
  ...
  function economicsCards(fc) {
    ...
    html += '</tbody></table></div>'
      + ((fc.superseded || []).length
        ? '<div class="meta" style="margin-top:12px">' + fc.superseded.length + ' older '
          + (fc.superseded.length === 1 ? 'download overlaps' : 'downloads overlap')
          + ' these windows and ' + (fc.superseded.length === 1 ? 'is' : 'are')
          + ' not added - a newer download of the same weeks replaces it, so nothing is '
          + 'counted twice. They are still listed under Imported files.</div>' : '')
  ...
  function inputStates(ds) {
    ...
    const list = [
      { name: 'Payments transaction history', status: led && led.to ? 'ready' : 'optional',
        detail: led && led.to ? 'Covers ' + fmtDay(led.from) + ' \u2013 ' + fmtDay(led.to)
          : 'Not needed for the forecast. Import one only if you want your actual fees, or '
            + 'payout dates measured from your own history.',
  ...
  function dataTiles(ds) {
    ...
    const inUse = inputs.filter(i => i.status !== 'optional');
    const ready = inUse.filter(i => i.status === 'ready').length;
    const attention = inUse.filter(i => i.status !== 'ready').length;
    ...
    const open = inUse.filter(i => i.status !== 'ready');
    ...
    return '<div class="grid g3">'
      ...
      + '<div class="tile-v fig">' + ready + ' of ' + inUse.length + ' ready</div>'
      ...
      + row('Transactions', led && led.to ? 'to ' + fmtShortDay(led.to) : 'Not used', led && led.to ? 'ok' : null)
  ...
  function inputCards(ds) {
    ...
    html += '<div id="drop" style="display:flex;flex-direction:column;justify-content:center">'
      ...
      + '<div class="meta" id="progtext" style="margin-top:8px"></div></div>';
    return html + '</div>';
  }

  /* ---------- Sales & fees forecast ---------- */

  /* The drop zone, on its own: for a Fees & Economics Preview saved by hand. */
  function dropZone() {
    return '<div id="drop" style="display:flex;flex-direction:column;justify-content:center;'
      + 'margin:4px 0 16px">' + ico('upload', { size: 'lg' })
      + '<h3>Drop a Fees &amp; Economics Preview CSV here</h3>'
      + '<div class="meta">or <label style="color:var(--accent);cursor:pointer;text-decoration:underline">'
      + 'choose files<input type="file" id="filepick" multiple accept=".csv" hidden></label>'
      + ' \u00b7 for a file saved from Seller Central by hand</div>'
      + '<div class="bar" id="prog" hidden><i></i></div>'
      + '<div class="meta" id="progtext" style="margin-top:8px"></div></div>';
  }

  /* When the newest forecast file arrived. */
  function lastForecastAt() {
    const at = state.imports.filter(i => i.family === 'Fees & Economics Preview')
      .map(i => i.fetchedAt || i.importedAt).filter(Boolean).sort().pop();
    return at || null;
  }

  function forecastStatus() {
    const f = forecastOf(scopedPreviews(), horizonStart(), horizonEnd());
    const row = (label, value, kind) => '<span class="ds-r"><span class="dot'
      + (kind ? ' is-' + kind : '') + '"></span>' + esc(label) + '<b>' + esc(value) + '</b></span>';
    const at = lastForecastAt();
    const cloud = cloudWords();
    const morn = scheduleWords();
    return '<span class="ds-t">Forecast</span>'
      + row('Loaded to', f.loadedTo ? fmtShortDay(f.loadedTo) : 'Nothing yet', f.loadedTo ? 'ok' : null)
      + row('Next 8 weeks', f.coveredDays + '/' + f.days + ' days',
        f.coveredDays === f.days ? 'ok' : f.coveredDays ? 'warn' : null)
      + row('Downloaded', at ? ago(at) : 'Never', at ? 'ok' : null)
      + row('Every morning', morn[0], morn[1])
      + row('Supabase', cloud[0], cloud[1]);
  }

  /* Where the figures are, in two or three words - from what the project
     answered, never from the settings alone. */
  function cloudWords() {
    if (!state.workerInfo) return ['Not here', null];
    const st = state.syncState || {};
    if (!state.sync) {
      if (state.mirrorLoad !== 'ok') return ['Checking\u2026', null];
      const m = state.mirror || {};
      return [!m.configured || !m.canWrite ? 'Not set up' : 'Not ready', 'warn'];
    }
    if (st.status === 'saved') return ['Saved ' + (st.lastSyncedAt ? ago(st.lastSyncedAt) : ''), 'ok'];
    if (st.status === 'saving' || st.status === 'syncing') return ['Saving\u2026', null];
    if (st.status === 'conflict') return ['Needs a choice', 'warn'];
    if (st.status === 'failed') return ['Not saved yet', 'warn'];
    return ['Connected', 'ok'];
  }

  /* While the morning download waits for the person, every screen says so. */
  function scheduleBar() {
    const run = state.scheduleRun;
    if (!run || run.status !== 'waiting') return '';
    return '<div class="updbar is-warn">' + ico('missing') + '<span class="grow"><b>Amazon needs you.</b> '
      + esc(run.note || 'Complete the sign-in or check in the Amazon window.') + '</span>'
      + (continueRun ? '<button class="btn sm" data-schedcontinue="1">I\u2019ve done it \u2014 continue</button>' : '')
      + '<button class="btn sec sm" data-schedstop="1">Stop</button></div>';
  }

  /* While Supabase is not holding the figures, every screen says so, and
     what is left to do - it is where they are meant to be kept. */
  function cloudBar() {
    if (!state.workerInfo || state.mirrorLoad !== 'ok' || state.cloudBarHidden || state.setupOpen) return '';
    const st = state.syncState || {};
    if (state.sync && st.status !== 'failed') return '';
    const m = state.mirror || {};
    const cs = state.cloudStatus;
    const why = !m.configured ? 'Supabase is not connected on this computer.'
      : !m.canWrite ? 'Supabase is connected, but its secret key is not added on this computer.'
      : st.status === 'failed' ? (st.reason || 'The last save did not reach Supabase.')
      : cs && !cs.ready ? (cs.detail || 'Supabase is not ready.') : '';
    if (!why) return '';
    return '<div class="updbar is-warn">' + ico('missing') + '<span class="grow"><b>Your figures are only on '
      + 'this computer.</b> ' + esc(why) + '</span>'
      + (state.sync ? '<button class="btn sm" id="syncnow">Try again now</button>'
        : '<button class="btn sm" data-setupopen="1">Finish setting up Supabase</button>')
      + '<button class="btn sec sm" data-cloudbarhide="1" title="Hide until the app is opened again">Later</button>'
      + '</div>';
  }

  const pct = (part, whole) => (part == null || !whole ? '\u2014'
    : (Math.round(part / whole * 1000) / 10).toFixed(1) + '%');

  /* ── the forecast screen's charts: dates, zoom, and lines over time ── */

  /* The dates the forecast screen shows: tomorrow to the date picked, or to
     the end of what is downloaded, or eight weeks. */
  function salesRange() {
    const tomorrow = horizonStart();
    const probe = forecastOf(scopedPreviews(), tomorrow, tomorrow);
    const from = state.salesFrom || tomorrow;
    const to = state.salesTo && state.salesTo >= from ? state.salesTo
      : (probe.loadedTo && probe.loadedTo >= from ? probe.loadedTo : CSV.addDays(from, 55));
    return { from, to, loadedFrom: probe.loadedFrom, loadedTo: probe.loadedTo };
  }

  /* A chart's zoom, kept inside the dates on screen; none when it would
     show them all anyway. */
  function clampZoom(id, range) {
    const z = state.zoom[id];
    if (!z) return null;
    range = range || salesRange();
    const a = z.from < range.from ? range.from : z.from;
    const b = z.to > range.to ? range.to : z.to;
    if (a >= b || (a === range.from && b === range.to)) { delete state.zoom[id]; return null; }
    return { from: a, to: b };
  }
  function setZoom(id, z) {
    if (z && z.from < z.to) state.zoom[id] = z; else delete state.zoom[id];
    render();
  }
  /* In (half as many days), out (twice as many), earlier, later - around a
     date when one is given (the pointer's, for Ctrl + scroll). */
  function zoomChart(id, kind, at) {
    const range = salesRange();
    const total = CSV.daysBetween(range.from, range.to) + 1;
    if (kind === 'reset' || total < 3) { setZoom(id, null); return; }
    const z = clampZoom(id, range) || range;
    let a = CSV.daysBetween(range.from, z.from), b = CSV.daysBetween(range.from, z.to);
    const len = b - a + 1;
    if (kind === 'in' || kind === 'out') {
      const nl = Math.max(3, Math.min(total, Math.round(len * (kind === 'in' ? 0.5 : 2))));
      const p = at ? CSV.daysBetween(range.from, at) : (a + b) / 2;
      const frac = len > 1 ? (p - a) / (len - 1) : 0.5;
      a = Math.round(p - frac * (nl - 1));
      b = a + nl - 1;
    } else {
      const step = Math.max(1, Math.round(len / 2)) * (kind === 'left' ? -1 : 1);
      a += step; b += step;
    }
    if (a < 0) { b -= a; a = 0; }
    if (b > total - 1) { a = Math.max(0, a - (b - (total - 1))); b = total - 1; }
    setZoom(id, a === 0 && b === total - 1 ? null
      : { from: CSV.addDays(range.from, a), to: CSV.addDays(range.from, b) });
  }
  function zoomBar(id, z, range) {
    const days = z ? CSV.daysBetween(z.from, z.to) + 1 : CSV.daysBetween(range.from, range.to) + 1;
    const b = (k, icon, label, off) => '<button type="button" class="icon-button icon-button--sm" data-czoom="' + k
      + '" data-chart="' + id + '" title="' + label + '" aria-label="' + label + '"' + (off ? ' disabled' : '') + '>'
      + ico(icon) + '</button>';
    return '<div class="zoom-row zoombar">'
      + (z ? '<span class="zoom-row__state">' + ico('add') + 'Zoomed in: ' + esc(fmtShortDay(z.from)) + ' – '
        + esc(fmtShortDay(z.to)) + ' of ' + esc(fmtShortDay(range.from)) + ' – ' + esc(fmtShortDay(range.to)) + '</span>'
        : '<span class="zoom-row__hint">Drag across the chart to zoom in · Ctrl + scroll zooms · '
          + 'double-click resets</span>')
      + '<div class="zoom-row__buttons">' + b('out', 'minus', 'Zoom out', !z) + b('in', 'add', 'Zoom in', days <= 3)
      + '<span class="zoom-row__sep"></span>'
      + b('left', 'chevronLeft', 'Earlier', !z || z.from <= range.from)
      + b('right', 'chevronRight', 'Later', !z || z.to >= range.to) + '</div>'
      + (z ? '<button type="button" class="btn plain sm" data-czoom="reset" data-chart="' + id + '">' + ico('reset')
        + 'Reset zoom</button>' : '')
      + '</div>';
  }

  /* Two or more choices, one of them on: the design's segmented control. */
  const segmented = (label, attr, items, cur, extra) => '<div class="segmented" role="group" aria-label="'
    + esc(label) + '">' + items.map(([k, n]) => '<button type="button" ' + attr + '="' + esc(k) + '"'
      + (extra || '') + ' aria-pressed="' + (cur === k) + '">' + esc(n) + '</button>').join('') + '</div>';

  /* A chart of any set of lines over the chosen dates, with a chip per line
     to show or hide it: fees against net sales, for everything or for one
     product. */
  function overTime(id, title, days, series, defaults, range, cur) {
    const keys = series.map(x => x.key);
    let shown = (state.chartShow[id] || defaults).filter(k => keys.indexOf(k) >= 0);
    if (!shown.length) shown = defaults.slice(0, 1);
    state.chartShow[id] = shown;
    const mode = state.chartMode[id] === 'weekly' ? 'weekly' : 'running';
    const z = clampZoom(id, range);
    return '<div class="overtime" id="' + id + 'chart"><div class="card__header" style="margin-bottom:12px">'
      + '<div class="card__titles"><h3 class="card__title">' + esc(title) + '</h3></div>'
      + '<div class="card__actions">'
      + segmented('How the lines add up', 'data-cmode', [['running', 'Running total'], ['weekly', 'Week by week']], mode,
        ' data-chart="' + id + '"')
      + '</div></div>'
      + '<div class="serchips" role="group" aria-label="Lines on the chart">' + series.map(s => '<button type="button" '
        + 'class="chip serchip" data-sser="' + id + '" data-key="' + esc(s.key) + '" aria-pressed="'
        + (shown.indexOf(s.key) >= 0) + '" style="--c:' + s.colour + '" title="Show or hide on the chart">'
        + '<span class="cdot"></span>' + esc(s.name) + '</button>').join('')
      + '</div>'
      + zoomBar(id, z, range)
      + Charts.forecast(days, { id, series, metrics: shown, mode, currency: cur, mondayOf: Simple.mondayOf,
        zoom: z, pctOf: 'netSales', legend: false })
      + '</div>';
  }

  screens.sales = function () {
    const scoped = scopedPreviews();
    if (!scoped.length) {
      return '<section class="card"><div class="empty-state">'
        + '<div class="empty-state__icon">' + ico('forecast') + '</div>'
        + '<h2>No forecast yet</h2>'
        + '<p>Get Amazon’s estimate of your sales and fees. You pick the dates, it downloads in about a '
        + 'minute, and it appears here.</p>'
        + (state.workerInfo ? '<button class="btn lg" data-startall="1">' + ico('download')
          + 'Get Amazon’s forecast</button>' : '')
        + '<div class="empty-state__aside">Or drop a Fees &amp; Economics Preview CSV on '
        + '<button class="linkbtn" data-go="data">Downloads</button>.</div>'
        + '</div></section>';
    }

    const range = salesRange();
    const { from, to } = range;
    const f = forecastOf(scoped, from, to);

    /* the dates */
    /* Any dates: From and To, a step the same length earlier or later, the
       usual spans from tomorrow, and a bar to scroll the window across
       everything downloaded. */
    const tomorrow = horizonStart();
    const len = CSV.daysBetween(from, to) + 1;
    const spanFrom = [range.loadedFrom, tomorrow, from].filter(Boolean).sort()[0];
    const spanTo = [range.loadedTo, to].filter(Boolean).sort().pop();
    const maxOff = Math.max(0, CSV.daysBetween(spanFrom, spanTo) - (len - 1));
    const off = Math.max(0, Math.min(maxOff, CSV.daysBetween(spanFrom, from)));
    const stepWords = len === 1 ? 'a day' : len + ' days';
    const shownDates = fmtDay(from) + (len > 1 ? ' – ' + fmtDay(to) : '');
    const quick = [1, 7, 14, 30, 56, 120].find(n => from === tomorrow && CSV.addDays(state.today, n) === to);
    const step = (dir, icon, words, off2) => '<button type="button" class="icon-button" data-sstep="' + dir
      + '" title="' + words + '" aria-label="' + words + '"' + (off2 ? ' disabled' : '') + '>' + ico(icon) + '</button>';
    let html = '<section class="card" id="salesdates" aria-label="Dates"><div class="date-bar">'
      + '<div class="field"><label class="field__label" for="s-from">From</label>'
      + '<input type="date" id="s-from" value="' + esc(from) + '" min="' + esc(spanFrom) + '"></div>'
      + '<div class="field"><label class="field__label" for="s-to">Up to</label>'
      + '<input type="date" id="s-to" value="' + esc(to) + '" min="' + esc(from) + '"></div>'
      + '<div class="field"><span class="field__label">Move</span><div class="zoom-row__buttons">'
      + step('-1', 'chevronLeft', stepWords + ' earlier', from <= spanFrom)
      + step('1', 'chevronRight', stepWords + ' later', to >= spanTo) + '</div></div>'
      + '<div class="date-bar__quick"><span class="field__label">Quick choice</span>'
      + segmented('Quick choice', 'data-sdays', [1, 7, 14, 30, 56, 120].map(n => [String(n),
        n === 1 ? 'Tomorrow' : n === 56 ? '8 weeks' : n === 120 ? '4 months' : n + ' days']), String(quick))
      + '</div></div>'
      + (maxOff > 0 ? '<div class="scrollrow"><span>' + esc(fmtShortDay(spanFrom)) + '</span>'
        + '<input type="range" id="s-scroll" min="0" max="' + maxOff + '" step="1" value="' + off + '" data-span-from="'
        + esc(spanFrom) + '" data-len="' + len + '" aria-label="Scroll the dates shown">'
        + '<span>' + esc(fmtShortDay(spanTo)) + '</span></div>' : '')
      + '<div class="date-meta"><span>Showing <b id="s-shown">' + esc(shownDates) + '</b></span><span>·</span>'
      + '<span>' + len + (len === 1 ? ' day' : ' days') + '</span><span>·</span>'
      + '<span>' + (at => at ? 'downloaded ' + esc(ago(at)) : 'imported from a file')(lastForecastAt()) + '</span>'
      + (f.store ? '<span>·</span><span>' + esc(f.store) + '</span>' : '') + '</div>';
    if (f.coveredDays < f.days) {
      html += '<div class="note is-warn" role="status">' + ico('warning') + '<div><b>'
        + (f.days - f.coveredDays) + ' of your ' + f.days + ' days have no forecast yet.</b> '
        + (f.loadedTo ? 'Amazon’s forecast is downloaded up to ' + esc(fmtDay(f.loadedTo)) + '. ' : '')
        + 'The figures below cover the other ' + f.coveredDays + ' days only.</div>'
        + (state.workerInfo ? '<button class="btn sec sm note__action" data-sget="1">'
          + ico('download') + 'Get Amazon’s forecast for ' + esc(fmtShortDay(from))
          + ' – ' + esc(fmtShortDay(to)) + '</button>' : '')
        + '</div>';
    } else if (!f.exact) {
      html += '<div class="note">' + ico('info') + '<div>These dates are part of a longer download, so its '
        + 'totals are split evenly across its days. For Amazon’s own figures for exactly these dates, '
        + (state.workerInfo ? '<button class="linkbtn" data-sget="1">download them</button>.' : 'download them.')
        + '</div></div>';
    }
    html += '</section>';

    if (!f.coveredDays) return html;

    /* the totals: each tile adds itself to the chart below, or takes itself
       off it - one to all four, to compare */
    const cur = state.filters.currency || f.currency || 'USD';
    const metrics = Charts.MEASURES.map(x => x.key).filter(k => (state.salesMetrics || []).indexOf(k) >= 0);
    if (!metrics.length) metrics.push('netSales');
    const source = f.exact
      ? '<span class="source-tag source-tag--exact" title="Amazon’s own forecast for exactly these dates">'
        + ico('projected') + 'Amazon forecast</span>'
      : '<span class="source-tag" title="A share of a longer download: its totals split evenly across its days">'
        + ico('projected') + 'Share of a longer forecast</span>';
    const pick = (key, label, cents, foot, src) => {
      const on = metrics.indexOf(key) >= 0;
      return '<button type="button" class="tile" data-smetric="' + key + '" aria-pressed="' + on
        + '" style="--c:' + Charts.MEASURES.find(x => x.key === key).colour + '" title="'
        + (on ? 'Take ' + esc(label.toLowerCase()) + ' off the chart' : 'Add ' + esc(label.toLowerCase())
          + ' to the chart') + '">'
        + '<span class="tile__head"><span class="cdot"></span><span class="tile__label">' + esc(label) + '</span>'
        + '<span class="tile__toggle"><span class="tile__toggle-text">' + (on ? 'On chart' : 'Add to chart') + '</span>'
        + '<span class="tile__toggle-mark">' + ico('check') + '</span></span></span>'
        + (cents == null ? '<span style="margin-top:6px">' + unavailable(null, { size: 'sm' }) + '</span>'
          : '<span class="tile__value fig' + (cents < 0 ? ' is-neg' : '') + '">'
            + esc(M.fmt(cents, { currency: cur })) + '</span>'
          + '<span class="tile__foot">' + src + (foot ? '<span>' + esc(foot) + '</span>' : '') + '</span>')
        + '</button>';
    };
    html += '<div class="tiles" id="salestiles" role="group" aria-label="Totals — each adds its line to the chart">'
      + pick('netSales', 'Net sales', f.netSales,
        (f.sales != null ? 'Sales ' + M.fmt(f.sales, { currency: cur }) + ' before returns' : '')
        + (f.units != null ? ' · ' + f.units.toLocaleString() + ' units' : ''), source)
      + pick('fees', 'Amazon fees', f.feeTotal == null ? null : -f.feeTotal,
        pct(f.feeTotal, f.netSales) + ' of net sales', source)
      + pick('advertising', 'Advertising', f.advertising == null ? null : -f.advertising,
        pct(f.advertising, f.netSales) + ' of net sales', source)
      + pick('after', 'After fees and advertising', f.afterFeesAndAds,
        'After fees only: ' + M.fmt(f.afterFees, { currency: cur }),
        '<span class="source-tag" title="Worked out from the figures beside it">' + ico('calculated') + 'Calculated</span>')
      + '</div>';

    /* the chart of the picked tiles, read by hovering */
    const names = metrics.map(k => Charts.MEASURES.find(x => x.key === k).name);
    const mode = state.salesMode === 'weekly' ? 'weekly' : 'running';
    const all4 = metrics.length === 4;
    html += '<section class="card" id="saleschart"><div class="card__header">'
      + '<div class="card__titles"><h2 class="card__title">'
      + esc(names.length > 1 ? names.slice(0, -1).join(', ') + ' and ' + names[names.length - 1] : names[0])
      + (mode === 'weekly' ? ', week by week' : ', running total') + '</h2></div>'
      + '<div class="card__actions">'
      + '<button type="button" class="btn ' + (all4 ? 'tint' : 'sec') + ' sm" data-sall="1" aria-pressed="' + all4 + '">'
      + (all4 ? ico('check') + 'Just net sales' : 'Compare all four') + '</button>'
      + segmented('How the lines add up', 'data-smode', [['running', 'Running total'], ['weekly', 'Week by week']], mode)
      + '</div></div>'
      + zoomBar('sales', clampZoom('sales', range), range)
      + Charts.forecast(f.daily, { id: 'sales', metrics, mode, currency: cur, mondayOf: Simple.mondayOf,
        zoom: clampZoom('sales', range), pctOf: 'netSales' })
      + '<div class="meta" style="margin-top:10px">Hover over the chart to read any '
      + (mode === 'weekly' ? 'week' : 'day') + '. Click the tiles above to add or take them off. '
      + (f.files.length === 1 ? 'Amazon gives one total for the whole download, so each day carries an equal '
        + 'share of it.' : 'Amazon gives one total per download; within each, every day carries an equal share.')
      + '</div></section>';

    /* where net sales go: every fee, advertising, and what is left */
    const feeList = f.fees.filter(y => y.amount);
    const order = feeList.map(x => x.name);
    const colourOf = n => Charts.feeColour(n, order.indexOf(n));
    const partsFor = (feeBy, ads) => order.map(n => ({ name: n, amount: feeBy[n] || 0 }))
      .concat([{ name: 'Advertising', amount: ads || 0 }]);
    const allFees = {};
    for (const x of feeList) allFees[x.name] = x.amount;
    const left = f.afterFeesAndAds;
    /* the lines of the fees-over-time charts: net sales, each kind of fee,
       advertising, and what is left (off to start with) */
    const lineSeries = [{ key: 'netSales', name: 'Net sales', colour: 'var(--mark-1)', sign: 1 }]
      .concat(order.map(n => ({ key: 'fee:' + n, name: n, colour: colourOf(n), sign: -1 })))
      .concat([{ key: 'advertising', name: 'Advertising', colour: Charts.feeColour('Advertising'), sign: -1 },
        { key: 'left', name: 'Left after fees and ads', colour: Charts.feeColour('Left after fees and ads'), sign: 1,
          dash: '9 5' }]);
    const lineDefault = lineSeries.filter(x => x.key !== 'left').map(x => x.key);
    const widest = Math.max(1, ...feeList.map(x => x.amount), f.feeTotal || 0, f.advertising || 0, Math.abs(left || 0));
    const feeRow = (name, colour, cents, share, cls) => '<div class="grid-table__row' + (cls ? ' ' + cls : '')
      + '" role="row"><span class="cell-with-dot"><span class="cdot" style="--c:' + colour + '"></span><span>'
      + esc(name) + '</span></span><span class="num">' + amount(cents) + '</span><span class="num">'
      + pct(share, f.netSales) + '</span><div class="inline-bar"><div class="inline-bar__fill" style="width:'
      + Math.min(100, Math.max(0.5, Math.abs(share || 0) / widest * 100)).toFixed(1) + '%;--c:' + colour
      + '"></div></div></div>';
    html += '<h2 class="section-title">Where your net sales go <small>'
      + esc(M.fmt(f.netSales, { currency: cur })) + ' net sales</small></h2>'
      + '<section class="card" id="spendcard"><div class="card__header" style="margin-bottom:12px">'
      + '<div class="card__titles"><h3 class="card__title">Out of every ' + esc(M.fmt(100, { currency: cur }).replace(/\.00$/, ''))
      + ' of net sales</h3><p class="card__subtitle">Each fee and the advertising, as a share of net sales, and what '
      + 'is left. Hover over a piece for the amounts.</p></div></div>'
      + Charts.shareBar({ label: 'All products', netSales: f.netSales, parts: partsFor(allFees, f.advertising) },
        { currency: cur })
      + '<div class="divider" style="margin:24px 0 20px"></div>'
      + overTime('spend', 'Net sales and each fee, over time', Simple.seriesFor(f, null), lineSeries, lineDefault,
        range, cur)
      + '<div class="grid-table" role="table" style="margin-top:20px;--cols:minmax(0,1.4fr) 130px 110px minmax(0,1fr)">'
      + '<div class="grid-table__row grid-table__row--head" role="row"><span>What</span><span class="num">Amount</span>'
      + '<span class="num">Of net sales</span><span></span></div>'
      + feeList.map(x => feeRow(x.name, colourOf(x.name), -x.amount, x.amount)).join('')
      + feeRow('All Amazon fees', 'var(--mark-2)', f.feeTotal == null ? null : -f.feeTotal, f.feeTotal, 'is-subtotal')
      + feeRow('Advertising (Sponsored Products)', Charts.feeColour('Advertising'),
        f.advertising == null ? null : -f.advertising, f.advertising)
      + feeRow('Left after fees and ads', Charts.feeColour('Left after fees and ads'), left, left, 'is-total')
      + '</div></section>';

    /* costs by product, against each product's own net sales */
    const sort = state.skuSort || 'cost';
    const share = state.skuShare !== false;
    const costOf = r => (r.fees || 0) + (r.advertising || 0);
    const ranked = f.bySku.filter(r => (r.netSales || 0) > 0 || costOf(r)).slice().sort((a2, b2) => {
      if (sort === 'sales') return (b2.netSales || 0) - (a2.netSales || 0);
      if (sort === 'pct') {
        const ra = a2.netSales > 0 ? costOf(a2) / a2.netSales : Infinity;
        const rb = b2.netSales > 0 ? costOf(b2) / b2.netSales : Infinity;
        return rb - ra || costOf(b2) - costOf(a2);
      }
      return costOf(b2) - costOf(a2);
    });
    const showN = state.skuAll ? ranked.length : Math.min(20, ranked.length);
    const losing = ranked.filter(r => (r.netSales || 0) - costOf(r) < 0).length;
    const pickSku = ranked.some(r => r.msku === state.skuPick) ? state.skuPick : (ranked[0] ? ranked[0].msku : null);
    html += '<h2 class="section-title">Costs by product <small>' + ranked.length + ' products'
      + (losing ? ' · <span class="is-neg">' + losing + ' cost more than they sell</span>' : '')
      + '</small></h2>'
      + '<section class="card" id="skuspend"><div class="card__header" style="align-items:center">'
      + '<div class="card__actions" style="flex:1"><span class="field__label" style="margin:0">Sort</span>'
      + segmented('Sort the products', 'data-ssort', [['cost', 'Biggest costs'], ['pct', 'Highest % of sales'],
        ['sales', 'Biggest sales']], sort)
      + '</div><div class="card__actions"><span class="field__label" style="margin:0">Bars</span>'
      + segmented('How the bars are drawn', 'data-sshare', [['1', 'Share of its sales'], ['0', 'Amounts']], share ? '1' : '0')
      + '</div></div>'
      + (pickSku ? '<div class="skupick"><label class="field__label" for="skupick">' + ico('product')
        + '</label><select id="skupick" aria-label="Product">'
        + ranked.slice(0, 500).map(r => '<option value="' + esc(r.msku) + '"' + (r.msku === pickSku ? ' selected' : '')
          + '>' + esc(r.msku) + (r.asin ? ' · ' + esc(r.asin) : '') + ' — costs ' + pct(costOf(r), r.netSales)
          + ' of its sales</option>').join('')
        + '</select><span class="meta">or click a product below</span></div>'
        + overTime('sku', pickSku + ', over time', Simple.seriesFor(f, pickSku), lineSeries, lineDefault, range, cur)
        + '<div class="divider" style="margin:20px 0 16px"></div>'
        : '')
      + '<div class="legend" style="margin:0 0 12px">' + order.concat(['Advertising', 'Left after fees and ads'])
        .map(n => '<span><i class="swatch" style="--c:' + colourOf(n) + '"></i>' + esc(n) + '</span>').join('')
      + (share ? '<span><i class="swatch swatch--marker"></i>100% of net sales</span>'
        : '<span><i class="swatch swatch--marker"></i>Its net sales</span>')
      + '</div>'
      + Charts.productRows(ranked.slice(0, showN).map(r => ({ label: r.msku, sub: r.asin, netSales: r.netSales,
        parts: partsFor(r.feeBy || {}, r.advertising) })), { currency: cur, share, picked: pickSku,
        ariaLabel: 'Costs by product' })
      + (ranked.length > 20 ? '<div class="btnrow" style="justify-content:center;margin-top:12px">'
        + '<button type="button" class="btn sec" data-sskuall="' + (state.skuAll ? '0' : '1') + '">'
        + (state.skuAll ? 'Show the top 20' : 'Show all ' + ranked.length) + '</button></div>' : '')
      + '</section>';

    /* week by week */
    const wk = f.weeks.reduce((t, w) => ({ netSales: t.netSales + w.netSales, fees: t.fees + w.fees,
      advertising: t.advertising + w.advertising }), { netSales: 0, fees: 0, advertising: 0 });
    html += '<h2 class="section-title">Week by week</h2>'
      + '<section class="card"><div class="grid-table" role="table" style="--cols:minmax(0,1fr) repeat(4, minmax(0,1fr))">'
      + '<div class="grid-table__row grid-table__row--head" role="row"><span>Week of</span>'
      + '<span class="num">Net sales</span><span class="num">Amazon fees</span><span class="num">Advertising</span>'
      + '<span class="num">After fees &amp; ads</span></div>';
    for (const w of f.weeks) {
      html += '<div class="grid-table__row" role="row"><span>' + esc(fmtShortDay(w.week))
        + (w.days < 7 ? ' <span class="faint">(' + w.days + (w.days === 1 ? ' day' : ' days') + ')</span>' : '')
        + '</span><span class="num">' + amount(w.netSales) + '</span><span class="num">' + amount(-w.fees) + '</span>'
        + '<span class="num">' + amount(-w.advertising) + '</span><span class="num">'
        + amount(w.netSales - w.fees - w.advertising) + '</span></div>';
    }
    if (f.weeks.length > 1) {
      html += '<div class="grid-table__row is-total" role="row"><span>All ' + f.weeks.length + ' weeks</span>'
        + '<span class="num">' + amount(wk.netSales) + '</span><span class="num">' + amount(-wk.fees) + '</span>'
        + '<span class="num">' + amount(-wk.advertising) + '</span><span class="num">'
        + amount(wk.netSales - wk.fees - wk.advertising) + '</span></div>';
    }
    html += '</div></section>';

    /* the products, as a table */
    const shown = f.bySku.slice(0, 150);
    html += '<h2 class="section-title">By product <small>' + f.bySku.length + ' products</small></h2>'
      + '<section class="card"><div class="grid-scroll"><div class="grid-table" role="table" style="--cols:'
      + 'minmax(0,1.3fr) 60px minmax(0,1fr) minmax(0,1fr) minmax(0,1.3fr) minmax(0,1fr) 70px minmax(0,1fr)">'
      + '<div class="grid-table__row grid-table__row--head" role="row"><span>Product</span><span class="num">Units</span>'
      + '<span class="num">Net sales</span><span class="num">Amazon fees</span><span>Biggest fee</span>'
      + '<span class="num">Advertising</span><span class="num">Costs</span><span class="num">After fees &amp; ads</span></div>';
    for (const r of shown) {
      const top = Object.entries(r.feeBy || {}).sort((x, y) => y[1] - x[1])[0];
      html += '<div class="grid-table__row" role="row"><div class="cell-stack"><b title="' + esc(r.msku) + '">'
        + esc(r.msku) + '</b>' + (r.asin ? '<small>' + esc(r.asin) + '</small>' : '') + '</div>'
        + '<span class="num">' + (r.units || 0).toLocaleString() + '</span>'
        + '<span class="num">' + amount(r.netSales) + '</span>'
        + '<span class="num">' + amount(r.fees == null ? null : -r.fees) + '</span>'
        + (top && top[1] ? '<span class="cell-with-dot"><span class="cdot" style="--c:' + colourOf(top[0]) + '"></span>'
          + '<span title="' + esc(top[0]) + '">' + esc(Charts.shortName(top[0])) + ' <span class="faint">'
          + pct(top[1], r.netSales) + '</span></span></span>' : '<span class="faint">—</span>')
        + '<span class="num">' + amount(r.advertising == null ? null : -r.advertising) + '</span>'
        + '<span class="num">' + pct(costOf(r), r.netSales) + '</span>'
        + '<span class="num"><b>' + amount(r.net) + '</b></span></div>';
    }
    html += '</div></div>'
      + (f.bySku.length > shown.length ? '<div class="meta" style="margin-top:8px">The top '
        + shown.length + ' of ' + f.bySku.length + ' by net sales.</div>' : '')
      + '<div class="meta" style="margin-top:12px">Amazon fees here are every fee Amazon forecasts for the '
      + 'product — referral, FBA fulfilment, storage and the rest. Costs are fees and advertising as a share '
      + 'of the product’s net sales.</div></section>';

    if (f.superseded) {
      html += '<div class="meta" style="margin:4px 2px">' + f.superseded + ' older overlapping '
        + (f.superseded === 1 ? 'download is' : 'downloads are') + ' replaced by a newer one and not '
        + 'added, so nothing is counted twice.</div>';
    }
    return html;
  };

  /* Payout rules, as they were on Data & Assumptions before 5.0. */
  function payoutRulesCard() {
    let html = '';
  ...
  /* ---------- Your own figures (More tools) ---------- */
  screens.inputs = function () {
    const on = SHOW_ITEMS.filter(i => state.show[i.key]);
    let html = '<div class="note">' + ico('info') + '<div>Amazon\u2019s downloads do not carry these. The '
      + 'Sales &amp; fees forecast does not need them; they feed the other tools under More tools \u2014 Cash '
      + 'Dashboard, Payout Forecast, Cash Plan and Profitability. '
      + '<button data-go="settings" style="background:none;border:0;padding:0;color:var(--accent);cursor:pointer;'
      + 'font:inherit;text-decoration:underline">Choose which appear here in Settings</button>.</div></div>';
    if (!on.length) {
      return html + '<section class="card">' + Charts.emptyState('Everything here is turned off',
        'Turn the ones you want back on in Settings.') + '</section>';
    }
    const TAGS = { ready: ['is-ok', 'Ready'], partial: ['is-warn', 'Partial'], missing: ['', 'Missing'],
      optional: ['nodot', 'Optional'] };
    const states = inputStates(dataset());
    html += '<div class="grid g3">';
    for (const item of on) {
      const i = states.find(x => x.name === item.state);
      if (!i) continue;
      const t = TAGS[i.status];
      html += '<article class="card" style="display:flex;flex-direction:column;gap:8px;padding:20px 22px">'
        + '<div class="card-h nowrap" style="margin:0"><h2 style="font-size:var(--t-h2)">'
        + esc(i.name) + '</h2><span class="tag ' + t[0] + '">' + t[1] + '</span></div>'
        + '<div class="meta" style="flex:1">' + esc(i.detail) + '</div>'
        + '<div class="meta" style="font-size:var(--t-tag)">From: ' + esc(item.where) + '</div>'
        + (i.action ? '<div class="btnrow">' + i.action + '</div>' : '')
        + '</article>';
    }
    html += '</div>';
    if (state.show.balance) html += recordedBalancesCard();
    if (state.show.rules) html += payoutRulesCard();
    if (state.show.deposits) html += depositsCard();
    if (state.show.costs) html += costsCard();
    if (state.show.ads) html += adsCard();

    const out = Store.outstanding(checklistState());
    html += '<details class="card"><summary>' + ico('chevronRight', { size: 'sm' })
      + 'What is still needed \u00b7 ' + out.length + '</summary><div class="body">'
      + '<div class="sub">For the tools under More tools, in the order that unblocks the most. ' + out.length
      + ' of ' + Store.CHECKLIST.length + ' outstanding. The Sales &amp; fees forecast needs none of them.</div>';
  ...
  screens.settings = function () {
    const morningCard = scheduleCard();
    const sw = (key, name, where) => '<label class="setrow"><span><b>' + esc(name) + '</b>'
      + (where ? '<span class="meta">' + esc(where) + '</span>' : '') + '</span>'
      + '<input type="checkbox" role="switch" class="switch" data-show="' + key + '"'
      + (state.show[key] ? ' checked' : '') + ' aria-label="' + esc(name) + '"></label>';
    let html = '<section class="card" id="appearancecard"><div class="card-h">' + ico(state.theme === 'dark' ? 'dark'
      : state.theme === 'light' ? 'light' : 'system') + '<h2>Appearance</h2></div>'
      + '<div class="sub">Light, dark, or the same as Windows \u2014 it changes when Windows does.</div>'
      + segmented('Appearance', 'data-themepick', THEMES, state.theme)
      + '</section>'
      + morningCard + '<section class="card"><div class="card-h">' + ico('assumptions') + '<h2>What to show</h2></div>'
      + '<div class="sub">Figures Amazon\u2019s downloads do not carry. Turn on the ones you want to enter '
      + 'yourself; they appear under <b>More tools \u2192 Your own figures</b>. Turning one off only hides it '
      + '\u2014 nothing you entered is deleted.</div>'
      + SHOW_ITEMS.map(i => sw(i.key, i.name, i.where)).join('')
      + '</section>'
      + '<section class="card"><div class="card-h">' + ico('dashboard') + '<h2>Sidebar</h2></div>'
      + sw('more', 'More tools', 'The tools from before 5.0 \u2014 Cash Dashboard, Payout Forecast, Cash Plan, '
        + 'Amazon Expenses, Profitability, Reconciliation \u2014 and Your own figures.')
      + '</section>'
      + '<section class="card" id="guidecard"><div class="card-h">' + ico('info') + '<h2>Guide</h2></div>'
      + '<div class="sub">A step-by-step tour of the app, pointing at each part on the screen. It runs by '
      + 'itself the first time; after an update, a shorter one shows what changed.</div>'
      + '<div class="btnrow"><button class="btn" data-guide="full">Show the guide</button>'
      + '<button class="btn sec" data-guide="new">What\u2019s new in ' + esc(GUIDE_LATEST) + '</button></div>'
      + '</section>';
    return html;
  };
  ...
  screens.data = function () {
    ...
    if (!state.imports.length) {
      html += '<div class="meta" style="margin-top:8px">No files yet. Get Amazon\u2019s forecast with the button '
        + 'at the top, or drop a Fees &amp; Economics Preview CSV here.</div>';
    } else {
  ...
  const SUGGESTED = [
    'Summarise my forecast sales and fees.',
    'Which products bring in the most after fees?',
    'Which fees take the biggest share?',
    'How do the coming weeks compare?',
  ];
  ...
  function buildBrief() {
    ...
    const L = [];
    /* The forecast first: it is what the app is for now. */
    {
      const { from, to } = salesRange();
      const f = forecastOf(scopedPreviews(), from, to);
      L.push('SALES & FEES FORECAST (Amazon Fees & Economics Preview), ' + from + ' to ' + to
        + ', ' + f.coveredDays + ' of ' + f.days + ' days covered'
        + (f.exact ? ' (Amazon\u2019s own figures)' : ' (part of a longer download, split evenly by day)') + ':');
      if (f.coveredDays) {
        L.push('  net sales ' + M.fmt(f.netSales) + '; sales before returns ' + M.fmt(f.sales)
          + '; units ' + (f.units == null ? 'unknown' : f.units));
        L.push('  Amazon fees ' + M.fmt(f.feeTotal) + '; advertising ' + M.fmt(f.advertising)
          + '; after fees ' + M.fmt(f.afterFees) + '; after fees and advertising ' + M.fmt(f.afterFeesAndAds));
        L.push('  fees by type: ' + f.fees.map(x => x.name + ' ' + M.fmt(x.amount)).join('; '));
        L.push('  by week (week of: net sales / fees / advertising): ' + f.weeks.map(w => w.week + ': '
          + M.fmt(w.netSales) + ' / ' + M.fmt(w.fees) + ' / ' + M.fmt(w.advertising)).join('; '));
        L.push('  top products (net sales, Amazon fees, advertising, after fees and ads; biggest fees): '
          + f.bySku.slice(0, 15).map(r => r.msku + ' ' + M.fmt(r.netSales) + ', ' + M.fmt(r.fees) + ', '
            + M.fmt(r.advertising) + ', ' + M.fmt(r.net) + '; ' + Object.entries(r.feeBy || {})
              .sort((a, b) => b[1] - a[1]).slice(0, 3).map(([n, v]) => n + ' ' + M.fmt(v)).join(', '))
            .join(' | '));
      } else {
        L.push('  no forecast downloaded for these dates.');
      }
      L.push('');
    }
    L.push('APPLICATION: Amazon sales and fees forecast (cash-planning figures below are secondary).');
  ...
  async function removeImport(id) {
    ...
    /* Drop the account copy as well, or the next reconcile pulls it back -
       and say it was removed, so another computer's copy does not either. */
    const at = new Date().toISOString();
    state.removedImports = Sync.trimRemovals((state.removedImports || []).concat(gone.map(r =>
      Object.assign(Sync.removal(r, at), { jobIds: [].concat(r.jobIds || [], r.jobId ? [r.jobId] : []) }))), at);
    ...
    const setProg = (pct, text, final) => {
      ...
      if (final) {
        state.lastImport = Object.assign(
          ...
          final === true ? {} : final);
        noticeForImport(state.lastImport);
        /* The bar is for while it works; what happened is in the bell. */
        setTimeout(() => {
          const b = $('#prog'), tt = $('#progtext');
          if (b) b.hidden = true;
  ...
  async function importPreview(file, setProg, fail, contentHash) {
    ...
    if (prior) {
      ...
      if (replaced) {
        /* Recorded as let go. Without this the helper - which still held the
           older download - had it imported straight back on the next check,
           over the newer one: yesterday's forecast replacing today's. The
           record also keeps other computers from bringing it back. */
        const at = new Date().toISOString();
        const jobsOf = [...new Set([].concat(replaced.jobIds || [], replaced.jobId ? [replaced.jobId] : []))];
        state.removedImports = Sync.trimRemovals((state.removedImports || [])
          .concat([Object.assign(Sync.removal(replaced, at), { jobIds: jobsOf })]), at);
        /* A version the morning download fetched is its own to clear away:
           the file, the archived rows and their copy in Supabase. */
        if (replaced.schedule && state.worker) {
          for (const jid of jobsOf) state.worker.remove(jid).then(r => { if (r && r.archived) refreshMirrorSoon(); })
            .catch(() => { /* the removal record keeps it out */ });
        }
      }
      if (state.sync && state.sync.available) {
  ...
  /* The outcome of a press of the button, in the bell. Each report's own
     progress is announced as its job changes (noticeJobChanges). */
  async function getTodaysData(choice) {
    await getTodaysDataInner(choice);
    const said = state.refreshNote || '';
    if (state.refreshPhase === 'failed') notify('error', 'Could not get Amazon data', said);
    else if (/skipped/.test(said)) notify('warn', 'Part of the download was skipped', said);
    else if (/^(Already running|Nothing to fetch)/.test(said)) notify('info', said);
  }

  /* What "Get Amazon data" fetches: the forecast. The transaction history
     is optional and imported by hand if wanted, so it is never asked for,
     skipped, or nagged about from here. */
  const FETCHED = ['fees-preview'];

  async function getTodaysDataInner(choice) {
    ...
    const ready = (state.workerInfo.reports || []).filter(r => r.ready && FETCHED.indexOf(r.id) >= 0);
    if (!ready.length) {
      state.refreshPhase = 'failed';
      state.refreshNote = (state.workerInfo.reports || []).length
        ? 'No report is set up yet — it is a one-time step per report. '
          + 'Open "Forecast report settings" below to do it.'
    ...
    try {
      const r = await state.worker.refreshToday({
        ...
        /* The historical report follows the reporting period you are looking
           at; the forecast keeps its own forward window - unless the download
           window chose dates for either, which then win. */
        from: state.filters.from || null,
        to: state.filters.to || null,
        forecast: (choice && choice.forecast) || null,
        history: (choice && choice.history) || null,
        reports: FETCHED,
      });
      const n = (r.created || []).length, running = (r.alreadyRunning || []).length;
      /* Only "downloading" when something is. With nothing new asked for, the
         button stayed disabled on "Downloading from Amazon" for good. */
      state.refreshPhase = n || running ? 'downloading' : 'done';
    ...
    if (t.jobId) {
      ...
      if (job) {
        html += '<div class="note ' + (job.status === 'complete' ? 'is-ok'
          ...
          + esc(job.status === 'complete' ? (job.statusDetail || '')
            : (job.lastError || job.statusDetail || ''))
  ...
  /* The forecast's settings. The transaction report's account type and
     country list are gone from here: the app downloads the forecast only. */
  function downloadScopeCard(w) {
    ...
    /* The dates are chosen in the Get Amazon data window every time, so
       there is nothing to set for them here. */
    html += '<div class="meta" style="margin-top:4px">The marketplace above is the default. '
      + 'Dates, and the country for one download, are chosen in the window that '
      + 'opens when you press Get today’s Amazon data.</div>';
  ...
  function startButton(w) {
    ...
    const unready = (w.reports || []).filter(r => !r.ready && FETCHED.indexOf(r.id) >= 0);
    ...
    if (busy) {
      ...
      note = 'Amazon\u2019s forecast for your sales and fees. You choose the dates and '
        + 'country before anything is downloaded.';
    }
  ...
  async function startAll() {
    ...
    return openGetWindow();
  }

  /* ── notifications ──────────────────────────────────────────────────────
     Asked for by the owner: one place at the top for what happened - a
     download asked for, downloaded, imported, completed, failed - so the
     screens themselves stop carrying one-off messages. Kept in this browser
     (the last 60), because they are a record of this app's activity here,
     not data. Mistakes in a form still show beside the form: that is where
     they have to be fixed. */
  /* A function declaration, and the key written inline: the state object
     reads this while it is being built, before any const here exists. */
  function loadNotices() {
    try {
      const raw = JSON.parse(localStorage.getItem('fba-notices-v1') || '[]');
      return Array.isArray(raw) ? raw.slice(0, 60) : [];
    } catch (e) { return []; }
  }
  function saveNotices() {
    try { localStorage.setItem('fba-notices-v1', JSON.stringify(state.notices.slice(0, 60))); } catch (e) { /* private window */ }
  }

  const REPORT_NAMES = {
    'fees-preview': 'Fees & Economics Preview',
    'date-range-transactions': 'Payments transactions',
  };
  const reportName = id => REPORT_NAMES[id] || id || 'A report';
  const jobLabel = j => reportName(j.reportType)
    + (j.requestedFrom && j.requestedTo ? ' (' + fmtShortDay(j.requestedFrom) + ' \u2013 '
      + fmtShortDay(j.requestedTo) + (j.marketplace ? ', ' + j.marketplace : '') + ')' : '');

  /* kind: ok | warn | error | info. Toasts for the ones worth interrupting
     for: something finished, or something went wrong. */
  function notify(kind, title, detail, extra) {
    const n = Object.assign({
      id: 'n' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
      at: new Date().toISOString(), kind, title: String(title || ''),
      detail: detail ? String(detail) : '', read: false,
    }, extra || {});
    /* The same thing said twice in a minute is said once. */
    const last = state.notices[0];
    if (last && last.title === n.title && last.detail === n.detail
        && Date.parse(n.at) - Date.parse(last.at) < 60000) return last;
    state.notices.unshift(n);
    state.notices = state.notices.slice(0, 60);
    saveNotices();
    if (kind !== 'info' || (extra && extra.toast)) showToast(n);
    paintNotices();
    return n;
  }
  ...
  function noticeBell() {
    ...
    const bad = state.notices.some(n => !n.read && n.kind === 'error');
    return '<button class="bellbtn" id="noticebtn" aria-expanded="' + state.noticeOpen + '" '
      + 'aria-label="Notifications' + (unread ? ', ' + unread + ' new' : '') + '" title="Notifications">'
      + ico('bell') + (unread ? '<span class="bellbadge' + (bad ? '' : ' is-quiet') + '">'
  ...
  function ago(iso) {
    ...
    return (d.toDateString() === new Date().toDateString() ? 'Today' : d.toLocaleDateString()) + ', ' + time;
  }

  const KIND_ICON = { ok: 'verified', warn: 'missing', error: 'missing', info: 'info' };

  function noticeItem(n, fresh) {
    let html = '<div class="notice-item k-' + esc(n.kind) + (fresh ? ' is-new' : '') + '">'
      + ico(KIND_ICON[n.kind] || 'info') + '<div>'
      + '<div class="notice-title">' + esc(n.title) + '</div>'
      + (n.detail ? '<div class="notice-detail">' + esc(n.detail) + '</div>' : '');
    if ((n.rows && n.rows.length) || (n.list && n.list.length)) {
      html += '<details><summary>Details</summary>'
        + (n.rows && n.rows.length ? '<dl>' + n.rows.map(r => '<dt>' + esc(r[0]) + '</dt><dd>'
          + esc(r[1]) + '</dd>').join('') + '</dl>' : '')
        + (n.list && n.list.length ? '<ul style="padding-left:18px;margin:6px 0 0">'
          + n.list.map(x => '<li>' + esc(x) + '</li>').join('')
          + (n.more ? '<li>and ' + n.more + ' more</li>' : '') + '</ul>' : '')
        + '</details>';
    }
    return html + '<div class="notice-when">' + esc(ago(n.at)) + '</div></div></div>';
  }

  function noticePanel() {
    if (!state.noticeOpen) return '';
    const list = state.notices;
    return '<div class="notice-scrim" data-noticeclose="1"></div>'
      + '<div class="notice-panel" role="dialog" aria-label="Notifications">'
      + '<div class="notice-head"><h2>Notifications</h2>'
      + (list.length ? '<button class="btn sec sm" id="noticeclear">Clear all</button>' : '')
      + '<button class="btn sec sm" data-noticeclose="1" aria-label="Close">' + ico('close', { size: 'sm' }) + '</button></div>'
      + '<div class="notice-list">'
      + (list.length ? list.map(n => noticeItem(n, !n.seenOpen)).join('')
        : '<div class="notice-empty">Nothing yet. Downloads, imports and anything that goes wrong '
  ...
  function paintNotices() {
    ...
    if (slot) slot.innerHTML = noticePanel();
    const btn = document.getElementById('noticebtn');
    if (btn) btn.outerHTML = noticeBell();
  }

  function openNotices(open) {
    state.noticeOpen = open;
    if (open) {
      /* Read once opened; "new" stays highlighted until the panel closes. */
      for (const n of state.notices) n.read = true;
    } else {
      for (const n of state.notices) n.seenOpen = true;
    }
    saveNotices();
    paintNotices();
  }
  ...
  function showToast(n) {
    ...
    if (!slot) return;
    slot.innerHTML = '<div class="toast k-' + esc(n.kind) + '" data-toastopen="1" role="status">'
      + ico(KIND_ICON[n.kind] || 'info')
      + '<div><div class="notice-title">' + esc(n.title) + '</div>'
      + (n.detail ? '<div class="notice-detail">' + esc(n.detail.length > 160 ? n.detail.slice(0, 157) + '\u2026' : n.detail)
        + '</div>' : '') + '</div>'
      + '<button class="x" data-toastx="1" aria-label="Dismiss">' + ico('close', { size: 'sm' }) + '</button></div>';
    clearTimeout(toastTimer);
    toastTimer = setTimeout(() => { slot.innerHTML = ''; }, n.kind === 'error' ? 12000 : 6000);
  }

  function ensureNoticeSlots() {
    if (!document.getElementById('noticeslot')) {
      const a = document.createElement('div'); a.id = 'noticeslot'; document.body.appendChild(a);
    }
    if (!document.getElementById('toastslot')) {
      const b = document.createElement('div'); b.id = 'toastslot';
      b.setAttribute('aria-live', 'polite'); document.body.appendChild(b);
    }
  }

  /* An import, in words - with the receipt folded inside. */
  function noticeForImport(li) {
    if (!li) return;
    const r = li.receipt;
    if (li.duplicate) return notify('info', 'Already imported: ' + (li.name || 'this file'), li.message);
    if (li.error || li.ok === false) {
      return notify('error', (li.name || 'A file') + ' was not imported',
        (li.error || li.message || '') + (li.detail ? ' ' + li.detail : ''));
    }
    if (!r) return notify('ok', li.message || ('Imported ' + (li.name || 'a file')));
    const rej = r.rejected || [];
    const rows = [['File', r.file], ['Report', r.family]];
    if (r.period) rows.push(['Dates', r.period]);
    if (r.marketplace) rows.push(['Marketplace', r.marketplace]);
    if (r.currency) rows.push(['Currency', r.currency]);
    if (r.rowsProcessed != null) rows.push(['Rows read', r.rowsProcessed.toLocaleString()]);
    if (r.rowsAccepted != null) rows.push(['Rows imported', r.rowsAccepted.toLocaleString()]);
    rows.push(['Rows rejected', String(rej.length)]);
    if (r.stored) rows.push(['Saved', r.stored.ok ? 'Yes, read back' : 'In memory only: ' + (r.stored.reason || '')]);
    return notify(rej.length || (r.stored && !r.stored.ok) ? 'warn' : 'ok',
      'Imported ' + r.family + (r.period ? ' \u00b7 ' + r.period : ''),
      (r.rowsAccepted == null ? '' : r.rowsAccepted.toLocaleString() + ' rows')
        + (rej.length ? ', ' + rej.length + ' rejected' : '') + '.',
      { rows, list: rej.slice(0, 10).map(x => 'Line ' + x.line + ': ' + x.reason + (x.detail ? ' - ' + x.detail : ''))
        .concat(r.issues || []), more: Math.max(0, rej.length - 10) });
  }

  /* Download jobs: say each change of state once. The first look only
     learns what is there - reopening the app is not news. */
  function noticeJobChanges() {
    const seen = new Map(state.jobs.map(j => [j.jobId, j.status]));
    const prev = state.jobSeen;
    state.jobSeen = seen;
    if (!prev) return;
    for (const j of state.jobs) {
      const was = prev.get(j.jobId);
      if (was === j.status) continue;
      const name = jobLabel(j);
      if (j.status === 'importing' && was !== 'complete') {
        notify('info', 'Downloaded: ' + name, 'Importing it now.');
      } else if (j.status === 'complete') {
        notify('ok', 'Complete: ' + name, j.statusDetail || '');
      } else if (j.status === 'failed') {
        notify('error', 'Failed: ' + name, j.lastError || j.statusDetail || 'No reason was given.');
      } else if (j.status === 'login-required') {
        notify('warn', 'Amazon wants you to sign in', 'For ' + name + '. Sign in in the Amazon window; '
          + 'the download carries on by itself, or press Retry on the report.');
      } else if (was === undefined && (j.status === 'queued' || j.status === 'requesting')) {
        notify('info', 'Asked Amazon for ' + name);
      }
    }
  }

  /* ── the "Get Amazon data" window ──────────────────────────────────────
     Asked for by the owner: before anything is requested, say WHAT - which
     dates, which country - and let it be changed. The forecast and the
     transaction history have different natural windows (ahead and behind),
     so each has its own dates. Nothing here is sent until Download. */
  function openGetWindow(pre) {
    const w = state.workerInfo || {};
    const s = w.settings || {};
    state.getWin = {
      fFrom: (pre && pre.from) || horizonStart(), fTo: (pre && pre.to) || horizonEnd(),
      skuMkt: state.skuMarketplace || s.skuMarketplace || 'US',
      hFrom: CSV.addDays(state.today, -44), hTo: state.today,
      mkts: selectedMarketplaces().slice(),
      acct: state.accountType || s.accountType || 'All (Unified Reports)',
      error: null,
    };
    render();
    setTimeout(() => { const f = $('#gw-go'); if (f) f.focus(); }, 40);
  }

  const GW_SKU_MARKETS = [
    { code: 'US', name: 'United States' }, { code: 'CA', name: 'Canada' },
    { code: 'MX', name: 'Mexico' }, { code: 'BR', name: 'Brazil' },
  ];

  function getWindow() {
    const g = state.getWin;
    if (!g) return '';
    const w = state.workerInfo || {};
    const reports = w.reports || [];
    const fcRep = reports.find(r => r.id === 'fees-preview');
    const fcReady = !!(fcRep && fcRep.ready);
    const days = (a, b) => (a && b && a <= b) ? CSV.daysBetween(a, b) + 1 : 0;
    const quick = (attr, v, label) => '<button type="button" class="chip" ' + attr + '="' + v + '">'
      + esc(label) + '</button>';

    let html = '<div class="modal-scrim" data-gwclose="1"></div>'
      + '<div class="modal" role="dialog" aria-modal="true" aria-labelledby="gwh" data-keeptyped>'
      + '<div class="modal-h"><h2 id="gwh" style="margin:0">Get Amazon data</h2>'
      + '<button class="btn sec sm" data-gwclose="1" style="margin-left:auto">Cancel</button></div>'
      + '<p class="meta" style="margin:0 0 14px">Amazon\u2019s forecast for your sales and fees. '
      + 'Check the dates and country, then Download. '
      + 'A browser window opens and closes by itself; leave it alone while it works.</p>';

    /* the forecast */
    html += '<section style="border:1px solid var(--line);border-radius:10px;padding:14px 16px;margin-bottom:12px">'
      + '<div class="card-h" style="margin:0 0 6px"><h3 style="font-size:var(--t-h3)">Forecast \u00b7 Fees &amp; Economics Preview</h3>'
      + (fcReady ? '' : '<span class="tag is-warn">Not ready</span>') + '</div>'
      + '<div class="frow f3">'
      + '<div class="field"><label class="lbl" for="gw-skumkt">Country</label><select id="gw-skumkt">'
      + GW_SKU_MARKETS.map(m => '<option value="' + m.code + '"' + (m.code === g.skuMkt ? ' selected' : '') + '>'
        + esc(m.name) + '</option>').join('')
      + (GW_SKU_MARKETS.some(m => m.code === g.skuMkt) ? '' : '<option selected value="' + esc(g.skuMkt) + '">' + esc(g.skuMkt) + '</option>')
      + '</select></div>'
      + '<div class="field"><label class="lbl" for="gw-ffrom">From</label>'
      + '<input type="date" id="gw-ffrom" value="' + esc(g.fFrom) + '" min="' + esc(horizonStart()) + '"></div>'
      + '<div class="field"><label class="lbl" for="gw-fto">To</label>'
      + '<input type="date" id="gw-fto" value="' + esc(g.fTo) + '" min="' + esc(horizonStart()) + '"></div></div>'
      + '<div class="btnrow" style="gap:6px;margin-top:4px">'
      + quick('data-gwf', '56', 'Next 8 weeks') + quick('data-gwf', '30', 'Next 30 days')
      + quick('data-gwf', '14', 'Next 14 days') + '</div>'
      + '<div class="meta" style="margin-top:8px">' + days(g.fFrom, g.fTo) + ' days of Amazon\u2019s own '
      + 'estimate, starting tomorrow at the earliest - a forecast of today would count a day that is '
      + 'already half over.</div></section>';

    /* The forecast is the point of the app; the transaction history is not
       asked for from here. (Import a transactions file by hand if wanted.) */
    if (g.error) {
      html += '<div class="note is-warn" role="alert" style="margin-top:12px">' + ico('missing')
        + '<div>' + esc(g.error) + '</div></div>';
    }
    html += '<div class="btnrow" style="margin-top:16px;justify-content:flex-end">'
      + '<button class="btn sec" data-gwclose="1">Cancel</button>'
      + '<button class="btn" id="gw-go"' + (fcReady ? '' : ' disabled') + '>'
      + ico('download', { size: 'sm' }) + 'Download</button></div></div>';
    return html;
  }

  /* Reads the window's fields into state, so a redraw shows what was typed. */
  function readGetWindow() {
    const g = state.getWin;
    if (!g) return;
    const v = id => { const el = document.getElementById(id); return el ? el.value : null; };
    g.fFrom = v('gw-ffrom') || g.fFrom; g.fTo = v('gw-fto') || g.fTo;
    g.skuMkt = v('gw-skumkt') || g.skuMkt; g.acct = v('gw-acct') || g.acct;
    g.hFrom = v('gw-hfrom') || g.hFrom; g.hTo = v('gw-hto') || g.hTo;
  }

  async function submitGetWindow() {
    readGetWindow();
    const g = state.getWin;
    const reports = (state.workerInfo || {}).reports || [];
    const fcReady = reports.some(r => r.id === 'fees-preview' && r.ready);
    const tomorrow = horizonStart();
    const problem = !fcReady ? 'The forecast report is not available from the helper yet.'
      : !g.fFrom || !g.fTo ? 'Choose the forecast dates.'
        : g.fFrom > g.fTo ? 'The forecast ends before it starts.'
          : g.fFrom < tomorrow ? 'The forecast starts tomorrow at the earliest (' + fmtDay(tomorrow) + ').'
            : null;
    if (problem) { g.error = problem; render(); return; }

    /* Remembered for next time, on the helper, like the panel's settings. */
    state.skuMarketplace = g.skuMkt;
    const choice = { forecast: { from: g.fFrom, to: g.fTo }, history: null };
    state.getWin = null;
    render();
    await saveDownloadSettings();
    return getTodaysData(choice);
  }
  ...
  function refreshButton() {
    ...
    const label = busy ? REFRESH_WORDS[phase]
      : (!known || unready.length) ? 'Start setup'
  ...
  const importingNow = new Set();
  const clearingJobs = new Set();
  async function refreshJobs() {
    ...
    const haveHere = id => state.imports.some(r => r.jobId === id
      || (Array.isArray(r.jobIds) && r.jobIds.indexOf(id) >= 0));
    const letGo = new Set([].concat(...(state.removedImports || []).map(r => r.jobIds || [])));
    /* A scheduled download let go on ANY computer - replaced, or a duplicate
       another computer also fetched - is cleared from this helper too: its
       file, its archive and its rows in Supabase. */
    for (const j of state.jobs) {
      if (letGo.has(j.jobId) && j.scheduled && Worker2.TERMINAL.indexOf(j.status) >= 0 && !clearingJobs.has(j.jobId)) {
        clearingJobs.add(j.jobId);
        state.worker.remove(j.jobId).then(r => { if (r && r.archived) refreshMirrorSoon(); }).catch(() => { /* next time */ });
      }
    }
    for (const j of state.jobs) {
      const waiting = j.status === 'importing';
      const doneElsewhere = j.status === 'complete' && j.filePath && !haveHere(j.jobId) && !letGo.has(j.jobId);
      if ((!waiting && !doneElsewhere) || importingNow.has(j.jobId)) continue;
      importingNow.add(j.jobId);
      try {
        ...
        const rec = savedRec;
        const sameFile = !!(imported && imported.duplicate);
        if (rec.family === 'Payments transactions' && !sameFile && !await persistLedger()) throw new Error('Transaction storage could not be verified.');
        if (rec) {
          rec.fetchedAt = new Date().toISOString();
          /* Every download that delivered this file, not just the latest.
             Moving the one id to each new download left the previous job
             looking un-imported, so the next refresh imported it again -
             back and forth, for ever. */
          rec.jobIds = [...new Set([].concat(rec.jobIds || [], rec.jobId ? [rec.jobId] : [], [j.jobId]))];
          rec.jobId = rec.jobId || j.jobId;
        ...
        if (j.status === 'importing') {
          /* The same file as one already in the app: nothing new was added,
             and it says so - with the rows that ARE there - rather than
             "Imported None of None rows", which read as the data being lost. */
          await state.worker.markImported(j.jobId, sameFile
            ? { sameAs: rec.name || 'an earlier download', rowsAccepted: rec.rowCount == null ? null : rec.rowCount }
        ...
        notify('error', 'Downloaded but not imported: ' + jobLabel(j),
          e.message + ' The file is kept; Retry imports it without asking Amazon again.');
  ...
  /* ── the morning download ─────────────────────────────────────────────
     ...
     Every morning at the set time, Amazon's forecast one window at a time:
     the next 30 days each on its own, then week by week to about four months
     (lib/schedule.js). One report at a time with a pause between, and a stop -
     handed to the person - the moment Amazon asks for a sign-in, a CAPTCHA or
     any other check. It is the same browser and the same steps as pressing
     the button yourself; nothing here disguises what it is.

     One computer runs it (the owner), so Amazon is never asked twice; the
     others take the figures from Supabase. */
  ...
  function myDeviceId() {
    ...
    if (!id) {
      ...
      try { localStorage.setItem('fba-device-id', id); } catch (e) { /* this session only */ }
    }
    return id;
  }
  /* Shorter pauses and a smaller plan, for testing in a plain browser against
     the stand-in helper. Never in the installed app (it has a shell). */
  function scheduleTestOverrides() {
    try {
      if (window.desktopShell || !/^(localhost|127\.0\.0\.1)$/.test(location.hostname)) return {};
      return JSON.parse(localStorage.getItem('fba-schedule-test') || '{}');
    } catch (e) { return {}; }
  }
  const scheduleCfg = () => Schedule.settings(Object.assign({}, state.schedule, scheduleTestOverrides()));
  const isScheduleOwner = () => !(state.schedule && state.schedule.owner)
    || state.schedule.owner === myDeviceId();
  /* The computer's own name (the desktop app knows it). This is shown on the
     OTHER computers - "runs on DESKTOP-4F2K" - so without one it must not say
     "this computer", which there would mean the wrong one. */
  const hostName = () => (state.shell && state.shell.hostname) || 'another computer';
  const pad2 = n => String(n).padStart(2, '0');
  function localNowParts() {
    const d = new Date();
    return { date: localToday(), time: pad2(d.getHours()) + ':' + pad2(d.getMinutes()) };
  }
  ...
  function setSchedule(patch) {
    state.schedule = Object.assign({}, state.schedule || {}, patch);
    persist();
  }
  function claimSchedule() {
    setSchedule({ owner: myDeviceId(), ownerName: hostName(), ownerSince: localToday() });
  }

  /* This computer's run, kept here only, so a restart mid-run picks up where
     it stopped (the windows already fetched today are not asked for again). */
  function loadRun() {
    try {
      const r = JSON.parse(localStorage.getItem('fba-schedule-run') || 'null');
      if (r && /^(running|waiting|stopping)$/.test(r.status)) {
        r.status = 'interrupted';
        r.note = 'The app closed while this was running. It carries on at the next check.';
      }
      return r;
    } catch (e) { return null; }
  }
  function saveRun() {
    try { localStorage.setItem('fba-schedule-run', JSON.stringify(state.scheduleRun)); } catch (e) { /* this session */ }
  }

  /* When each forecast file was downloaded, so that for any day the newest
     download is the one used (Preview.ownership). */
  function previewWhen() {
    const at = new Map(state.imports.map(i => [i.id, i.fetchedAt || i.importedAt || null]));
    return p => at.get(p.importId) || null;
  }
  const forecastOf = (previews, from, to) => Simple.forecastFor(previews, from, to, { whenOf: previewWhen() });

  function scheduleDoneToday(today) {
    const done = new Set();
    for (const i of state.imports) {
      if (i.schedule && i.scheduleWindow && localDayOf(i.fetchedAt) === today) done.add(i.scheduleWindow);
    }
    return done;
  }
  function coveredSet(from, to) {
    const set = new Set();
    for (const p of state.previews) {
      if (!p || !p.period || !p.period.start || p.period.end < from || p.period.start > to) continue;
      for (let d = p.period.start > from ? p.period.start : from; d <= p.period.end && d <= to; d = CSV.addDays(d, 1)) set.add(d);
    }
    return set;
  }
  const haveJob = id => state.imports.some(r => r.jobId === id || (Array.isArray(r.jobIds) && r.jobIds.indexOf(id) >= 0));
  const shellBusy = on => { try { if (window.desktopShell && window.desktopShell.busy) window.desktopShell.busy(on); } catch (e) { /* no shell */ } };

  /* A Windows notification, for when the app is in the tray at 6 AM and the
     person is not looking at it. Clicking it brings the app forward. */
  function desktopNotice(title, body) {
    try {
      if (typeof Notification === 'undefined') return;
      const show = () => {
        const n = new Notification(title, { body });
        n.onclick = () => { if (window.desktopShell && window.desktopShell.show) window.desktopShell.show(); };
      };
  ...
  function needsYou(detail) {
    notify('warn', 'Amazon needs you', detail, { toast: true });
    desktopNotice('Amazon needs you', detail);
  }

  /* A pause the Stop button can cut short (unless keep: a report already
     asked for is always seen through). */
  const pause = (ms, keep) => new Promise(res => {
    const t0 = Date.now();
    const tick = () => {
      const r = state.scheduleRun;
      if ((!keep && r && r.status === 'stopping') || Date.now() - t0 >= ms) res();
      else setTimeout(tick, 500);
    };
    tick();
  });
  let continueRun = null;
  const waitForContinue = () => new Promise(res => { continueRun = res; });

  /* ── one computer at a time ────────────────────────────────────────

     Every computer with the app has its own helper, and each could run the
     morning download - asking Amazon for the same reports twice and saving
     them twice. "Who is the owner" (Settings) says which one should; this
     makes sure only one DOES, even when two start at the same moment, or the
     app is open in two places on one computer.

     A note in Supabase says who is running today's download. A computer
     writes its name, waits, and reads it back: the one whose name is still
     there goes ahead, the other stands down. While running, it checks the
     note before every report and stops if another computer has taken over.
     A note not touched for ten minutes belongs to a run that has ended. */
  const LEASE_FRESH_MS = 10 * 60 * 1000;
  const leaseFresh = l => !!(l && l.beatAt && Date.now() - Date.parse(l.beatAt) < LEASE_FRESH_MS);
  const canShare = () => !!(state.sync && state.sync.available && state.sync.readLease);

  async function takeLease(today, byHand) {
    if (!canShare()) return { ok: true };             // nothing shared, so nothing to clash with
    const me = myDeviceId();
    let cur = null;
    try { cur = await state.sync.readLease(); } catch (e) { return { ok: true }; }
    const other = cur && cur.device !== me && cur.date === today;
    if (other && cur.status === 'running' && leaseFresh(cur)) {
      return { ok: false, why: 'Today\u2019s download is running on ' + (cur.name || 'another computer')
        + '. This one stood down, so nothing is asked for twice.' };
    }
    if (other && cur.status !== 'running' && !byHand) {
      return { ok: false, why: 'Today\u2019s download was already done on ' + (cur.name || 'another computer') + '.' };
    }
    const at = new Date().toISOString();
    try {
      await state.sync.writeLease({ device: me, name: hostName(), date: today, status: 'running', startedAt: at, beatAt: at });
      await pause(2500 + Math.floor(Math.random() * 1500), true);   // let a computer that wrote at the same moment land
      const back = await state.sync.readLease();
      if (!back || back.device !== me) {
        return { ok: false, why: (back && back.name ? back.name : 'Another computer')
          + ' started today\u2019s download at the same moment. This one stood down.' };
      }
    } catch (e) { return { ok: true }; }
    return { ok: true };
  }

  /* Before each report: still ours? Touch the note so others know it is live. */
  async function keepLease(today) {
    if (!canShare()) return true;
    const me = myDeviceId();
    try {
      const cur = await state.sync.readLease();
      if (cur && cur.device !== me && cur.date === today && cur.status === 'running' && leaseFresh(cur)) return false;
      await state.sync.writeLease(Object.assign({}, cur && cur.device === me ? cur : {},
        { device: me, name: hostName(), date: today, status: 'running', beatAt: new Date().toISOString() }));
    } catch (e) { /* a missed check is not a reason to stop */ }
    return true;
  }

  async function releaseLease(today, status) {
    if (!canShare()) return;
    try {
      const cur = await state.sync.readLease();
      if (cur && cur.device !== myDeviceId()) return;   // someone else's now; leave it
      const at = new Date().toISOString();
      await state.sync.writeLease(Object.assign({}, cur || {}, { device: myDeviceId(), name: hostName(), date: today,
        status, finishedAt: at, beatAt: at }));
    } catch (e) { /* it goes stale by itself in ten minutes */ }
  }

  let scheduleBusy = false;
  async function runSchedule(trigger) {
    if (scheduleBusy || !state.worker) return;
    state.workerInfo = await state.worker.probe().catch(() => null) || state.workerInfo;
    if (!state.workerInfo) return;
    const today = localToday();
    const ready = (state.workerInfo.reports || []).some(r => r.ready && FETCHED.indexOf(r.id) >= 0);
    if (!ready) {
      state.scheduleRun = { date: today, trigger, status: 'stopped', total: 0, done: 0, failed: 0,
        note: 'The forecast report is not set up yet — a one-time step, under Downloads → Forecast report settings.' };
      saveRun(); render();
      return;
    }
    scheduleBusy = true;
    const byHand = trigger === 'button' || trigger === 'tray';
    const lease = await takeLease(today, byHand);
    if (!lease.ok) {
      state.scheduleRun = { date: today, trigger, status: 'elsewhere', total: 0, done: 0, failed: 0, note: lease.why,
        finishedAt: new Date().toISOString() };
      /* Not asked again every minute: once today is someone else's, it is. */
      leaseQuietUntil = Date.now() + 15 * 60 * 1000;
      saveRun();
      scheduleBusy = false;
      render();
      return;
    }
    if (!(state.schedule && state.schedule.owner)) claimSchedule();
    const cfg = scheduleCfg();
    const cov = coveredSet(CSV.addDays(today, cfg.near + 1), CSV.addDays(today, cfg.far + 7));
    const windows = Schedule.due(today, cfg, { doneToday: scheduleDoneToday(today), lastWeekly: cfg.lastWeekly,
      covered: d => cov.has(d) });
    const run = state.scheduleRun = { date: today, trigger, total: windows.length, done: 0, failed: 0,
      status: 'running', startedAt: new Date().toISOString(), finishedAt: null, current: null, note: null,
      failures: [], weekly: windows.some(w => w.kind === 'week') };
    if (!windows.length) run.note = 'Everything due today was already downloaded.';
    saveRun();
    shellBusy(true);
    render();
    let inARow = 0;
    /* A failed report counts; three in a row end the morning. */
    const failed = (w, r) => {
      run.failed++;
      run.failures.push({ from: w.from, to: w.to, why: r.why });
      if (r.stop) { run.note = r.why; run.status = 'stopping'; return; }
      if (++inARow >= 3) {
        run.note = 'Stopped after three reports in a row did not come through. The last said: ' + r.why;
        run.status = 'stopping';
      }
    };
    try {
      /* In batches: ask Amazon for up to ten reports back to back, then
         collect them - by the time the last is asked for, the first are
         built. Nobody waits while Amazon builds each one in turn. */
      for (let b = 0; b < windows.length && run.status !== 'stopping'; b += cfg.batchSize) {
        const asked = [];
        for (const w of windows.slice(b, b + cfg.batchSize)) {
          if (run.status === 'stopping') break;
          if (canShare()) {
            if (!(await keepLease(today))) {
              run.note = 'Another computer took over today\u2019s download, so this one stopped \u2014 nothing is '
                + 'asked for twice.';
              run.status = 'stopping';
              break;
            }
            /* A date another computer already fetched this morning is not fetched again. */
            await syncPull();
            if (scheduleDoneToday(today).has(Schedule.key(w))) {
          ...
          saveRun();
          paintSchedule();
          const r = await askWindow(w);
          if (r.ok) { asked.push({ w, jobId: r.jobId }); inARow = 0; } else failed(w, r);
          saveRun();
          paintSchedule();
          if (run.status !== 'stopping') await pause(cfg.askGapSeconds * 1000);
        }
        for (const x of asked) {
          ...
          saveRun();
          paintSchedule();
          const r = await collectWindow(x.jobId, x.w);
          if (r.ok) { run.done++; inARow = 0; } else failed(x.w, r);
          saveRun();
          paintSchedule();
          await pause(cfg.gapSeconds * 1000);
        }
      }
    } finally {
      const stopped = run.status === 'stopping';
      run.status = stopped ? 'stopped' : run.failed ? 'partial' : 'done';
      await releaseLease(today, run.status);
      run.finishedAt = new Date().toISOString();
      run.current = null;
      saveRun();
      /* Marked as run even when it stopped, so it does not start again every
         minute; "Run now" is there for another go. */
      const patch = { lastRunDate: today, lastRun: { date: today, startedAt: run.startedAt, finishedAt: run.finishedAt,
        done: run.done, total: run.total, failed: run.failed, status: run.status, by: hostName() } };
      if (run.weekly && !run.failures.some(f => CSV.daysBetween(f.from, f.to) > 0)) patch.lastWeekly = today;
      setSchedule(patch);
      let let_go = 0;
      try { let_go = await pruneScheduled(); } catch (e) { /* tried again next time */ }
      shellBusy(false);
      scheduleBusy = false;
      const said = run.done + ' of ' + run.total + ' reports downloaded'
        + (run.skipped ? ' (' + run.skipped + ' already fetched by another computer)' : '')
        + (run.failed ? ', ' + run.failed + ' did not come through' : '')
        + (let_go ? '; ' + let_go + ' replaced ' + (let_go === 1 ? 'version' : 'versions') + ' let go' : '') + '.';
      if (run.status === 'done') notify('ok', 'This morning’s forecast is in', said);
      else {
        notify('warn', 'The morning download stopped', (run.note ? run.note + ' ' : '') + said, { toast: true });
        desktopNotice('The morning download stopped', run.note || said);
      }
      render();
    }
  }

  /* Ask Amazon for one window's report, and stop there: done once Amazon has
     the request (or the report is already further on - asked for earlier
     this morning). */
  async function askWindow(w) {
    let res;
    try {
      res = await state.worker.refreshToday({ forecast: { from: w.from, to: w.to }, reports: FETCHED,
        fresh: true, scheduled: w.kind, requestOnly: true });
    } catch (e) { return { ok: false, why: e.message }; }
    const job = (res.created || [])[0] || (res.alreadyRunning || [])[0];
    if (!job) {
      const nc = (res.notConfigured || [])[0];
      return { ok: false, stop: !!nc, why: nc ? (nc.reason || 'The forecast report is not set up.')
        : 'The helper did not start a report.' };
    }
    const r = await waitForJob(job.jobId, w, 'asked');
    return r.ok ? { ok: true, jobId: job.jobId } : r;
  }

  /* Fetch a report asked for earlier; done once it is in the app. */
  async function collectWindow(jobId, w) {
    const j = state.jobs.find(x => x.jobId === jobId);
    if (j && j.status === 'requested') {
      try { await state.worker.collect(jobId); } catch (e) { return { ok: false, why: e.message }; }
    }
    return waitForJob(jobId, w, 'collected');
  }

  /* goal 'asked': Amazon has the request. 'collected': the file is in the app. */
  async function waitForJob(jobId, w, goal) {
    const run = state.scheduleRun;
    const deadline = Date.now() + 40 * 60 * 1000;
    let told = false, notHere = 0;
    while (Date.now() < deadline) {
      await pause(5000, true);
      await refreshJobs();
      const j = state.jobs.find(x => x.jobId === jobId);
      if (!j) return { ok: false, why: 'The report disappeared from the helper.' };
      if (j.status === 'login-required') {
        if (!told) {
          told = true;
          run.status = 'waiting';
          run.note = 'Amazon is asking for a sign-in or a check (a CAPTCHA, a code). Complete it in the Amazon '
            + 'window; the download carries on by itself.';
          saveRun();
          needsYou(run.note);
          render();
        }
      ...
      if (run.status === 'waiting') { run.status = 'running'; run.note = null; render(); }
      if (goal === 'asked' && /^(requested|importing|complete|validating|downloading)$/.test(j.status)) {
        return { ok: true };
      }
      if (j.status === 'complete') {
        if (haveJob(jobId)) { markScheduled(jobId, w); return { ok: true }; }
        if (++notHere >= 4) return { ok: false, why: 'Downloaded, but it could not be added to the app.' };
        continue;
      }
      if (j.status === 'failed' || j.status === 'cancelled') {
        const why = j.lastError || 'The report did not come through.';
        if (/sign.?in/i.test(why)) {
          /* Amazon waited for a sign-in and nobody came. Hold the run until
             the person is back, rather than trying the rest and failing each. */
          run.status = 'waiting';
          run.note = 'Amazon asked for a sign-in or a check and nobody was there. Sign in in the Amazon window '
            + 'that opens, then press Continue.';
          saveRun();
          needsYou(run.note);
          render();
          const go = await waitForContinue();
          if (!go) return { ok: false, stop: true, why: 'Stopped while waiting for a sign-in.' };
          run.status = 'running';
          run.note = null;
          render();
          try { await state.worker.retry(jobId); } catch (e) { return { ok: false, why: e.message }; }
          told = false;
          continue;
        }
        return { ok: false, why };
    ...
    return { ok: false, why: 'Amazon had not finished the report after 40 minutes.' };
  }

  function markScheduled(jobId, w) {
    const rec = state.imports.find(i => i.jobId === jobId || (Array.isArray(i.jobIds) && i.jobIds.indexOf(jobId) >= 0));
    if (!rec) return;
    rec.schedule = w.kind;
    rec.scheduleWindow = Schedule.key(w);
    persist();
  }

  /* Let go of what the schedule itself downloaded and no longer needs: a
     version every day of which a newer download now covers, or dates long
     past. Gone from this computer, from the helper (file, archive, and the
     rows in Supabase), and - through the removal record - from every other
     computer. A file someone imported by hand is never touched. */
  async function pruneScheduled() {
    const today = localToday();
    const own = Preview.ownership(state.previews, { whenOf: previewWhen() });
    const entries = [];
    for (const i of state.imports) {
      if (i.family !== 'Fees & Economics Preview' || !i.schedule) continue;
      const p = state.previews.find(x => x.importId === i.id);
      if (!p || !p.period) continue;           // no file here to judge by
      entries.push({ id: i.id, scheduled: true, to: p.period.end, owns: (own.owned.get(p) || []).length });
    }
    const ids = Schedule.prunable(entries, today, scheduleCfg());
    if (!ids.length) return 0;
    const now = new Date().toISOString();
    const gone = state.imports.filter(i => ids.indexOf(i.id) >= 0);
    const jobsOf = i => [...new Set([].concat(i.jobIds || [], i.jobId ? [i.jobId] : []))];
    state.removedImports = Sync.trimRemovals((state.removedImports || [])
      .concat(gone.map(i => Object.assign(Sync.removal(i, now), { jobIds: jobsOf(i) }))), now);
    state.previews = state.previews.filter(p => ids.indexOf(p.importId) < 0);
    state.imports = state.imports.filter(i => ids.indexOf(i.id) < 0);
    for (const i of gone) {
      for (const jid of jobsOf(i)) {
        if (state.jobs.some(j => j.jobId === jid)) {
          try { await state.worker.remove(jid); } catch (e) { /* the removal record keeps it out */ }
        }
      }
      if (state.sync && state.sync.available) {
        try { await state.sync.deleteBlob(blobKeyFor(i)); } catch (e) { /* best effort */ }
      }
    }
    refreshMirrorSoon();
    bumpForecast();
  ...
  let leaseQuietUntil = 0;
  /* The set time plus two hours, as HH:MM - when a computer that is not the
     one set to run it steps in, because that one has not. */
  const standInTime = t => {
    const [h, m] = String(t || '06:00').split(':').map(Number);
    return pad2(Math.min(23, h + 2)) + ':' + pad2(m || 0);
  };
  function scheduleTick() {
    if (scheduleBusy || !state.workerInfo || Date.now() < leaseQuietUntil) return;
    const r = state.scheduleRun;
    if (r && r.status === 'elsewhere' && r.date === localToday() && /already done/.test(r.note || '')) return;
    const cfg = scheduleCfg();
    if (!cfg.enabled) return;
    const now = localNowParts();
    if (isScheduleOwner()) {
      if (Schedule.isDue(now, cfg, cfg.lastRunDate)) runSchedule('schedule');
      return;
    }
    /* Not this computer's job - unless the one it belongs to has not done it
       two hours after the time (switched off, asleep, gone for good). Then
       this one stands in; the note in Supabase still stops two running. A
       computer silent for two days is taken over for good. */
    if (!supportsStandIn()) return;
    if (!Schedule.isDue(now, Object.assign({}, cfg, { time: standInTime(cfg.time) }), cfg.lastRunDate)) return;
    const silentSince = cfg.lastRunDate || (state.schedule && state.schedule.ownerSince) || null;
    if (silentSince && CSV.daysBetween(silentSince, now.date) >= 2) claimSchedule();
    runSchedule('stand-in');
  }
  /* Standing in needs the shared note - without Supabase, two computers
     could not see each other, so each keeps to its own. */
  const supportsStandIn = () => canShare();
  let scheduleTimer = null;
  function startSchedule() {
    if (scheduleTimer) return;
    scheduleTimer = setInterval(scheduleTick, 60000);
    setTimeout(scheduleTick, 15000);
    try {
      if (window.desktopShell && window.desktopShell.onScheduleRun) {
        window.desktopShell.onScheduleRun(() => runSchedule('tray'));
      }
    } catch (e) { /* no shell */ }
  }

  /* The morning download in a few words, for the sidebar. */
  function scheduleWords() {
    const cfg = scheduleCfg();
    const run = state.scheduleRun;
    if (!state.workerInfo && isScheduleOwner()) return ['Needs the helper', null];
    if (!cfg.enabled) return ['Off', null];
    if (!isScheduleOwner()) return ['On ' + (state.schedule.ownerName || 'another computer'), 'ok'];
    if (run && run.status === 'elsewhere' && run.date === localToday()) return ['On another computer', 'ok'];
    if (run && run.status === 'running') return ['Downloading ' + run.done + '/' + run.total, null];
    if (run && run.status === 'waiting') return ['Needs you', 'warn'];
    if (run && run.date === localToday() && run.finishedAt) {
      const t = new Date(run.finishedAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });
      return run.status === 'done' ? ['Done ' + t, 'ok'] : ['Stopped ' + t, 'warn'];
    }
    return ['Next at ' + cfg.time, null];
  }
  function paintSchedule() {
    const sn = document.getElementById('sidenote');
    if (sn) sn.innerHTML = forecastStatus();
    const sc = document.getElementById('schedulecard');
    if (sc && state.screen === 'settings') sc.outerHTML = scheduleCard();
  }

  /* Settings: the morning download. */
  function scheduleCard() {
    const cfg = scheduleCfg();
    const run = state.scheduleRun;
    const mine = isScheduleOwner();
    const desktop = !!(window.desktopShell && window.desktopShell.startup);
    const startup = !!(state.shell && state.shell.startup);
    const row = (label, meta, control) => '<label class="setrow"><span><b>' + label + '</b>'
      + (meta ? '<span class="meta">' + meta + '</span>' : '') + '</span>' + control + '</label>';
    let html = '<section class="card" id="schedulecard"><div class="card-h">' + ico('download')
      + '<h2>Every morning</h2></div>'
      + '<div class="sub">Amazon’s forecast, downloaded for you: the next 30 days one day at a time, every '
      + 'morning; then week by week to about four months, refreshed once a week. It asks Amazon for up to '
      + cfg.batchSize + ' reports, ' + cfg.askGapSeconds + ' seconds apart, then collects them as Amazon finishes '
      + 'building them — about an hour for a full morning. If Amazon asks you to sign in or complete a '
      + 'check, it stops, tells you, and carries on once you have. Replaced versions are let go, so storage '
      + 'stays level.</div>'
      + row('Download every morning', 'About 30 reports most mornings, about 45 once a week.',
        '<input type="checkbox" role="switch" class="switch" data-sched="enabled"' + (cfg.enabled ? ' checked' : '')
        + ' aria-label="Download every morning">')
      + row('At', 'If the computer is off or asleep then, it runs as soon as it is back.',
        '<input type="time" id="schedtime" value="' + esc(cfg.time) + '" step="900" style="width:auto">')
      + (desktop ? row('Start with Windows, in the tray', 'So the morning download runs without opening the '
        + 'app. Closing the window keeps it in the tray; Quit is in the tray’s menu.',
        '<input type="checkbox" role="switch" class="switch" data-startup="1"' + (startup ? ' checked' : '')
        + ' aria-label="Start with Windows">') : '');

    if (!mine) {
      html += '<div class="note" style="margin-top:12px">' + ico('info') + '<div>It runs on <b>'
        + esc((state.schedule && state.schedule.ownerName) || 'another computer') + '</b>, so Amazon is only asked '
        + 'once; this computer takes the figures from Supabase. '
        + '<button class="btn sec sm" data-schedhere="1" style="margin-left:6px">Run it here instead</button></div></div>';
    } else if (desktop && cfg.enabled && !startup) {
      html += '<div class="note is-warn" style="margin-top:12px">' + ico('missing') + '<div>It only runs while this '
        + 'app is open. Switch on <b>Start with Windows</b> for it to run by itself.</div></div>';
    }

    if (run) {
      const when = run.finishedAt ? new Date(run.finishedAt).toLocaleString() : null;
      const cls = run.status === 'done' || run.status === 'elsewhere' ? 'is-ok'
        : /waiting|stopped|partial|interrupted/.test(run.status) ? 'is-warn' : '';
      const words = {
        running: 'Downloading: ' + run.done + ' of ' + run.total + ' done'
          + (run.current ? ' · ' + (run.phase === 'asking' ? 'asking Amazon for ' : 'collecting ')
            + fmtShortDay(run.current.from)
            + (run.current.to !== run.current.from ? ' – ' + fmtShortDay(run.current.to) : '') : ''),
        waiting: 'Waiting for you',
        stopping: 'Stopping after this report…',
        done: 'Done ' + (when || '') + ': ' + run.done + ' of ' + run.total + ' downloaded',
        partial: 'Finished ' + (when || '') + ': ' + run.done + ' of ' + run.total + ' downloaded, ' + run.failed
          + ' did not come through',
        stopped: 'Stopped ' + (when || '') + ': ' + (run.done || 0) + ' of ' + (run.total || 0) + ' downloaded',
        interrupted: 'Interrupted',
        elsewhere: 'Not run here today',
      }[run.status] || run.status;
      html += '<div class="note ' + cls + '" style="margin-top:12px">'
        + ico(run.status === 'done' ? 'verified' : cls ? 'missing' : 'asOf') + '<div><b>' + esc(words) + '.</b>'
        + (run.note ? ' ' + esc(run.note) : '')
        + (run.failures && run.failures.length && run.status !== 'running'
          ? '<div class="meta" style="margin-top:6px">Not downloaded: ' + run.failures.slice(0, 6).map(f =>
            esc(fmtShortDay(f.from) + (f.to !== f.from ? '–' + fmtShortDay(f.to) : '')) + ' (' + esc(f.why) + ')').join('; ')
            + (run.failures.length > 6 ? '; and ' + (run.failures.length - 6) + ' more' : '') + '</div>' : '')
        + '</div></div>';
    }
    html += '<div class="btnrow" style="margin-top:12px">';
    if (run && run.status === 'running') {
      html += '<button class="btn sec" data-schedstop="1">Stop after this report</button>';
    } else if (run && run.status === 'waiting') {
      html += (continueRun ? '<button class="btn" data-schedcontinue="1">I’ve done it — continue</button>' : '')
        + '<button class="btn sec" data-schedstop="1">Stop</button>';
    } else if (mine && state.workerInfo) {
      html += '<button class="btn sec" data-schedrun="1">' + ico('download', { size: 'sm' }) + 'Run it now</button>';
    }
  ...
  function downloadsPanel() {
    ...
    const phase = derivePhase();
    html += startButton(w);
    /* Only what needs doing NOW stays here; what happened is in the bell. */
    if (phase === 'login') {
      html += '<div class="note is-warn" style="margin-top:12px">' + ico('missing') + '<div>'
        + '<b>Amazon wants you to sign in.</b> Finish signing in in the Amazon window, then '
        + 'press Retry on the report below - your place is kept and nothing is requested twice.'
        + '</div></div>';
    }
    ...
    /* What the next download will ask for: whatever the window says. It
       opens on the next eight weeks, and nothing is sent until Download. */
    html += '<div class="meta" style="margin-top:10px">You choose the dates and country in the '
      + 'window each time. It opens on the next eight weeks: <b>'
      + esc(fmtDay(horizonStart())) + ' – ' + esc(fmtDay(horizonEnd())) + '</b>.</div>';
    ...
    if (!state.setup || state.setup.phase === 'idle') {
      html += '<details class="dl" style="margin-top:20px"><summary>'
        + 'Forecast report settings</summary><div style="padding-top:12px">';

      html += '<div class="tscroll"><table><thead><tr><th>Report</th>'
        + '<th class="wrap">Page it opens</th><th>State</th><th class="act"></th>'
        + '</tr></thead><tbody>';
      for (const r of (w.reports || []).filter(x => FETCHED.indexOf(x.id) >= 0)) {
    ...
    if (state.stateHome === 'helper') {
      const kept = state.sync && /^(saved|connected)$/.test((state.syncState || {}).status);
      html += '<div class="note ' + (kept ? 'is-ok' : 'is-warn') + '">' + ico(kept ? 'verified' : 'missing')
        + '<div><b>' + (kept ? 'Kept in your Supabase project.' : 'Only on this computer.') + '</b> '
        + (kept ? 'Supabase holds your figures. This computer keeps a copy so the app works offline, '
            + 'and your other computers pick up changes within a minute.'
          : 'Your figures are in the helper\u2019s database on this computer, which every browser here '
            + 'reads. Finish setting up Supabase to keep them there and see them on your other computers.')
        + '</div></div>';
    ...
    if (state.jobs.length) {
      ...
      for (const j of state.jobs.slice(0, 12)) {
        ...
        html += '<tr><td>' + esc(reportName(j.reportType)) + (j.marketplace ? '<div class="meta">'
          + esc(j.marketplace) + '</div>' : '') + '</td>'
          ...
          + '<td class="wrap">' + esc(j.status === 'complete' ? (j.statusDetail || '')
            : (j.lastError || j.statusDetail || '')) + '</td>'
  ...
  let mirrorPeek = null;
  let mirrorNoticedAt = null;
  function noticeMirror(m) {
    const last = m && m.auto && m.auto.last;
    if (!last || !last.at || last.at === mirrorNoticedAt) return;
    mirrorNoticedAt = last.at;
    if (!last.ok) notify('error', 'Sending to Supabase did not work', last.detail || '');
    else if (last.reports || last.deleted) notify('info', 'Sent to Supabase', last.detail || '');
  }
  ...
  function mirrorPanel() {
    ...
    html += '<p class="muted" style="margin-top:0">Supabase is where your figures are '
      + 'kept: every change is saved to a project you own, and every computer running this '
      + 'app with the same project and its secret key shows the same figures, picking up '
      + 'changes within a minute. This computer keeps a copy so the app works offline. If two '
      + 'computers change something at once, nothing is overwritten: you are shown both and '
      + 'choose. Your Amazon sign-in and keys never leave this computer.</p>';
  ...
  function setupModal() {
    const m = state.mirror;
    const cs = state.cloudStatus;
    /* The third step: connected, key added, and the project still cannot
       hold the figures - almost always the table, made once. */
    const tableStep = !!(m && m.configured && m.canWrite && cs && !cs.ready && !state.sync);
    if (!state.setupOpen || !m || (m.configured && m.canWrite && !tableStep)) return '';
    const busy = state.mirrorBusy;
    const said = state.mirrorSaid;
    const step = !m.configured ? 1 : !m.canWrite ? 2 : 3;
    let html = '<div class="modal-scrim" data-setupskip="1"></div>'
      + '<div class="modal" role="dialog" aria-modal="true" aria-labelledby="setuph">'
      + '<div class="modal-h"><span class="tag nodot">Step ' + step + ' of 3</span>'
      + '<button class="btn sec sm" data-setupskip="1" style="margin-left:auto">Skip for now</button></div>'
      + '<h2 id="setuph">' + (step === 1 ? 'Keep your figures in your own Supabase database'
        : step === 2 ? 'Let this computer save your figures there'
          : 'Make the table your figures are kept in') + '</h2>'
      + '<p class="meta" style="margin:6px 0 14px">' + (step === 1
        ? 'Supabase becomes where your figures are kept: every change is saved there, and every '
          + 'computer running this app with the same project shows the same figures. Your Amazon '
          + 'sign-in and your keys stay on this computer.'
        : step === 2
          ? 'Connected. One key to go: the secret key lets this computer write to the tables, which '
            + 'stay locked to everything else. It is stored encrypted, on this computer only.'
          : 'Connected, with the secret key. One last thing, done once in Supabase: the table that '
            + 'holds your figures. Copy the setup, open the SQL editor, paste it and press <b>Run</b>. '
            + 'It only adds what is missing and changes nothing that is there.') + '</p>';
    if (step === 3) {
      const ref = String(m.url || '').replace(/^https?:\/\//, '').split('.')[0];
      html += (cs && cs.code !== 'not_ready' && cs.detail ? '<div class="note is-warn">' + ico('missing')
          + '<div>' + esc(cs.detail) + '</div></div>' : '')
        + '<pre class="sqlbox">' + esc(m.schemaSql || '') + '</pre>'
        + '<div class="btnrow" style="margin-top:12px">'
        + '<button class="btn sec" data-copysql="1">' + ico('file', { size: 'sm' }) + 'Copy the setup</button>'
        + (ref ? '<a class="btn sec" href="https://supabase.com/dashboard/project/' + esc(ref)
          + '/sql/new" target="_blank" rel="noopener">Open the SQL editor</a>' : '')
        + '<button class="btn" data-cloudcheck="1"' + (busy === 'checking' ? ' disabled' : '') + '>'
        + (busy === 'checking' ? 'Checking\u2026' : 'I ran it \u2014 check again') + '</button></div>';
    } else {
    ...
    html += '<p class="meta" style="margin:14px 0 0;color:var(--ink-3)">Skip it and a bar at the top of '
      + 'the app says what is left, and brings you back here.</p></div>';
    return html;
  }
  ...
  function noticeUpdate(info) {
    const u = info && info.updates;
    if (!u || !u.ready || !u.readyVersion || state.noticedUpdate === u.readyVersion) return;
    state.noticedUpdate = u.readyVersion;
    notify('info', 'Version ' + u.readyVersion + ' is ready to install',
      'Use the bar at the top. It installs in the background and Ledger opens again by itself.', { toast: true });
  }
  ...
  async function shellBoot() {
    ...
    try {
      ...
      noticeUpdate(state.shell);
      window.desktopShell.onUpdate(info => { state.shell = info; noticeUpdate(info); render(); });
      render();
  ...
  async function startCloudSync() {
    ...
    try {
      ...
      if (!st || !st.ready) {
        ...
        paintSyncBadge();
        /* Connected, key added, and still not holding anything - almost always
           the one-time table. The setup window's last step does exactly that. */
        if (!state.setupSkipped && st && st.code === 'not_ready') state.setupOpen = true;
        return;
      }
      const sync = Sync.create({
        ...
        onStatus: s => {
          state.syncState = s;
          paintSyncBadge();
          if (s.status === 'failed') notify('error', 'Syncing with your other computers did not work', s.reason || '');
          if (s.status === 'conflict') notify('warn', 'Changed on another computer',
            'Nothing was overwritten. Open Data & Assumptions to choose which version to keep.');
  ...
  /* The files behind any import this computer has the row for but not the
     file itself: on first contact, and whenever another computer added one.
     A file not there yet is marked, and asked for again on the next pull. */
  async function pullBlobs() {
    const sync = state.sync;
    let got = 0;
    for (const imp of state.imports) {
      if (imp.family === 'Fees & Economics Preview') {
        if (state.previews.some(x => x.importId === imp.id)) { delete imp.remoteOnly; continue; }
        const p = await sync.getBlob(blobKeyFor(imp));
        if (p) { p.importId = imp.id; state.previews.push(p); delete imp.remoteOnly; got++; }
        ...
        const led = blob ? Store.deserialiseLedger(blob, Ledger) : null;
        if (led && led.rowCount) { state.ledger = led; delete imp.remoteOnly; got++; await persistLedger(); }
        else imp.remoteOnly = true;
      }
    }
    return got;
  ...
  /* Supabase is where the figures live. Every minute while the app is open,
     and whenever its window comes back into view, this computer asks the
     project whether another computer changed anything, and takes it. Its own
     unsent changes go first - a push refuses rather than overwrite - so a
     pull can never drop them. */
  let pulling = false, pullTimer = null, lastPullAt = 0;
  async function syncPull() {
    const sync = state.sync;
    if (!sync || !sync.available || pulling || state.syncBusy || syncTimer) return;
    const now = state.syncState && state.syncState.status;
    if (now === 'conflict' || now === 'saving' || now === 'syncing') return;
    pulling = true;
    lastPullAt = Date.now();
    try {
      const remote = await sync.pullState();
      if (remote && remote.rev > sync.baseRev) {
        const merged = sync.mergeState(collectSyncState(), remote.payload, false);
        applySyncState(merged);
        sync.setBaseRev(remote.rev);
        await pullBlobs();
        bumpForecast();
        await persist({ quiet: true });
        /* Only send back what this computer adds; the same thing sent back
           would set the other computer off doing the same. */
        if (!Sync.sameState(merged, remote.payload) || now === 'failed') scheduleSyncPush();
        else sync.setStatus('saved', { lastSyncedAt: new Date().toISOString(), reason: null });
        render();
      } else {
        if (state.imports.some(i => i.remoteOnly) && await pullBlobs()) {
          bumpForecast();
          await persist({ quiet: true });
          render();
        }
        if (now === 'failed') scheduleSyncPush();     // a failed save is tried again, not left
      }
    } catch (e) {
      if (!(e && e.code === 'conflict')) sync.setStatus('failed', { reason: describeSyncError(e) });
    } finally {
      pulling = false;
      /* "Saved 3 min ago" stays true between redraws. */
      const sn = document.getElementById('sidenote');
      if (sn) sn.innerHTML = forecastStatus();
    }
  }
  function startPulling() {
    if (!pullTimer) pullTimer = setInterval(syncPull, 60000);
  }
  const pullIfStale = () => { if (state.sync && Date.now() - lastPullAt > 15000) syncPull(); };
  ...
  function applySyncState(merged) {
    Object.assign(state, {
      ...
      schedule: merged.schedule || state.schedule || null,
      removedImports: merged.removedImports || [],
    });
    /* A file another computer let go goes here too - its parsed rows with it. */
    const keep = new Set(state.imports.map(i => i.id));
    state.previews = state.previews.filter(p => !p.importId || keep.has(p.importId));
  }
  ...
  async function syncPush(quiet) {
    ...
    try {
      ...
      /* The files first, then the list that names them - so another computer
         never reads a row whose file has not arrived. */
      const holds = imp => (imp.family === 'Fees & Economics Preview'
        ? state.previews.some(x => x.importId === imp.id)
        : imp.family === 'Payments transactions' && !!state.ledger);
      let manifest = await sync.readManifest();
      for (const imp of state.imports) {
        if (imp.remoteOnly || !holds(imp)) continue;
        ...
        if (imp.family === 'Fees & Economics Preview') {
          ...
          await sync.putBlob(blobKeyFor(imp), p, { kind: 'preview', name: imp.name });
        } else {
      ...
      /* "Saved" is said only after asking the project and finding it there -
         never on the strength of a request that did not complain. */
      const back = await sync.pullState();
      manifest = await sync.readManifest();
      const missing = state.imports.filter(imp => !imp.remoteOnly && holds(imp)
        && !manifest[sync.safeKey ? sync.safeKey(blobKeyFor(imp)) : blobKeyFor(imp)]);
      if (!back || back.rev < sync.baseRev || missing.length) {
        sync.setStatus('failed', { progress: null, reason: missing.length
          ? missing.length + (missing.length === 1 ? ' file is' : ' files are') + ' not in Supabase yet. '
            + 'They are on this computer and will be sent again in a minute.'
          : 'Supabase did not show what was just saved. It will be sent again in a minute.' });
        return;
      }
      sync.setStatus('saved', { lastSyncedAt: new Date().toISOString(), progress: null, reason: null });
    } catch (e) {
  ...
  function syncPanel() {
    ...
    let html = '<section class="card" id="syncpanel"><div class="card-h">' + ico('asOf')
      + '<h2>Supabase</h2></div>'
      + '<div class="sub">Where your figures are kept, and whether your other computers see them.</div>';
  ...
  function stateDoc() {
    return {
      ...
      schedule: state.schedule, removedImports: state.removedImports,
    };
  }
  ...
  function applyStateDoc(s) {
    ...
    Object.assign(state, {
      ...
      schedule: s.schedule || null, removedImports: s.removedImports || [],
  ...
  async function persist(opts) {
    ...
    /* Every local save goes to Supabase. Hooking it here rather than at each
       of the fourteen call sites means a new one cannot forget to. A save
       that only took Supabase's own copy is not sent back to it. */
    if (!(opts && opts.quiet)) scheduleSyncPush();
  }
  ...
  function topControls() {
    ...
    let html = guideButton() + noticeBell();
    if (s === 'inputs' || s === 'settings') {
      if (s === 'inputs' && state.show.balance) html += '<button class="btn sec" id="updbal">Update balance</button>';
      if (state.workerInfo) html += refreshButton();
      return html;
    }
    if (s === 'sales' || s === 'data') {
      /* Only when there is a choice to make. */
      if (s === 'sales' && new Set(state.previews.map(p => p.store).filter(Boolean)).size > 1) html += marketplaceSelect();
      if (s === 'sales' && new Set(state.previews.map(p => p.currency).filter(Boolean)).size > 1) html += currencySelect();
  ...
  function updateBar() {
    ...
    /* Installing: said with the app's own face, over everything, from the
       moment it is asked for until the window closes - rather than the window
       simply vanishing. */
    if (state.installing) {
      return '<div class="inst-scrim" role="alertdialog" aria-modal="true" aria-labelledby="insttitle">'
        + '<div class="inst-sheet">' + APP_ICON
        + '<h2 id="insttitle">Installing Ledger ' + esc(state.installing) + '</h2>'
        + '<p>The app closes, installs the update and opens again by itself \u2014 about ten seconds. '
        + 'Nothing to click.</p>'
        + '<div class="progress progress--indeterminate" role="progressbar" aria-label="Installing">'
        + '<div class="progress__fill"></div></div></div></div>';
    }

    if (u.ready) {
      return '<div class="updbar is-ready">' + ico('verified')
        + '<span class="grow"><b>Ledger ' + esc(u.readyVersion || '')
        + ' is downloaded and ready.</b> It installs in the background and the app opens again by itself '
        + '\u2014 about ten seconds.</span>'
        + '<button class="btn sm" data-installupdate="1">' + (state.installHeld ? 'Try again' : 'Restart and install')
        + '</button>'
        + (state.installHeld ? '<span class="meta" style="flex-basis:100%">' + esc(state.installHeld) + '</span>' : '')
        + '</div>';
    }
    ...
    /* Downloading is worth showing because it explains the disk and network
       ...
       assume has stalled - so it shows how far it has got. */
    if (u.state === 'available') {
      const pc = typeof u.percent === 'number' ? Math.max(0, Math.min(100, Math.round(u.percent))) : null;
      return '<div class="updbar is-working">' + ico('download')
        + '<span class="grow"><b>Downloading ' + (u.version ? 'Ledger ' + esc(u.version) : 'an update') + '\u2026</b> '
        + 'You can keep working; it offers to install once it is here.</span>'
        + (pc != null ? '<span class="upd-pct">' + pc + '%</span>' : '')
        + '<div class="progress' + (pc == null ? ' progress--indeterminate' : '') + '" role="progressbar" '
        + 'aria-label="Downloading the update"' + (pc != null ? ' aria-valuemin="0" aria-valuemax="100" aria-valuenow="'
          + pc + '"' : '') + '><div class="progress__fill"' + (pc != null ? ' style="width:' + pc + '%"' : '')
        + '></div></div></div>';
    }
    return '';
  }
  ...
  function dataStatus(readyN, totalN) {
    ...
    const tx = led && led.to ? row('Transactions', 'to ' + fmtShortDay(led.to), 'ok')
      : row('Transactions', 'Not used', null);
  ...
  function render() {
    ...
    /* A forecast for Canada is in CAD. Showing USD over it would read as
       "no forecast" - so when nothing matches the chosen currency and store,
       follow the newest download instead. */
    if (state.screen === 'sales' && state.previews.length && !scopedPreviews().length) {
      const latest = state.previews[state.previews.length - 1];
      if (latest.currency) state.filters.currency = latest.currency;
      state.filters.marketplace = null;
    }
    M.setCurrency(state.filters.currency);
    const sc = screenById(state.screen);
    $('#pagetitle').textContent = sc.title;
    const tbt = $('#tbtitle');
    if (tbt) tbt.textContent = sc.title;
    const blurb = $('#pageblurb');
    if (blurb) blurb.textContent = sc.blurb || '';

    const ub = $('#updatebar');
    if (ub) ub.innerHTML = updateBar() + scheduleBar() + cloudBar();
    ...
    try {
      const ds = dataset();
      body = ds.hiddenByRange && !PLAIN.has(state.screen)
        ? rangeEmptyCard(ds)
    ...
    $('#view').innerHTML = (state.screen === 'dashboard' ? setupCard() : '') + body
      + (PLAIN.has(state.screen) ? '' : dateBasisNote());
    const sm = $('#setupmodal');
    if (sm) sm.innerHTML = setupModal() || getWindow();

    const rd = dataset().readiness;
    const readyN = Dataset.ready(rd).length, blockedN = Dataset.blocking(rd).length;
    const countedN = Dataset.counted(rd).length;
    ...
    /* More tools: everything 5.0 took out of the way, one click from view. */
    if (state.show.more) {
      const open = state.moreOpen;
      navHtml += '<button type="button" id="moretoggle" class="navmore" aria-expanded="' + open + '">'
        + ico(open ? 'chevronDown' : 'chevronRight') + '<span>More tools</span>'
        + '<span class="navbadge" style="color:var(--ink-3)">' + HIDDEN_SCREENS.length + '</span></button>';
      if (open) {
        let g = null;
        navHtml += '<div class="navsub">';
        for (const s of HIDDEN_SCREENS) {
          if (s.group !== g) { navHtml += '<div class="navsubgroup" aria-hidden="true">' + esc(s.group) + '</div>'; g = s.group; }
          navHtml += '<button data-go="' + s.id + '"' + (s.id === state.screen ? ' aria-current="page"' : '')
            + '>' + ico(s.icon) + '<span>' + esc(s.name) + '</span></button>';
        }
        navHtml += '</div>';
      }
    }
    navHtml += '<div class="navgroup" aria-hidden="true">App</div>';
  ...
  function go(id) {
    state.screen = id; state.tab = null;
    /* Landing on one of the More tools opens the list, so where you are shows. */
    if (HIDDEN_SCREENS.some(s => s.id === id) && !state.moreOpen) { state.moreOpen = true; saveMoreOpen(); }
    render();
  }
  ...
  document.addEventListener('click', e => {
    ...
    if (e.target.closest('#noticebtn')) { openNotices(!state.noticeOpen); return; }
    if (e.target.closest('#moretoggle')) { state.moreOpen = !state.moreOpen; saveMoreOpen(); render(); return; }
    const gd = e.target.closest('[data-guide]');
    if (gd) { startGuide(gd.getAttribute('data-guide') === 'new' ? 'new' : 'full'); return; }
    if (e.target.closest('#guidebtn')) { startGuide('full'); return; }
    const sm = e.target.closest('[data-smetric]');
    if (sm) {
      /* On or off the chart. The last one stays: a chart of nothing says nothing. */
      const key = sm.getAttribute('data-smetric');
      const cur = (state.salesMetrics || []).slice();
      const at = cur.indexOf(key);
      if (at < 0) cur.push(key); else if (cur.length > 1) cur.splice(at, 1);
      state.salesMetrics = cur; render();
      const again = document.querySelector('[data-smetric="' + key + '"]');
      if (again) again.focus({ preventScroll: true });
      return;
    }
    if (e.target.closest('[data-sall]')) {
      state.salesMetrics = (state.salesMetrics || []).length === 4 ? ['netSales']
        : Charts.MEASURES.map(x => x.key);
      render(); return;
    }
    const ss = e.target.closest('[data-ssort]');
    if (ss) { state.skuSort = ss.getAttribute('data-ssort'); render(); return; }
    const ssh = e.target.closest('[data-sshare]');
    if (ssh) { state.skuShare = ssh.getAttribute('data-sshare') === '1'; render(); return; }
    const sall = e.target.closest('[data-sskuall]');
    if (sall) { state.skuAll = sall.getAttribute('data-sskuall') === '1'; render(); return; }
    const psk = e.target.closest('[data-sku]');
    if (psk) { state.skuPick = psk.getAttribute('data-sku'); render(); return; }
    const ser = e.target.closest('[data-sser]');
    if (ser) {
      const id = ser.getAttribute('data-sser'), k = ser.getAttribute('data-key');
      const cur = (state.chartShow[id] || []).slice();
      const at = cur.indexOf(k);
      if (at < 0) cur.push(k); else if (cur.length > 1) cur.splice(at, 1);
      state.chartShow[id] = cur; render(); return;
    }
    const cmo = e.target.closest('[data-cmode]');
    if (cmo) { state.chartMode[cmo.getAttribute('data-chart')] = cmo.getAttribute('data-cmode'); render(); return; }
    const czo = e.target.closest('[data-czoom]');
    if (czo) { zoomChart(czo.getAttribute('data-chart'), czo.getAttribute('data-czoom')); return; }
    /* a tap on a bar reads it, as hovering does */
    const tz = e.target.closest('.tipzone');
    if (tz) { const tt = e.target.closest('[data-tip]'); Charts.showTip(tz, tt, e.clientX, e.clientY); return; }
    const smo = e.target.closest('[data-smode]');
    if (smo) { state.salesMode = smo.getAttribute('data-smode'); render(); return; }
    /* a tap on the chart reads it, as hovering does */
    const fch = e.target.closest('.fchart');
    if (fch) {
      if (Date.now() - fcDragEnd > 300) Charts.forecastHover(fch, Charts.forecastAt(fch, e.clientX));
      return;
    }
    const sd = e.target.closest('[data-sdays]');
    if (sd) {
      state.salesFrom = null;
      state.salesTo = CSV.addDays(state.today, +sd.getAttribute('data-sdays'));
      render(); return;
    }
    const sst = e.target.closest('[data-sstep]');
    if (sst) {
      /* The same number of days, earlier or later: 27-28 Sep, then 29-30. */
      const r = salesRange();
      const shift = (+sst.getAttribute('data-sstep')) * (CSV.daysBetween(r.from, r.to) + 1);
      const nf = CSV.addDays(r.from, shift);
      state.salesFrom = nf === horizonStart() ? null : nf;
      state.salesTo = CSV.addDays(r.to, shift);
      render(); return;
    }
    if (e.target.closest('[data-sget]')) {
      /* Amazon forecasts from tomorrow, so a window reaching back starts there. */
      const r = salesRange();
      openGetWindow({ from: r.from > horizonStart() ? r.from : horizonStart(), to: r.to });
      return;
    }
    if (e.target.closest('[data-noticeclose]')) { openNotices(false); return; }
    if (e.target.closest('#noticeclear')) { state.notices = []; saveNotices(); paintNotices(); return; }
    if (e.target.closest('[data-toastx]')) {
      const t = document.getElementById('toastslot'); if (t) t.innerHTML = ''; return;
    }
    if (e.target.closest('[data-toastopen]')) {
      const t = document.getElementById('toastslot'); if (t) t.innerHTML = '';
      openNotices(true); return;
    }
    if (e.target.closest('[data-startall]')) { startAll(); return; }
    if (state.getWin) {
      if (e.target.closest('[data-gwclose]')) { state.getWin = null; render(); return; }
      if (e.target.closest('#gw-go')) { submitGetWindow(); return; }
      if (e.target.closest('[data-gwsetup]')) {
        state.getWin = null; render();
        chooseProfileThenSetup('date-range-transactions');
        return;
      }
      const gf = e.target.closest('[data-gwf]');
      if (gf) {
        readGetWindow();
        state.getWin.fFrom = horizonStart();
        state.getWin.fTo = CSV.addDays(state.today, +gf.getAttribute('data-gwf'));
        state.getWin.error = null;
        render(); return;
      }
      const gh = e.target.closest('[data-gwh]');
      if (gh) {
        readGetWindow();
        const k = gh.getAttribute('data-gwh');
        const t = state.today;
        if (k === 'month') { state.getWin.hFrom = t.slice(0, 8) + '01'; state.getWin.hTo = t; }
        else if (k === 'lastmonth') {
          const end = CSV.addDays(t.slice(0, 8) + '01', -1);
          state.getWin.hFrom = end.slice(0, 8) + '01'; state.getWin.hTo = end;
        } else { state.getWin.hFrom = CSV.addDays(t, -(+k - 1)); state.getWin.hTo = t; }
        state.getWin.error = null;
        render(); return;
      }
    }
    ...
    if (e.target.closest('[data-cloudbarhide]')) { state.cloudBarHidden = true; render(); return; }
    if (e.target.closest('[data-schedrun]')) { runSchedule('button'); return; }
    if (e.target.closest('[data-schedcontinue]')) {
      if (continueRun) { const go = continueRun; continueRun = null; go(true); }
      render(); return;
    }
    if (e.target.closest('[data-schedstop]')) {
      const run = state.scheduleRun;
      if (run && /running|waiting/.test(run.status)) { run.status = 'stopping'; saveRun(); }
      if (continueRun) { const go = continueRun; continueRun = null; go(false); }
      render(); return;
    }
    if (e.target.closest('[data-schedhere]')) { claimSchedule(); render(); return; }
    if (e.target.closest('[data-copysql]')) {
      const sql = (state.mirror && state.mirror.schemaSql) || '';
      const done = () => notify('ok', 'Setup copied', 'Paste it into Supabase \u2192 SQL Editor and press Run.',
        { toast: true });
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(sql).then(done).catch(() => notify('warn', 'Could not copy',
          'Select the text in the box and copy it by hand.', { toast: true }));
      }
      return;
    }
    if (e.target.closest('[data-cloudcheck]')) {
      state.mirrorBusy = 'checking'; render();
      stopCloudSync();
      startCloudSync().then(() => {
        state.mirrorBusy = '';
        if (state.sync) { state.setupOpen = false; notify('ok', 'Supabase is ready', 'Your figures are being saved there now.', { toast: true }); }
        render();
      });
      return;
    }
    ...
    if (iu && window.desktopShell && window.desktopShell.install) {
      const u = (state.shell && state.shell.updates) || {};
      state.installHeld = null;
      state.installing = u.readyVersion || 'the update';
      render();
      window.desktopShell.install()
        .then(r => {
          if (r && r.ok === false) {
            state.installing = null;
            state.installHeld = r.detail || 'The update could not be installed yet.';
            render();
            return;
          }
          /* The window normally closes within a few seconds. If it is still
             here long after, say so rather than leave the sheet up for ever. */
          setTimeout(() => {
            if (!state.installing) return;
            state.installing = null;
            state.installHeld = 'The update did not start. Try again, or quit Ledger from the tray and open it again '
              + '\u2014 it installs as it closes.';
            render();
          }, 60000);
        })
        .catch(e2 => {
          state.installing = null;
          state.installHeld = (e2 && e2.message) || 'The update could not be installed yet.';
          render();
        });
      return;
    }

    const tp = e.target.closest('[data-themepick]');
    if (tp) { applyTheme(tp.getAttribute('data-themepick')); render(); return; }
    ...
    if (jy) {
      ...
      const imp = state.imports.find(i => i.jobId === id
        || (Array.isArray(i.jobIds) && i.jobIds.indexOf(id) >= 0));
      /* The same file can arrive in several downloads. Deleting ONE of them
         must not take the figures away while another still delivered them -
         that is how deleting an old duplicate row emptied the forecast. */
      const otherJobs = imp ? [...new Set([].concat(imp.jobIds || [], imp.jobId ? [imp.jobId] : []))]
        .filter(x => x !== id && state.jobs.some(j => j.jobId === x)) : [];
      const keepFigures = otherJobs.length > 0;
      state.confirmJob = null;
      state.worker.remove(id)
        .then(res => {
          if (res && res.archived) refreshMirrorSoon();
          if (keepFigures) {
            imp.jobIds = otherJobs;
            imp.jobId = otherJobs[0];
            persist();
          }
          /* Reported, not assumed. The helper says what it actually removed:
             a file it did not put there is left alone and says so. */
          const rows = !imp ? '' : keepFigures
            ? '. Its figures stay: the same file also came from another download'
            : ' and the ' + (imp.rowCount || 0).toLocaleString() + ' rows it put in the app';
        ...
        .then(() => { if (state.jobNote) notify('info', state.jobNote); state.jobNote = null; })
        .catch(err => { notify('error', 'Could not delete the report', (err && err.message) || String(err)); })
        ...
        .then(() => imp && !keepFigures ? removeImport(imp.id) : null)
  ...
  document.addEventListener('change', e => {
    ...
    if (id === 'skupick') { state.skuPick = e.target.value; render(); return; }
    if (e.target.getAttribute && e.target.getAttribute('data-sched') === 'enabled') {
      const on = !!e.target.checked;
      setSchedule({ enabled: on });
      if (on && !(state.schedule && state.schedule.owner)) claimSchedule();
      render(); return;
    }
    if (id === 'schedtime') {
      if (/^\d{2}:\d{2}$/.test(e.target.value)) setSchedule({ time: e.target.value });
      render(); return;
    }
    if (e.target.getAttribute && e.target.getAttribute('data-startup')) {
      const want = !!e.target.checked;
      if (window.desktopShell && window.desktopShell.startup) {
        try { localStorage.setItem('fba-startup-set', '1'); } catch (err) { /* this session */ }
        window.desktopShell.startup(want).then(v => { state.shell = Object.assign({}, state.shell || {}, { startup: v }); render(); });
      }
      return;
    }
    const shw = e.target.getAttribute && e.target.getAttribute('data-show');
    if (shw) { state.show[shw] = !!e.target.checked; saveShow(); render(); return; }
    if (id === 's-from') {
      const v = e.target.value;
      if (v) {
        state.salesFrom = v === horizonStart() ? null : v;
        if (salesRange().to < v) state.salesTo = v;
        render();
      }
      return;
    }
    if (id === 's-to') {
      const v = e.target.value;
      if (v && v >= salesRange().from) { state.salesTo = v; render(); }
      return;
    }
    if (id === 's-scroll') {
      /* Let go of the bar: the whole page follows. (While dragging, only the
         dates above it change - redrawing under the pointer would drop it.) */
      const d = scrollDates(e.target);
      state.salesFrom = d.from === horizonStart() ? null : d.from;
      state.salesTo = d.to;
      render();
      return;
    }
    if (state.getWin && e.target.closest && e.target.closest('[aria-labelledby="gwh"]')) {
      const box = e.target.getAttribute && e.target.getAttribute('data-gwmkt');
      readGetWindow();
      if (box) {
        const set = new Set(state.getWin.mkts);
        if (e.target.checked) set.add(box); else set.delete(box);
        state.getWin.mkts = MARKETPLACES.filter(m => set.has(m));
      }
      state.getWin.error = null;
      render();
      return;
  ...
  function scrollDates(el) {
    const from = CSV.addDays(el.getAttribute('data-span-from'), +el.value);
    return { from, to: CSV.addDays(from, (+el.getAttribute('data-len') || 1) - 1) };
  }
  document.addEventListener('input', e => {
    if (!e.target || e.target.id !== 's-scroll') return;
    const d = scrollDates(e.target);
    const out = document.getElementById('s-shown');
    if (out) out.textContent = fmtDay(d.from) + (d.to !== d.from ? ' \u2013 ' + fmtDay(d.to) : '');
  });

  /* Zooming a chart: drag across it to zoom into those dates, double-click
     to show them all again, Ctrl + scroll to zoom around the pointer. */
  let fcDrag = null, fcDragEnd = 0, fcWheelAt = 0;
  document.addEventListener('mousedown', e => {
    if (e.button !== 0 || !e.target.closest) return;
    const c = e.target.closest('.fchart');
    if (!c || !e.target.closest('svg')) return;
    const i = Charts.forecastAt(c, e.clientX);
    if (i == null) return;
    fcDrag = { c, i0: i, x0: e.clientX, moved: false };
    e.preventDefault();
    c.focus({ preventScroll: true });   // so + and - zoom it next
  });
  document.addEventListener('mouseup', e => {
    const d = fcDrag;
    fcDrag = null;
    if (!d || !d.moved) return;
    Charts.forecastBrush(d.c, null);
    fcDragEnd = Date.now();
    const r = Charts.forecastRange(d.c, d.i0, Charts.forecastAt(d.c, e.clientX, true));
    if (r && r.from < r.to) setZoom(d.c.getAttribute('data-chart'), r);
  });
  document.addEventListener('dblclick', e => {
    const c = e.target.closest && e.target.closest('.fchart');
    if (c) setZoom(c.getAttribute('data-chart'), null);
  });
  document.addEventListener('wheel', e => {
    ...
    if (!c) return;
    e.preventDefault();
    if (Date.now() - fcWheelAt < 180) return;
    fcWheelAt = Date.now();
    const i = Charts.forecastAt(c, e.clientX, true);
    const r = Charts.forecastRange(c, i, i);
    zoomChart(c.getAttribute('data-chart'), e.deltaY < 0 ? 'in' : 'out', r && r.from);
  }, { passive: false });

  /* The forecast chart's reading follows the pointer, and goes when it leaves. */
  document.addEventListener('mousemove', e => {
    if (fcDrag) {
      if (Math.abs(e.clientX - fcDrag.x0) > 6) fcDrag.moved = true;
      if (fcDrag.moved) Charts.forecastBrush(fcDrag.c, fcDrag.i0, Charts.forecastAt(fcDrag.c, e.clientX, true));
    }
    const z = e.target.closest && e.target.closest('.tipzone');
    if (z) { Charts.showTip(z, e.target.closest('[data-tip]'), e.clientX, e.clientY); return; }
    const c = e.target.closest && e.target.closest('.fchart');
    if (!c) return;
    const i = Charts.forecastAt(c, e.clientX);
    if (i !== c._fcAt) Charts.forecastHover(c, i);
  });
  document.addEventListener('mouseout', e => {
    const z = e.target.closest && e.target.closest('.tipzone');
    if (z && !(e.relatedTarget && z.contains(e.relatedTarget))) Charts.showTip(z, null);
    const c = e.target.closest && e.target.closest('.fchart');
    if (c && !(e.relatedTarget && c.contains(e.relatedTarget))) {
      Charts.forecastHover(c, null);
    }
  });
  document.addEventListener('focusout', e => {
    if (e.target.classList && e.target.classList.contains('fchart')) Charts.forecastHover(e.target, null);
  });

  document.addEventListener('keydown', e => {
    if (e.target && e.target.classList && e.target.classList.contains('fchart')) {
      if (Charts.forecastStep(e.target, e.key)) { e.preventDefault(); return; }
      const zk = { '+': 'in', '=': 'in', '-': 'out', '_': 'out', '0': 'reset' }[e.key];
      if (zk && !e.ctrlKey && !e.metaKey) {
        e.preventDefault();
        const id = e.target.getAttribute('data-chart');
        const at = e.target._fcAt;
        const r = at == null ? null : Charts.forecastRange(e.target, at, at);
        zoomChart(id, zk, r && r.from);
        const again = document.querySelector('.fchart[data-chart="' + id + '"]');
        if (again) again.focus({ preventScroll: true });
        return;
      }
    }
    if (e.key === 'Escape' && state.ask.open) toggleAsk(false);
    if (e.key === 'Escape' && state.getWin) { state.getWin = null; render(); }
    if (e.key === 'Escape' && state.noticeOpen) openNotices(false);
  ...
  /* ── the guide ───────────────────────────────────────────────────────────
     ...
     A step-by-step tour that points at each part of the screen. It runs by
     itself on a first install; after an update it runs a shorter one naming
     only what changed (CHANGES, newest last). Either can be replayed from
     Settings or the "?" button. What has been seen is kept on this computer.

     A step whose part is not on screen yet (no forecast downloaded, say) is
     shown in the middle, saying where that part will be - never skipped
     silently and never pointed at something that is not there. */
  const GUIDE = [
    { title: 'Welcome to Ledger',
      body: 'It shows Amazon’s own forecast of your sales and fees, from tomorrow to any date you pick. '
        + 'This guide takes about a minute. Use the arrow keys or the buttons; replay it any time from Settings.' },
    { target: '#topcontrols [data-startall]', title: 'Get Amazon’s forecast',
      body: 'Press this, pick the country and the dates, then Download. The first time, a browser window opens '
        + 'for you to sign in to Seller Central — your login stays on this computer. It takes about a minute.',
      missing: 'This button appears once the helper is running. Without it, drop a Fees & Economics Preview '
        + 'CSV on the Downloads screen.' },
    { screen: 'sales', target: '#salesdates', title: 'Pick your dates',
      body: 'From is always tomorrow. Choose Up to, or a quick 7 days to 8 weeks. If some of the days have '
        + 'no forecast yet, the app says so and offers to download them.',
      missing: 'The dates appear here once a forecast is downloaded.' },
    { screen: 'sales', target: '#salestiles', title: 'Your four totals',
      body: 'Net sales, Amazon fees, advertising, and what is left after both. Click a tile to put it on the '
        + 'chart or take it off — compare one, two, three or all four.',
      missing: 'The totals appear here once a forecast is downloaded.' },
    { screen: 'sales', target: '#saleschart .fchart', title: 'Read the chart',
      body: 'Hover over it (or click it and use the arrow keys) to read any day: each figure for that day and '
        + 'the total to date. Switch between Running total and Week by week at the top right.',
      missing: 'The chart appears here once a forecast is downloaded.' },
    { screen: 'sales', target: '#spendcard', title: 'Where your net sales go',
      body: 'Every Amazon fee and the advertising as a share of your net sales, and what is left. Hover over '
        + 'the bar for the amounts.',
      missing: 'This appears once a forecast is downloaded.' },
    { screen: 'sales', target: '#skuspend', title: 'Costs by product',
      body: 'Each product’s fees, kind by kind, against its own net sales — so you can see what takes '
        + 'the most. Sort by biggest costs or highest share; a product that costs more than it sells says so in red.',
      missing: 'This appears once a forecast is downloaded.' },
    { target: '#noticebtn', title: 'Notifications',
      body: 'Downloads, imports, errors and updates are listed here, so the screens stay clean. A number shows '
        + 'what is new.' },
    { target: '#sidenav [data-go="data"]', title: 'Downloads',
      body: 'Every file you have, what it covers, and a place to drop a CSV you saved from Seller Central yourself.' },
    { target: '#moretoggle', title: 'More tools',
      body: 'The other tools: Cash Dashboard, Payout Forecast, Cash Plan, Amazon Expenses, Profitability, '
        + 'Reconciliation, and Your own figures. Click to open the list.',
      missing: 'More tools is turned off in Settings.' },
    { target: '#sidenav [data-go="settings"]', title: 'Settings',
      body: 'Turn the extra inputs on or off — current balance, payout rules, bank deposits, product costs, '
        + 'advertising billing — and replay this guide.' },
    { title: 'You’re set',
      body: 'Start with Get today’s Amazon data at the top. After each update, a short guide like this one '
        + 'shows exactly what changed.' },
  ];
  /* What changed, by version. The "what's new" guide shows every version
     newer than the last one seen, oldest first. */
  const CHANGES = [
    { version: '5.1.0', steps: [
      { screen: 'sales', target: '#salestiles', title: 'Compare in one chart',
        body: 'The tiles now switch on and off. Pick one, two, three or all four, and the chart below shows '
          + 'them together.', missing: 'The tiles are on the forecast screen once a forecast is downloaded.' },
      { screen: 'sales', target: '#saleschart .fchart', title: 'That day, and to date',
        body: 'Hovering now reads each figure for that day and the total to date, with a line for every tile '
          + 'you picked. “Compare all four” does it in one click.',
        missing: 'The chart is on the forecast screen once a forecast is downloaded.' },
      { screen: 'sales', target: '#spendcard', title: 'New: where your net sales go',
        body: 'Every fee and the advertising as a share of net sales, and what is left for you.',
        missing: 'This is on the forecast screen once a forecast is downloaded.' },
      { screen: 'sales', target: '#skuspend', title: 'New: costs by product',
        body: 'Each product’s fees by kind, laid against its own net sales. Sort to find what takes the most.',
        missing: 'This is on the forecast screen once a forecast is downloaded.' },
      { target: '#moretoggle', title: 'Back: More tools',
        body: 'Everything taken out in 5.0 is back, in one list: the cash tools, expenses, profitability, '
          + 'reconciliation, and Your own figures.', missing: 'More tools is turned off in Settings.' },
      { target: '#sidenav [data-go="settings"]', title: 'New: Settings',
        body: 'Turn the current balance, payout rules, bank deposits, product costs and advertising billing on '
          + 'or off, as you like.' },
      { target: '#guidebtn', title: 'New: this guide',
        body: 'Replay the full guide, or this list of changes, from here or from Settings.' },
    ] },
  ];
  CHANGES.push({ version: '5.2.0', steps: [
    { screen: 'sales', target: '#saleschart .fchart', title: 'Every line named',
      body: 'Each line now has its name and amount at its end, pushed apart where lines run close \u2014 like '
        + 'advertising and after fees, which are often almost the same. After fees and ads is dashed so the line '
        + 'under it shows through.', missing: 'The chart is on the forecast screen once a forecast is downloaded.' },
    { screen: 'sales', target: '#saleschart .zoombar', title: 'Zoom',
      body: 'Drag across any chart to zoom into those dates. Ctrl + scroll zooms around the pointer, the arrows move '
        + 'earlier or later, and a double-click shows everything again.',
      missing: 'The chart is on the forecast screen once a forecast is downloaded.' },
    { screen: 'sales', target: '#spendchart', title: 'New: fees against net sales, over time',
      body: 'Net sales and every kind of fee in one chart. Click the names to show or hide them; the reading gives '
        + 'each one\u2019s share of sales so far.', missing: 'This is on the forecast screen once a forecast is downloaded.' },
    { screen: 'sales', target: '#skuchart', title: 'New: one product over time',
      body: 'Pick a product here, or click its bar in the list, to chart its sales against its own fees.',
      missing: 'This is on the forecast screen once a forecast is downloaded.' },
  ] });
  CHANGES.push({ version: '5.2.1', steps: [
    { screen: 'sales', target: '#salestiles', title: 'Money with its sign',
      body: 'Amounts now read $974.61 and -$318.51 instead of USD 974.61 \u2014 on the tiles, in the tables, '
        + 'on the charts and in every reading. A Canadian or Mexican forecast shows CA$ or MX$.',
      missing: 'The tiles are on the forecast screen once a forecast is downloaded.' },
  ] });
  CHANGES.push({ version: '5.3.0', steps: [
    { target: '#sidenote', title: 'Supabase is where your figures are kept',
      body: 'Every change is saved to your Supabase project, and your other computers pick it up within a '
        + 'minute \u2014 no reopening. This panel shows whether Supabase holds your figures and when it last saved.' },
    { title: 'If Supabase is not finished',
      body: 'A bar at the top of every screen says exactly what is left \u2014 the secret key, or the one-time '
        + 'table \u2014 and the setup walks you through it, with a Copy button for the table.' },
  ] });
  CHANGES.push({ version: '5.4.0', steps: [
    { screen: 'settings', target: '#schedulecard', title: 'New: every morning, by itself',
      body: 'At 6 AM the app downloads Amazon\u2019s forecast for the next 30 days one day at a time, then week by '
        + 'week to about four months \u2014 one report at a time, and saved to Supabase. It starts with Windows, '
        + 'in the tray. Change the time, or switch it off, here.' },
    { target: '#sidenote', title: 'Watch it here',
      body: '\u201cEvery morning\u201d says when it last ran, or how far it has got. If Amazon asks you to sign in or '
        + 'complete a check, it stops, tells you (a Windows notification too), and carries on once you have.' },
    { screen: 'sales', target: '#salesdates', title: 'Four months, day by day',
      body: 'Each day now uses the newest download that covers it, preferring the one-day report, so the chart '
        + 'shows Amazon\u2019s real day-to-day shape. \u201c4 months\u201d shows the whole span.',
      missing: 'This is on the forecast screen once a forecast is downloaded.' },
  ] });
  CHANGES.push({ version: '5.5.0', steps: [
    { screen: 'settings', target: '#schedulecard', title: 'One computer at a time',
      body: 'Every computer has a helper, but only one runs the morning download. Before it starts it leaves a '
        + 'note in Supabase and checks it; a second computer that starts at the same moment stands down, and a '
        + 'date another computer already fetched is skipped — so Amazon is never asked twice and nothing is '
        + 'saved twice.' },
    { target: '#sidenote', title: 'And if that computer is off',
      body: 'If today’s download has not happened two hours after the time, another computer with the app open '
        + 'does it instead. One silent for two days is taken over for good.' },
  ] });
  CHANGES.push({ version: '5.6.0', steps: [
    { screen: 'settings', target: '#schedulecard', title: 'A faster morning',
      body: 'The morning download now asks Amazon for up to ten reports, one after another, then collects them '
        + 'as Amazon finishes building them — about an hour for a full morning instead of several.' },
    { screen: 'sales', target: '#salesdates', title: 'Any dates you like',
      body: 'Pick From and To \u2014 27 to 28 Sep, say. The arrows move the same number of days earlier or '
        + 'later, the bar scrolls across everything downloaded, and \u201cTomorrow\u201d shows a single day.',
      missing: 'This is on the forecast screen once a forecast is downloaded.' },
  ] });
  CHANGES.push({ version: '6.0.0', steps: [
    { title: 'A new look: Ledger', body: 'The same app, redrawn: a quieter sidebar, a toolbar that stays at the top '
      + 'with the page’s name, and cards laid out the way Windows 11 lays them out. The figures are worked '
      + 'out exactly as before.' },
    { screen: 'settings', target: '#appearancecard', title: 'Light or dark',
      body: 'Choose Light, Dark, or the same as Windows. The title bar and the loading window follow.' },
    { screen: 'sales', target: '#salestiles', title: 'Each total is a switch',
      body: 'Click a total to add its line to the chart below; click again to take it off. The tick says which '
        + 'are on.', missing: 'This is on the forecast screen once a forecast is downloaded.' },
    { screen: 'sales', target: '#spendcard', title: 'Every dollar of net sales',
      body: 'One bar for every dollar: each fee, the advertising and what is left. Hover over a piece for its '
        + 'amount; the table under it has every figure.',
      missing: 'This is on the forecast screen once a forecast is downloaded.' },
    { title: 'Updates, quietly', body: 'When an update is ready, Restart and install shows its progress in the '
      + 'app, installs without an installer window, and opens Ledger again by itself.' },
  ] });
  GUIDE.splice(8, 0, { target: '#sidenote', title: 'Where your figures are kept',
    body: 'In your own Supabase project: every change is saved there, and your other computers pick it up '
      + 'within a minute. This panel says whether Supabase holds them, and when it last saved.' });
  GUIDE.splice(5, 0, { screen: 'sales', target: '#saleschart .zoombar', title: 'Zoom in',
    body: 'Drag across the chart to zoom into those dates; Ctrl + scroll zooms, the arrows move along, and a '
      + 'double-click shows everything again. Every chart here zooms the same way.',
    missing: 'This appears once a forecast is downloaded.' });
  const GUIDE_LATEST = CHANGES[CHANGES.length - 1].version;
  const verCmp = (a, b) => {
  ...
  function loadGuide() {
    try { return JSON.parse(localStorage.getItem('fba-guide-v1') || 'null'); } catch (e) { return null; }
  }
  function saveGuide(g) {
    try { localStorage.setItem('fba-guide-v1', JSON.stringify(g)); } catch (e) { /* shown again next time */ }
  }

  let tour = null;             // { kind, steps, i, home } while the guide is showing

  function guideButton() {
    return '<button class="bellbtn" id="guidebtn" aria-label="Guide" title="Guide: how to use the app">'
      + ico('info') + '</button>';
  }

  function guideSteps(kind, since) {
    if (kind === 'full') return GUIDE;
    const newer = CHANGES.filter(c => verCmp(c.version, since) > 0);
    const list = newer.length ? newer : CHANGES.slice(-1);
    const versions = list.map(c => c.version);
    return [{ title: 'What’s new in ' + versions[versions.length - 1],
      body: 'Here is what changed' + (versions.length > 1 ? ' since ' + since : '') + ', pointed out on the '
        + 'screen. It takes a moment.', full: true }]
      .concat(...list.map(c => c.steps));
  }

  function startGuide(kind, since) {
    if (state.noticeOpen) openNotices(false);
    const g = loadGuide() || {};
    tour = { kind, steps: guideSteps(kind, since || g.seen), i: 0, home: state.screen };
    showStep();
  }
  ...
  function endGuide() {
    ...
    tour = null;
    const slot = document.getElementById('tourslot');
    if (slot) slot.innerHTML = '';
    if (home && home !== state.screen) go(home);
  }

  function showStep() {
    const st = tour.steps[tour.i];
    if (st.screen && st.screen !== state.screen) go(st.screen);
    if (st.target === '#moretoggle' && !state.show.more) { /* said in its words below */ }
    setTimeout(() => { if (tour) placeTour(true); }, 60);
  }

  function placeTour(scroll) {
    let slot = document.getElementById('tourslot');
    if (!slot) {
    ...
    /* A window that is still being laid out reports no size; measuring then
       put the highlight nowhere. Try again in a moment. */
    if (!window.innerWidth || !window.innerHeight) {
      setTimeout(() => { if (tour) placeTour(scroll); }, 200);
      return;
    }
    let el = st.target ? document.querySelector(st.target) : null;
    if (el && !el.getClientRects().length) el = null;
    const inSide = el && el.closest('#side, #top');
    if (el && scroll && !inSide) {
      const r0 = el.getBoundingClientRect();
      const room = window.innerHeight - 110;
      const want = r0.height > room * 0.6 ? 110 : Math.max(110, (window.innerHeight - r0.height) / 2 - 60);
      window.scrollTo(0, Math.max(0, window.scrollY + r0.top - want));
    }
    const last = tour.i === n - 1;
    const label = tour.kind === 'full' ? 'Guide · step ' + (tour.i + 1) + ' of ' + n
      : 'What’s new · ' + (tour.i + 1) + ' of ' + n;
    const card = '<div class="tour-card" role="dialog" aria-modal="true" aria-labelledby="tourh">'
      + '<div class="tour-k">' + esc(label) + '</div>'
      + '<h3 id="tourh">' + esc(st.title) + '</h3>'
      + '<p>' + esc(st.body) + '</p>'
      + (st.target && !el && st.missing ? '<p class="tour-miss">' + ico('info', { size: 'sm' }) + esc(st.missing) + '</p>' : '')
      + (st.full ? '<p><button class="tour-link" data-tour="full">Show the whole guide instead</button></p>' : '')
      + '<div class="tour-dots" aria-hidden="true">' + tour.steps.map((x, k) => '<i' + (k === tour.i ? ' class="on"' : '')
        + '></i>').join('') + '</div>'
      + '<div class="tour-foot">' + (last ? '' : '<button class="tour-link" data-tour="skip">Skip the guide</button>')
      + '<span class="grow"></span>'
      + (tour.i ? '<button class="btn sec sm" data-tour="back">Back</button>' : '')
      + '<button class="btn sm" data-tour="next" id="tournext">' + (last ? 'Done' : 'Next') + '</button></div></div>';
    let hole = '<div class="tour-hole is-none"></div>';
    /* Drawn where the part really is - off screen if it has been scrolled
       away, never pinned to the edge as if it were there. */
    const rect = el ? el.getBoundingClientRect() : null;
    const onScreen = rect && rect.bottom > 0 && rect.top < window.innerHeight;
    if (el) {
      const pad = 6;
      hole = '<div class="tour-hole" style="top:' + (rect.top - pad) + 'px;left:' + (rect.left - pad) + 'px;width:'
        + (rect.width + pad * 2) + 'px;height:' + (rect.height + pad * 2) + 'px"></div>';
    }
    const had = slot.querySelector('.tour-card');
    slot.innerHTML = '<div class="tour-block"></div>' + hole + card;
    const c = slot.querySelector('.tour-card');
    /* beside the part: below it, above it, or to its right (the sidebar) */
    const cw = c.offsetWidth, ch = c.offsetHeight, W = window.innerWidth, H = window.innerHeight;
    let x = (W - cw) / 2, y = (H - ch) / 2;
    if (el && onScreen) {
      const r = rect;
      if (inSide && r.right + 16 + cw < W && el.closest('#side')) { x = r.right + 18; y = r.top - 10; }
      else if (H - r.bottom > ch + 24) { y = r.bottom + 14; x = r.left; }
      else if (r.top > ch + 24) { y = r.top - ch - 14; x = r.left; }
      else {
        /* Taller than the room left: the card sits at the bottom and the
           highlight stops above it, so the card never covers what it names. */
        y = H - ch - 16; x = r.left;
        const h = slot.querySelector('.tour-hole');
        if (h) h.style.height = Math.max(40, y - 12 - (r.top - 6)) + 'px';
      }
    }
    c.style.left = Math.max(12, Math.min(W - cw - 12, x)) + 'px';
    c.style.top = Math.max(12, Math.min(H - ch - 12, y)) + 'px';
    if (scroll || !had) { const b = document.getElementById('tournext'); if (b) b.focus({ preventScroll: true }); }
  }
  ...
  function tourStep(delta) {
    ...
    if (next < 0) return;
    if (next >= tour.steps.length) { endGuide(); return; }
    tour.i = next;
    showStep();
  }
  ...
  document.addEventListener('click', e => {
    ...
    if (!b) return;
    const what = b.getAttribute('data-tour');
    if (what === 'next') tourStep(1);
    else if (what === 'back') tourStep(-1);
    else if (what === 'skip') endGuide();
    else if (what === 'full') { const home = tour.home; startGuide('full'); tour.home = home; }
  });
  document.addEventListener('keydown', e => {
    if (!tour) return;
    if (e.key === 'Escape') { e.preventDefault(); e.stopImmediatePropagation(); endGuide(); }
    else if (e.key === 'ArrowRight') { e.preventDefault(); tourStep(1); }
    else if (e.key === 'ArrowLeft') { e.preventDefault(); tourStep(-1); }
  }, true);
  window.addEventListener('resize', () => { if (tour) placeTour(false); });
  window.addEventListener('scroll', () => { if (tour) placeTour(false); }, { passive: true });

  /* On start: the whole guide on a first install, what changed after an
     update, nothing otherwise. A copy that was already in use before the
     guide existed is an update, not a first install - it has files or
     notifications already. Waits for any window that is open first. */
  function guideBoot(tries) {
    if (state.setupOpen || state.getWin) {
      if ((tries || 0) < 60) setTimeout(() => guideBoot((tries || 0) + 1), 2000);
      return;
    }
    let g = loadGuide();
    if (!g) {
      let used = state.imports.length > 0 || state.previews.length > 0;
      try { used = used || !!localStorage.getItem('fba-notices-v1'); } catch (e) { /* first run */ }
      g = used ? { done: true, seen: '5.0.1' } : { done: false, seen: null };
      saveGuide(g);
    }
    if (!g.done) { startGuide('full'); return; }
    if (verCmp(g.seen, GUIDE_LATEST) < 0) startGuide('new', g.seen);
  }

  /* The toolbar turns solid, with the page's name in it, once the large
     title has scrolled up under it. */
  function watchTitle() {
    const lt = $('#largetitle'), top = $('#top');
    if (!lt || !top || typeof IntersectionObserver === 'undefined') return;
    new IntersectionObserver(es => {
      for (const x of es) top.classList.toggle('is-compact', !x.isIntersecting);
    }, { rootMargin: '-52px 0px 0px 0px', threshold: 0 }).observe(lt);
  }
  ...
  (async function boot() {
    ...
    workerBoot();
    shellBoot().then(() => {
      /* Chosen when the morning download was asked for: 6 AM, starting with
         Windows. Switched on once, here, on the computer that runs it; after
         that the switch in Settings is the person's. */
      let asked = false;
      try { asked = !!localStorage.getItem('fba-startup-set'); } catch (e) { asked = true; }
      if (!asked && scheduleCfg().enabled && isScheduleOwner() && window.desktopShell && window.desktopShell.startup) {
        try { localStorage.setItem('fba-startup-set', '1'); } catch (e) { /* this session */ }
        window.desktopShell.startup(true).then(v => { state.shell = Object.assign({}, state.shell || {}, { startup: v }); render(); });
      }
    });
    startSchedule();
    setTimeout(guideBoot, 1200);
  })();
})();
