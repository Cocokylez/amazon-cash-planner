...
(function () {
  ...
  screens.sales = function () {
    ...
    /* the totals */
    html += '<div class="grid g4">'
      + statCard('available', 'Net sales', f.netSales,
      ...
      + statCard('expenses', 'Advertising', f.advertising == null ? null : -f.advertising,
        pct(f.advertising, f.netSales) + ' of net sales', origin)
      + statCard('profit', 'After fees and advertising', f.afterFeesAndAds,
        'After fees only: ' + M.fmt(f.afterFees, { bare: true }), 'CALCULATED')
      + '</div>';

    /* week by week */
    const max = Math.max(1, ...f.weeks.map(w => w.netSales));
    html += '<section class="card"><div class="card-h">' + ico('forecast') + '<h2>Week by week</h2></div>'
      + '<div class="tscroll"><table><thead><tr><th>Week of</th><th style="width:34%">Net sales</th>'
    ...
    for (const w of f.weeks) {
      ...
      html += '<tr><td>' + esc(fmtShortDay(w.week)) + (w.days < 7 ? ' <span class="meta">(' + w.days + ' days)</span>' : '')
        + '</td><td><div style="height:10px;border-radius:5px;background:var(--mark-1);width:'
        + Math.max(2, Math.round(w.netSales / max * 100)) + '%"></div></td>'
        + '<td class="n">' + amount(w.netSales) + '</td><td class="n">' + amount(-w.fees) + '</td>'
