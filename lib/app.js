...
(function () {
  ...
  const APP_SCREENS = [
    { id: 'settings', group: 'App', name: 'Settings', title: 'Settings', short: 'Settings', icon: 'assumptions',
  ...
  function zoomBar(id, z, range) {
    ...
    const b = (k, sym, label, off) => '<button class="chip zbtn" data-czoom="' + k + '" data-chart="' + id
      + '" title="' + label + '" aria-label="' + label + '"' + (off ? ' disabled' : '') + '>' + sym + '</button>';
    return '<div class="zoombar"><span class="meta">' + (z ? 'Zoomed in: <b>' + esc(fmtShortDay(z.from)) + ' \u2013 '
        + esc(fmtShortDay(z.to)) + '</b> of ' + esc(fmtShortDay(range.from)) + ' \u2013 ' + esc(fmtShortDay(range.to))
      : 'Drag across the chart to zoom in \u00b7 Ctrl + scroll zooms \u00b7 double-click resets') + '</span>'
      + '<span class="grow"></span>'
      + b('out', '\u2212', 'Zoom out', !z) + b('in', '+', 'Zoom in', days <= 3)
      + b('left', '\u2039', 'Earlier', !z || z.from <= range.from) + b('right', '\u203a', 'Later', !z || z.to >= range.to)
      + (z ? '<button class="chip" data-czoom="reset" data-chart="' + id + '">Reset zoom</button>' : '')
      + '</div>';
  }
  ...
  function overTime(id, title, days, series, defaults, range, cur) {
    ...
    return '<div class="overtime" id="' + id + 'chart"><div class="card-h" style="margin:18px 0 4px">'
      + '<h3 style="font-size:var(--t-h3)">' + esc(title) + '</h3>'
      + '<div class="btnrow" style="gap:6px;margin:0 0 0 auto">'
      + [['running', 'Running total'], ['weekly', 'Week by week']].map(([k, n]) => '<button class="chip" data-cmode="'
        + k + '" data-chart="' + id + '" aria-pressed="' + (mode === k) + '">' + n + '</button>').join('')
      + '</div></div>'
      + '<div class="serchips" role="group" aria-label="Lines on the chart">' + series.map(s => '<button class="chip serchip" '
        + 'data-sser="' + id + '" data-key="' + esc(s.key) + '" aria-pressed="' + (shown.indexOf(s.key) >= 0)
        + '" style="--fc:' + s.colour + '" title="Show or hide on the chart"><i></i>' + esc(s.name) + '</button>').join('')
      + '</div>'
      + zoomBar(id, z, range)
  ...
  screens.sales = function () {
    ...
    if (!scoped.length) {
      return '<section class="card is-primary" style="text-align:center;padding:40px 24px">'
        + '<div style="display:flex;justify-content:center">' + ico('forecast', { size: 'lg' }) + '</div>'
        + '<h2 style="margin-top:10px">No forecast yet</h2>'
        + '<p class="meta" style="max-width:560px;margin:8px auto 18px">Get Amazon\u2019s estimate of your '
        + 'sales and fees. You pick the dates, it downloads in about a minute, and it appears here.</p>'
        + (state.workerInfo ? '<button class="btn" data-startall="1">' + ico('download', { size: 'sm' })
          + 'Get Amazon\u2019s forecast</button>' : '')
        + '<p class="meta" style="margin-top:14px">Or drop a Fees &amp; Economics Preview CSV on '
        + '<button class="linkbtn" data-go="data" style="background:none;border:0;padding:0;'
        + 'color:var(--accent);cursor:pointer;font:inherit;text-decoration:underline">Downloads</button>.</p>'
        + '</section>';
    }
    ...
    const origin = f.exact ? 'AMAZON FORECAST' : 'MODEL FORECAST';
    ...
    let html = '<section class="card" id="salesdates"><div class="daterow">'
      + '<div class="field"><label class="lbl" for="s-from">From</label>'
      + '<input type="date" id="s-from" value="' + esc(from) + '" min="' + esc(spanFrom) + '"></div>'
      + '<div class="field"><label class="lbl" for="s-to">To</label>'
      + '<input type="date" id="s-to" value="' + esc(to) + '" min="' + esc(from) + '"></div>'
      + '<div class="field"><label class="lbl">Move</label><div class="btnrow" style="gap:6px;margin:0">'
      + '<button class="chip zbtn" data-sstep="-1" title="' + stepWords + ' earlier" aria-label="' + stepWords
      + ' earlier"' + (from <= spanFrom ? ' disabled' : '') + '>\u2039</button>'
      + '<button class="chip zbtn" data-sstep="1" title="' + stepWords + ' later" aria-label="' + stepWords
      + ' later"' + (to >= spanTo ? ' disabled' : '') + '>\u203a</button></div></div></div>'
      + '<div class="btnrow" style="gap:6px;margin:12px 0 0;flex-wrap:wrap">'
      + [1, 7, 14, 30, 56, 120].map(n => '<button class="chip" data-sdays="' + n + '" aria-pressed="'
        + (from === tomorrow && CSV.addDays(state.today, n) === to) + '">'
        + (n === 1 ? 'Tomorrow' : n === 56 ? '8 weeks' : n === 120 ? '4 months' : n + ' days') + '</button>').join('')
      + '</div>'
      + (maxOff > 0 ? '<div class="scrollrow"><span class="meta">' + esc(fmtShortDay(spanFrom)) + '</span>'
        ...
        + '<span class="meta">' + esc(fmtShortDay(spanTo)) + '</span></div>' : '')
      + '<div class="meta" style="margin-top:8px">Showing <b id="s-shown">' + esc(shownDates) + '</b> \u00b7 '
      + len + (len === 1 ? ' day' : ' days')
      + ' \u00b7 ' + (at => at ? 'downloaded ' + esc(ago(at)) : 'imported from a file')(lastForecastAt())
      + (f.store ? ' \u00b7 ' + esc(f.store) : '') + '</div>';
    if (f.coveredDays < f.days) {
      html += '<div class="note is-warn" style="margin-top:12px">' + ico('missing') + '<div><b>'
        ...
        + 'The figures below cover the other ' + f.coveredDays + ' days only.'
        + (state.workerInfo ? '<div class="btnrow" style="margin-top:10px"><button class="btn sm" data-sget="1">'
          + ico('download', { size: 'sm' }) + 'Get Amazon\u2019s forecast for ' + esc(fmtShortDay(from))
          + ' \u2013 ' + esc(fmtShortDay(to)) + '</button></div>' : '')
        + '</div></div>';
    } else if (!f.exact) {
      html += '<div class="meta" style="margin-top:8px">These dates are part of a longer download, so its '
        + 'totals are split evenly across its days. For Amazon\u2019s own figures for exactly these dates, '
        + (state.workerInfo ? '<button class="linkbtn" data-sget="1" style="background:none;border:0;padding:0;'
          + 'color:var(--accent);cursor:pointer;font:inherit;text-decoration:underline">download them</button>.'
          : 'download them.') + '</div>';
    }
    html += '</section>';
    ...
    const pick = (key, label, cents, meta, org) => {
      const on = metrics.indexOf(key) >= 0;
      return '<button type="button" class="tile tile-btn" data-smetric="' + key + '" aria-pressed="' + on
        + '" style="--fc:' + Charts.MEASURES.find(x => x.key === key).colour + '" title="'
        + (on ? 'Take ' + esc(label.toLowerCase()) + ' off the chart' : 'Add ' + esc(label.toLowerCase())
          + ' to the chart') + '"><span class="tile-l"><i class="fc-key"></i>' + esc(label)
        + '<span class="tile-on">' + (on ? 'On the chart' : 'Add to chart') + '</span></span>'
        + (cents == null ? '<span style="margin-top:6px">' + unavailable(null, { size: 'sm' }) + '</span>'
          : '<span class="tile-v fig">' + money(cents) + '</span>'
          + '<span class="tile-m">' + tag(org) + (meta ? '<span style="margin-left:8px">' + esc(meta) + '</span>' : '')
          + '</span>')
        + '</button>';
    };
    html += '<div class="grid g4" id="salestiles">'
      + pick('netSales', 'Net sales', f.netSales,
        (f.sales != null ? 'Sales ' + M.fmt(f.sales) + ' before returns' : '')
        + (f.units != null ? ' \u00b7 ' + f.units.toLocaleString() + ' units' : ''), origin)
      + pick('fees', 'Amazon fees', f.feeTotal == null ? null : -f.feeTotal,
        pct(f.feeTotal, f.netSales) + ' of net sales', origin)
      + pick('advertising', 'Advertising', f.advertising == null ? null : -f.advertising,
        pct(f.advertising, f.netSales) + ' of net sales', origin)
      + pick('after', 'After fees and advertising', f.afterFeesAndAds,
        'After fees only: ' + M.fmt(f.afterFees), 'CALCULATED')
      + '</div>';
    ...
    html += '<section class="card" id="saleschart"><div class="card-h">' + ico('forecast') + '<h2>'
      + esc(names.length > 1 ? names.slice(0, -1).join(', ') + ' and ' + names[names.length - 1] : names[0])
      + (mode === 'weekly' ? ', week by week' : ', running total') + '</h2>'
      + '<div class="btnrow" style="gap:6px;margin:0 0 0 auto">'
      + '<button class="chip" data-sall="1" aria-pressed="' + all4 + '">'
      + (all4 ? 'Just net sales' : 'Compare all four') + '</button>'
      + [['running', 'Running total'], ['weekly', 'Week by week']].map(([k, n]) => '<button class="chip" '
        + 'data-smode="' + k + '" aria-pressed="' + (mode === k) + '">' + n + '</button>').join('')
      + '</div></div>'
    ...
    const bar = (v, c) => '<div class="spbar"><i style="width:' + (f.netSales > 0
      ? Math.min(100, Math.max(0.5, v / f.netSales * 100)).toFixed(1) : 0) + '%;background:' + c + '"></i></div>';
    ...
    html += '<section class="card" id="spendcard"><div class="card-h">' + ico('expenses')
      + '<h2>Where your net sales go</h2><span class="meta" style="margin-left:auto">'
      + esc(M.fmt(f.netSales, { currency: cur })) + ' net sales</span></div>'
      + '<div class="sub">Each fee and the advertising, as a share of net sales, and what is left. '
      + 'Hover over the bar for the amounts.</div>'
      + Charts.spend([{ label: 'All products', netSales: f.netSales, parts: partsFor(allFees, f.advertising) }],
        { currency: cur, share: true, ariaLabel: 'Where net sales go' })
      + overTime('spend', 'Net sales and each fee, over time', Simple.seriesFor(f, null), lineSeries, lineDefault,
        range, cur)
      + '<div class="tscroll" style="margin-top:14px"><table><thead><tr><th>What</th><th class="n">Amount</th>'
      + '<th class="n">Of net sales</th><th style="width:30%"></th></tr></thead><tbody>';
    for (const x of feeList) {
      html += '<tr><td><i class="dotkey" style="background:' + colourOf(x.name) + '"></i>' + esc(x.name)
        + '</td><td class="n">' + amount(-x.amount) + '</td><td class="n">' + pct(x.amount, f.netSales)
        + '</td><td>' + bar(x.amount, colourOf(x.name)) + '</td></tr>';
    }
    html += '<tr><td><b>All Amazon fees</b></td><td class="n"><b>' + amount(f.feeTotal == null ? null : -f.feeTotal)
      + '</b></td><td class="n"><b>' + pct(f.feeTotal, f.netSales) + '</b></td><td>'
      + bar(f.feeTotal || 0, 'var(--mark-2)') + '</td></tr>'
      + '<tr><td><i class="dotkey" style="background:' + Charts.feeColour('Advertising') + '"></i>'
      + 'Advertising (Sponsored Products)</td><td class="n">' + amount(f.advertising == null ? null : -f.advertising)
      + '</td><td class="n">' + pct(f.advertising, f.netSales) + '</td><td>'
      + bar(f.advertising || 0, Charts.feeColour('Advertising')) + '</td></tr>'
      + '<tr><td><i class="dotkey" style="background:' + Charts.feeColour('Left after fees and ads') + '"></i>'
      + '<b>Left after fees and ads</b></td><td class="n"><b>' + amount(left) + '</b></td><td class="n"><b>'
      + pct(left, f.netSales) + '</b></td><td>' + bar(Math.max(0, left || 0), Charts.feeColour('Left after fees and ads'))
    ...
    const chip = (attr, val, on, label) => '<button class="chip" ' + attr + '="' + val + '" aria-pressed="' + on + '">'
      + label + '</button>';
    html += '<section class="card" id="skuspend"><div class="card-h">' + ico('profit')
      + '<h2>Costs by product</h2><span class="meta" style="margin-left:auto">' + ranked.length + ' products'
      + (losing ? ' \u00b7 <b style="color:var(--error)">' + losing + ' cost more than they sell</b>' : '')
      + '</span></div>'
      + '<div class="sub">Each product\u2019s Amazon fees and advertising laid along its own net sales, '
      + 'so you can see what takes the most from each one. Hover over a bar for the detail.</div>'
      + '<div class="btnrow" style="gap:6px;margin:0 0 12px;flex-wrap:wrap">'
      + '<span class="meta" style="align-self:center">Sort</span>'
      + chip('data-ssort', 'cost', sort === 'cost', 'Biggest costs')
      + chip('data-ssort', 'pct', sort === 'pct', 'Highest % of sales')
      + chip('data-ssort', 'sales', sort === 'sales', 'Biggest sales')
      + '<span class="meta" style="align-self:center;margin-left:12px">Bars</span>'
      + chip('data-sshare', '1', share, 'Share of its sales')
      + chip('data-sshare', '0', !share, 'Amounts')
      + '</div>'
      + (pickSku ? '<div class="skupick"><label class="lbl" for="skupick">Product</label><select id="skupick">'
        ...
        + '</select><span class="meta">or click a product’s bar below</span></div>'
        + overTime('sku', pickSku + ', over time', Simple.seriesFor(f, pickSku), lineSeries, lineDefault, range, cur)
        : '')
      + '<div class="legend" style="margin:18px 0 10px">' + order.concat(['Advertising', 'Left after fees and ads'])
        .map(n => '<span><i class="key" style="background:' + colourOf(n) + '"></i>' + esc(n) + '</span>').join('')
      + '</div>'
      + Charts.spend(ranked.slice(0, showN).map(r => ({ label: r.msku, sub: r.asin, netSales: r.netSales,
        parts: partsFor(r.feeBy || {}, r.advertising) })), { currency: cur, share, label: true,
        pickable: true, picked: pickSku, ariaLabel: 'Costs by product' })
      + (ranked.length > 20 ? '<div class="btnrow" style="margin-top:10px">' + chip('data-sskuall', state.skuAll ? '0' : '1',
        false, state.skuAll ? 'Show the top 20' : 'Show all ' + ranked.length) + '</div>' : '')
      + '</section>';

    /* week by week */
    html += '<section class="card"><div class="card-h">' + ico('forecast') + '<h2>Week by week</h2></div>'
      + '<div class="tscroll"><table><thead><tr><th>Week of</th>'
      + '<th class="n">Net sales</th><th class="n">Amazon fees</th><th class="n">Advertising</th>'
      + '<th class="n">After fees &amp; ads</th></tr></thead><tbody>';
    for (const w of f.weeks) {
      const after = w.netSales - w.fees - w.advertising;
      html += '<tr><td>' + esc(fmtShortDay(w.week)) + (w.days < 7 ? ' <span class="meta">(' + w.days + ' days)</span>' : '')
        + '</td><td class="n">' + amount(w.netSales) + '</td><td class="n">' + amount(-w.fees) + '</td>'
        + '<td class="n">' + amount(-w.advertising) + '</td><td class="n"><b>' + amount(after) + '</b></td></tr>';
    }
    html += '</tbody></table></div></section>';

    /* the products, as a table */
    const shown = f.bySku.slice(0, 150);
    html += '<section class="card"><div class="card-h">' + ico('profit') + '<h2>By product</h2>'
      + '<span class="meta" style="margin-left:auto">' + f.bySku.length + ' products</span></div>'
      + '<div class="tscroll"><table><thead><tr><th>Product</th><th class="n">Units</th>'
      + '<th class="n">Net sales</th><th class="n">Amazon fees</th><th>Biggest fee</th><th class="n">Advertising</th>'
      + '<th class="n">Costs</th><th class="n">After fees &amp; ads</th></tr></thead><tbody>';
    for (const r of shown) {
      const top = Object.entries(r.feeBy || {}).sort((x, y) => y[1] - x[1])[0];
      html += '<tr><td class="wrap">' + esc(r.msku) + (r.asin ? '<div class="meta">' + esc(r.asin) + '</div>' : '')
        + '</td><td class="n">' + (r.units || 0).toLocaleString() + '</td>'
        + '<td class="n">' + amount(r.netSales) + '</td>'
        + '<td class="n">' + amount(r.fees == null ? null : -r.fees) + '</td>'
        + '<td class="wrap">' + (top && top[1] ? '<i class="dotkey" style="background:' + colourOf(top[0]) + '"></i>'
          + esc(top[0]) + ' <span class="meta">' + pct(top[1], r.netSales) + '</span>' : '<span class="meta">\u2014</span>') + '</td>'
        + '<td class="n">' + amount(r.advertising == null ? null : -r.advertising) + '</td>'
        + '<td class="n">' + pct(costOf(r), r.netSales) + '</td>'
        + '<td class="n"><b>' + amount(r.net) + '</b></td></tr>';
    }
    html += '</tbody></table></div>'
      + (f.bySku.length > shown.length ? '<div class="meta" style="margin-top:8px">The top '
        + shown.length + ' of ' + f.bySku.length + ' by net sales.</div>' : '')
      + '<div class="meta" style="margin-top:8px">Amazon fees here are every fee Amazon forecasts for the '
  ...
  screens.settings = function () {
    ...
    let html = morningCard + '<section class="card"><div class="card-h">' + ico('assumptions') + '<h2>What to show</h2></div>'
  ...
  function noticeUpdate(info) {
    ...
    notify('info', 'Version ' + u.readyVersion + ' is ready to install',
      'Use the green bar at the top. It installs in about a minute and the app reopens.', { toast: true });
  }
    ...
    if (iu && window.desktopShell && window.desktopShell.install) {
      ...
      window.desktopShell.install()
        .then(r => {
          if (r && r.ok === false) {
            iu.disabled = false;
            iu.textContent = 'Try again';
            iu.title = r.detail || '';
            alert(r.detail || 'The update could not be installed yet.');
          }
        })
        .catch(() => render());
      return;
  ...
  const GUIDE = [
    { title: 'Welcome to Amazon Cash Planner',
