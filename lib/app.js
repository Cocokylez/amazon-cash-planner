...
(function () {
  ...
  screens.sales = function () {
    ...
    const probe = Simple.forecastFor(scoped, from, from);
    const loadedTo = probe.loadedTo;
    ...
    html += '<section class="card" id="saleschart"><div class="card-h">' + ico('forecast') + '<h2>'
      ...
      + '</div></div>'
      + Charts.forecast(f.daily, { metrics, mode, currency: cur, mondayOf: Simple.mondayOf })
    ...
    html += '<section class="card" id="skuspend"><div class="card-h">' + ico('profit')
      ...
      + '</div>'
      + '<div class="legend" style="margin:0 0 10px">' + order.concat(['Advertising', 'Left after fees and ads'])
  ...
  document.addEventListener('click', e => {
    ...
    if (fch) { Charts.forecastHover(fch, Charts.forecastAt(fch, e.clientX)); return; }
  ...
  document.addEventListener('mouseout', e => {
    ...
    const c = e.target.closest && e.target.closest('.fchart');
    if (c && !(e.relatedTarget && c.contains(e.relatedTarget)) && document.activeElement !== c) {
  ...
  document.addEventListener('keydown', e => {
    if (e.target && e.target.classList && e.target.classList.contains('fchart')
        && Charts.forecastStep(e.target, e.key)) { e.preventDefault(); return; }
