...
(function () {
  ...
  const hostName = () => (state.shell && state.shell.hostname) || 'this computer';
  ...
  function scheduleTick() {
    if (scheduleBusy || !state.workerInfo) return;
    const cfg = scheduleCfg();
    if (!cfg.enabled || !isScheduleOwner()) return;
    if (Schedule.isDue(localNowParts(), cfg, cfg.lastRunDate)) runSchedule('schedule');
  }
  ...
  function scheduleCard() {
    ...
    if (run) {
      ...
      const cls = run.status === 'done' ? 'is-ok' : /waiting|stopped|partial|interrupted/.test(run.status) ? 'is-warn' : '';
      const words = {
