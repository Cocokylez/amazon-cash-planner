...
(function () {
  ...
  function forecastStatus() {
    const f = Simple.forecastFor(scopedPreviews(), horizonStart(), horizonEnd());
  ...
  function salesRange() {
    ...
    const loadedTo = Simple.forecastFor(scopedPreviews(), from, from).loadedTo;
  ...
  screens.sales = function () {
    ...
    const range = salesRange();
    const { from, to } = range;
    const f = Simple.forecastFor(scoped, from, to);
    ...
    let html = '<section class="card" id="salesdates"><div class="frow f3" style="align-items:end">'
      ...
      + [7, 14, 30, 56].map(n => '<button class="chip" data-sdays="' + n + '"'
        + (CSV.addDays(state.today, n) === to ? ' aria-pressed="true"' : '') + '>'
        + (n === 56 ? '8 weeks' : n + ' days') + '</button>').join('')
      + '</div></div></div>'
  ...
  screens.settings = function () {
    ...
    let html = '<section class="card"><div class="card-h">' + ico('assumptions') + '<h2>What to show</h2></div>'
  ...
  function buildBrief() {
    ...
    /* The forecast first: it is what the app is for now. */
    {
      ...
      const f = Simple.forecastFor(scopedPreviews(), from, to);
  ...
  function render() {
    ...
    const ub = $('#updatebar');
    if (ub) ub.innerHTML = updateBar() + cloudBar();
