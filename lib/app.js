...
(function () {
  ...
  const SCREENS = [
    { id: 'dashboard', name: 'Cash Dashboard', title: 'Your Amazon cash outlook', short: 'Cash',
      icon: 'dashboard',
      blurb: 'How much you could request from Amazon right now, and what is left afterwards.' },
    { id: 'forecast', name: 'Payout Forecast', title: 'Payout forecast', short: 'Forecast',
      icon: 'forecast',
      blurb: 'What Amazon expects to owe you over the next eight weeks, and when it should reach '
        + 'your bank.' },
    { id: 'expenses', name: 'Amazon Expenses', title: 'Amazon expenses', short: 'Expenses',
      icon: 'expenses',
      blurb: 'Every fee Amazon has actually charged you, grouped by what it was for.' },
    { id: 'profit', name: 'Profitability', title: 'Profitability', short: 'Profit', icon: 'profit',
      blurb: 'What you kept after the costs recorded here \u2014 by period, and by product.' },
    { id: 'plan', name: 'Cash Plan', title: 'Cash plan', short: 'Plan', icon: 'plan',
      blurb: 'What is safe to spend once your bills and buffer are accounted for.' },
    { id: 'recon', name: 'Reconciliation', title: 'Reconciliation', short: 'Reconcile',
      icon: 'recon',
      blurb: 'Checks that prove the numbers tie back to Amazon\u2019s own files.' },
    { id: 'data', name: 'Data & Assumptions', title: 'Data & assumptions', short: 'Data',
      icon: 'data',
      blurb: 'Import your Amazon reports, and see exactly what each figure is based on.' },
  ];
  ...
  function previewCoverage() {
    ...
    return Preview.coverage(scopedPreviews(), state.today, CSV.addDays(state.today, 55));
  }
  ...
  function rangeControl() {
    const f = state.filters;
    const active = f.preset || (f.from || f.to ? 'custom' : 'all');
    let html = '<div class="rangebar"><div class="rangebar-in">'
      + '<span class="rangebar-l">' + ico('asOf', { size: 'sm' }) + 'Reporting period</span>'
      + '<div class="seg range-seg">'
      + '<button data-preset="all"' + (active === 'all' ? ' aria-current="true"' : '')
      + '>All dates</button>';
    for (const p of RANGE_PRESETS) {
      html += '<button data-preset="' + p.id + '"'
        + (active === p.id ? ' aria-current="true"' : '') + '>' + esc(p.label) + '</button>';
    }
    html += '</div>'
  ...
  function dateBasisNote() {
    ...
    if (!t) return '';
    return '<div class="meta datebasis">' + ico('info', { size: 'sm' }) + ' ' + t + '</div>';
  }
  ...
  function readinessPanel(opts) {
    ...
    let html = '<section class="card"><div class="card-h">' + ico('assumptions')
      + '<h2>What is ready, and what is not</h2></div>';
  ...
  function buildForecastUncached(account, scenario) {
    const from = state.today, to = CSV.addDays(from, 55);
    ...
    return Forecast.build({
      ...
      advertisingPaymentMethod: state.advertisingBilling.length ? state.advertisingBilling[0] : null,
  ...
  screens.dashboard = function () {
    ...
    /* What the imported files already support, first — before anything that
       is waiting on an input the person has not supplied. */
    if (ds.forecast.present) {
      const f = ds.forecast;
      html += '<section class="card"><div class="card-h">' + ico('forecast')
        + '<h2>Amazon\u2019s forecast for ' + esc(fmtDay(f.period.from)) + ' – '
        + esc(fmtDay(f.period.to)) + '</h2></div>'
        + '<div class="sub">Read from your Fees &amp; Economics Preview. These are Amazon\u2019s '
        + 'own estimates of what you will sell and be charged — not money you have, and not a '
        + 'payout.</div>'
        + partialPeriodNote(f)
        + forecastSummaryCards(f);
      html += '<div class="bridge" style="margin-top:20px">'
        + '<div class="bridge"><ul class="bridge">'
        + '<li><span class="l">Forecast net sales</span><span class="v">'
        + esc(M.fmt(f.netSales)) + '</span></li>'
        + '<li><span class="l">Less order-related Amazon fees</span><span class="v">'
        + esc(M.fmt(-(f.orderFees || 0))) + '</span></li>'
        + '<li class="total"><span class="l">Forecast net receivable</span><span class="v">'
        + esc(M.fmt(f.netReceivable)) + '</span></li>'
        + '<li class="ctx"><span class="l">Storage and advertising are period charges and are '
        + 'not part of this per-order figure</span><span class="v"></span></li>'
        + '</ul></div></div>';
      html += '<div class="btnrow" style="margin-top:16px">'
        + '<button class="btn sec" data-go="forecast">' + ico('forecast', { size: 'sm' })
        + 'See the full forecast</button>'
        + '<button class="btn sec" data-go="expenses">' + ico('expenses', { size: 'sm' })
        + 'See the fee breakdown</button></div>';
      html += '</section>';
    }

    if (ds.actual.present) {
      const a = ds.actual;
      html += '<section class="card"><div class="card-h">' + ico('verified')
        + '<h2>What actually happened, ' + esc(fmtDay(a.period.from)) + ' – '
        + esc(fmtDay(a.period.to)) + '</h2></div>'
        + '<div class="sub">From your Payments transaction export.</div>'
        + '<div class="grid g4">'
        + statCard('available', 'Net revenue', a.netRevenue, null, 'ACTUAL')
        + statCard('expenses', 'Amazon charges', -a.grossCharges, null, 'ACTUAL')
        + statCard('verified', 'Credits received', a.credits, null, 'ACTUAL')
        + statCard('profit', 'Net platform cost', -a.netCost, null, 'CALCULATED')
        + '</div>'
        + '<div class="btnrow" style="margin-top:16px">'
        + '<button class="btn sec" data-go="expenses">' + ico('expenses', { size: 'sm' })
        + 'Break this down</button></div></section>';
    }

    /* ── lead card: how much could we request? ── */
    html += '<section class="card" aria-labelledby="leadh">';
    html += '<div class="card-h">' + ico('available') + '<h2 id="leadh">How much could we request?</h2></div>';

    if (err || !br) {
      /* One sentence, not seven boxes. The form that fixes it is directly
         below, and the full checklist is at the foot of the page. */
      html += '<div class="sub">This is the one figure that needs your current Amazon balance. '
        + 'Everything above comes from your imported reports and is unaffected.</div>';
      html += '<div class="note is-warn" style="margin-bottom:0">' + ico('missing') + '<div>'
        + '<b>Enter your balances below and this fills in</b> — along with what is left after a '
        + 'payout, and when the next request would be allowed. Read them from Seller Central '
        + '→ Payments, all at the same moment.</div></div>';
      html += '</section>';
    } else {
      html += '<div class="sub">A planning view. Requesting earlier changes when cash arrives — it does '
        + 'not create sales, does not release held funds sooner, and leaves less for the next payout. '
        + 'Nothing here is sent to Amazon.</div>';

      html += '<div class="frow f3">'
        + '<div class="field"><label class="lbl" for="reqdate">Payout request date</label>'
        + '<input type="date" id="reqdate" value="' + esc(reqDate) + '" min="' + esc(state.today) + '"></div>'
        + '<div class="field"><label class="lbl" for="acctsel">Account stream</label>'
        + '<select id="acctsel">' + accountOptions(account) + '</select></div>'
        + '<div class="field"><label class="lbl" for="modesel">Request mode</label>'
        + '<select id="modesel"><option value="all_eligible">All eligible funds</option>'
        + '<option value="partial">A set amount</option></select></div>'
        + '</div>';

      html += '<div class="grid g2" style="align-items:start">';

      html += '<div>';
      html += '<div class="stat-l">' + ico('available') + 'Estimated eligible on '
        + esc(fmtDay(reqDate)) + '</div>';
      html += '<div class="hero fig">' + esc(M.fmt(br.eligible)) + '</div>';
      html += '<div style="margin:10px 0 0">' + tag('MODEL FORECAST')
        + ' <span class="meta">Based on your recorded balance, projected forward to this '
        + 'date.</span></div>';
      const scen = scenarioRange(account, reqDate);
      if (scen) html += scenarioBlock(scen);

      if (forecast && forecast.economicsOnly) {
        html += '<div class="note is-warn">' + ico('missing') + '<div>'
          + '<b>This figure contains no forecast activity.</b> It walks your recorded balance '
          + 'forward with nothing added and nothing taken away, because release timing has not '
          + 'been measured yet. Your imported forecast is intact and shown above; applying its '
          + 'charges without its receipts would read lower than the truth.</div></div>';
      }
      if (br.blocked) {
        html += '<div class="note is-error">' + ico('missing') + '<div>' + esc(br.blocked) + '</div></div>';
      }
      for (const w of br.warnings || []) {
        html += '<div class="note is-warn">' + ico('missing') + '<div>' + esc(w) + '</div></div>';
      }
      html += '<div class="btnrow" style="margin-top:16px">'
        + '<button class="btn sec" id="gocompare">' + ico('compare', { size: 'sm' })
        + 'Compare request dates</button></div>';
      html += '</div>';

      /* Surrounding facts. Rows whose input is missing are listed once at the
         bottom as a single line rather than each carrying its own panel. */
      const receipt = br.expectedBankReceipt;
      const transitMeasured = state.bankDeposits.length > 0;
      const pending = [];
      html += '<div><dl class="dl">';
      html += '<dt>' + ico('forecast') + 'Transfer initiated</dt><dd>'
        + esc(fmtDay(reqDate)) + '</dd>';
      if (receipt && receipt.date) {
        html += '<dt>' + ico('bankReceipt') + 'Expected bank arrival</dt><dd>'
          + esc(fmtDay(receipt.low))
          + (receipt.high !== receipt.low ? ' – ' + esc(fmtDay(receipt.high)) : '')
          + '<div style="margin-top:4px">' + tag(transitMeasured ? 'ACTUAL' : 'ASSUMPTION')
          + '</div></dd>';
      } else pending.push('when it reaches your bank (needs deposit history)');

      html += '<dt>' + ico('deferred') + 'Still deferred</dt><dd>'
        + amount(balance.deferred, { missing: 'a current deferred balance' }) + '</dd>';
      html += '<dt>' + ico('inTransit') + 'Already in transit</dt><dd>'
        + amount(balance.inTransit != null ? balance.inTransit : null,
          { missing: 'transfers already initiated' }) + '</dd>';
      html += '<dt>' + ico('available') + 'Left for the next payout</dt><dd>'
        + amount(run.ending.available) + '</dd>';
      if (state.policy.cooldownDays != null) {
        html += '<dt>' + ico('forecast') + 'Next request allowed</dt><dd>'
          + esc('After ' + state.policy.cooldownDays + ' days') + '</dd>';
      } else pending.push('when the next request is allowed (needs the account\u2019s rules)');
      html += '</dl>';

      if (pending.length) {
        html += '<div class="meta" style="margin-top:12px">' + ico('info', { size: 'sm' })
          + ' Not shown yet: ' + esc(pending.join('; ')) + '.</div>';
      }
      html += '</div>';
      html += '</div>';
      html += '</section>';
    ...
    if (!balance) html += balanceEntryCard(account);

    /* ── four supporting stat cards ── */
    if (balance) {
      html += '<div class="grid g4">';
      html += statCard('available', 'Available now', balance.available,
        'a current available balance', 'CURRENT');
      html += statCard('deferred', 'Currently deferred', balance.deferred,
        'a current deferred balance', 'CURRENT');
      html += statCard('inTransit', 'Transfers in transit',
        balance.inTransit != null ? balance.inTransit : null,
        'transfers already initiated', 'CURRENT');
      html += statCard('bankReceipt', 'Forecast bank receipts, 8 weeks',
        run && run.ending.bank ? run.ending.bank : null,
        'bank deposit history to date arrivals', 'MODEL FORECAST');
      html += '</div>';
    }

    /* ── bank receipts chart ── */
    if (run && bankWeeks(run).some(w => w.amount != null)) {
      html += '<section class="card"><div class="card-h">' + ico('bankReceipt')
        + '<h2>When cash should reach the bank</h2></div>'
        + '<div class="sub">Eight weeks of expected bank receipts, grouped by the week they land.</div>'
        + Charts.bankReceipts(bankWeeks(run))
        + '</section>';
    }

    /* ── how it is built: explanation, so it folds away by default ── */
    if (br) html += '<details><summary>' + ico('profit', { size: 'sm' })
      + 'How this estimate is built, step by step</summary>'
      + '<p class="meta" style="margin:12px 0">Every step below is a movement in the event '
      + 'ledger, not a separate estimate.</p>'
      + Charts.waterfall(waterfallSteps(br))
      + (br ? bridgeList(br) : '')
      + '</details>';
    ...
    /* One checklist, at the end, instead of a yellow box beside every figure. */
    html += readinessPanel();
    html += tagLegend(['CURRENT', 'ACTUAL', 'AMAZON FORECAST', 'MODEL FORECAST', 'CALCULATED']);

    return html;
  };

  /* A stat card whose value is a count or a label rather than money. */
  function statText(icon, label, text, origin) {
    return '<div class="card" style="margin:0">'
      + '<div class="stat-l">' + ico(icon) + esc(label) + '</div>'
      + '<div class="stat-v fig">' + esc(text) + '</div>'
      + (origin ? '<div style="margin-top:8px">' + tag(origin) + '</div>' : '')
      + '</div>';
  }
  ...
  function statCard(icon, label, cents, missing, origin) {
    return '<div class="card" style="margin:0">'
      + '<div class="stat-l">' + ico(icon) + esc(label) + '</div>'
      + (cents == null ? unavailable(missing, { size: 'sm' })
        : '<div class="stat-v fig">' + esc(M.fmt(cents)) + '</div>'
        + (origin ? '<div style="margin-top:8px">' + tag(origin) + '</div>' : ''))
      + '</div>';
  }
  ...
  function bankWeeks(run) {
    const weeks = [];
    let d = state.today;
    const end = CSV.addDays(state.today, 55);
    while (d <= end) {
  ...
  function comparisonCard(account, reqDate) {
    let html = '<section class="card" id="comparecard"><div class="card-h">' + ico('compare')
      + '<h2>Request early, or wait for the normal schedule</h2></div>'
      + '<div class="sub">Identical sales, fees and release assumptions on both sides. Only the '
      + 'withdrawal decision changes.</div>';

    const sched = Cash.scheduledPayoutDates(state.policy, state.today, CSV.addDays(state.today, 55));
    if (!sched.dates) {
    ...
    const cmp = Cash.comparePolicies({
      ...
      endpoint: CSV.addDays(state.today, 55),
    });
    ...
    html += '<div class="tscroll" style="margin-top:20px"><table><thead><tr>'
      + '<th>At ' + esc(fmtDay(CSV.addDays(state.today, 55))) + '</th>'
    ...
    html += plannedRequestsBlock(account);
    return html + '</section>';
  }

  function plannedRequestsBlock(account) {
    let html = '<h3 style="font-size:15px;margin:24px 0 8px">Your planned requests</h3>'
      + '<div class="sub">Each request takes what is eligible on its date and leaves the rest. '
      + 'Adding one earlier does not add money — it moves it forward.</div>';
    const plans = state.requestPlans.filter(p => !p.account || p.account === account)
      .sort((a, b) => a.date < b.date ? -1 : 1);

    if (!plans.length) {
      html += '<div class="note">' + ico('info') + '<div>No requests planned. The comparison above '
        + 'uses the single date chosen at the top.</div></div>';
    } else {
      let run = null;
      try {
  ...
  function balanceEntryCard(account) {
    return '<section class="card"><div class="card-h">' + ico('available')
      + '<h2>Record your current Amazon balances</h2></div>'
      ...
      + '<button class="btn" id="savebalance">Save this snapshot</button></section>';
  }
  ...
  screens.forecast = function () {
    ...
    const fc = currentForecast(account, 'base');
    const { run } = runFor(account, state.requestDate || CSV.addDays(state.today, 1));
    let html = '';
    ...
    if (fc.economicsOnly) {
      ...
      html += '<section class="card"><div class="card-h">' + ico('bankReceipt')
        + '<h2>Expected bank receipts</h2></div>'
        + '<div class="sub">' + esc(fmtDay(fc.horizon.from)) + ' to ' + esc(fmtDay(fc.horizon.to))
        + ' · run ' + esc(fc.runId) + ' · information cutoff ' + esc(fc.knownAt.slice(0, 10)) + '</div>'
        + Charts.bankReceipts(bankWeeks(run)) + '</section>';
    ...
    if (fc.limitations.length) {
      html += '<section class="card"><div class="card-h">' + ico('missing')
        + '<h2>What limits this forecast</h2></div><ul class="bridge">';
    ...
    for (const n of fc.notes) {
      html += '<div class="note is-warn">' + ico('missing') + '<div>' + esc(n.message) + '</div></div>';
    }

    html += assumptionsCard(fc) + dailyEconomicsDetails(fc);
    html += tagLegend(['AMAZON FORECAST', 'MODEL FORECAST', 'CALCULATED', 'ASSUMPTION']);
    return html;
  };
  ...
  function economicsCards(fc) {
    ...
    let html = '<div class="note is-warn">' + ico('missing') + '<div><b>Your preview imported '
      + 'cleanly and its figures are below.</b> The dated payout schedule is the one part that '
      + 'is still missing: it needs the Payments “Date range transaction” CSV, which is where '
      + 'the posted-to-release lag and the standard/invoiced split are measured. Nothing is '
      + 'assumed in their place.'
      + '<div class="btnrow" style="margin-top:12px">'
      + '<button class="btn" data-go="data">' + ico('upload', { size: 'sm' })
      + 'Import the transaction CSV</button></div></div></div>';
  ...
  function weekByWeek(fc, run) {
    let html = '<section class="card"><div class="card-h">' + ico('forecast')
      + '<h2>Week by week</h2></div>';
  ...
  function assumptionsCard(fc) {
    let html = '<details><summary>' + ico('assumptions', { size: 'sm' })
      + 'Assumptions in force (' + fc.assumptions.length + ')</summary>'
      + '<p class="meta" style="margin:12px 0">Each one says where it came from. Nothing here '
  ...
  function dailyEconomicsDetails(fc) {
    let html = '<details><summary>' + ico('info', { size: 'sm' })
      + 'Daily economic estimate behind the forecast</summary>'
  ...
  screens.expenses = function () {
    ...
    let html = periodFilter();

    html += '<div class="grid g4">';
    html += statCard('expenses', 'Gross platform charges', gross, null, 'ACTUAL');
    html += statCard('verified', 'Platform credits', credits, null, 'ACTUAL');
    html += statCard('expenses', 'Net platform cost', -(gross - credits), null, 'CALCULATED');
    html += statCard('missing', 'Unresolved debits', unresolvedD, null, 'ACTUAL');
    html += '</div>';
    ...
    html += '<section class="card"><div class="card-h">' + ico('expenses')
      + '<h2>Where the money went</h2></div>'
      + '<div class="sub">Net cost by category — charges less credits — on a posted-date basis.</div>'
      + Charts.categoryBars(bars) + '</section>';

    html += '<section class="card"><div class="card-h">' + ico('expenses')
      + '<h2>Category, subcategory and source</h2></div>'
      + '<div class="sub">Charges and credits are never netted into one bar: a reversal is not the '
      + 'absence of a charge. Expand a category to see its subcategories.</div>';
    html += '<div class="tscroll"><table><thead><tr><th>Category</th>'
    ...
    html += '<details><summary>' + ico('info', { size: 'sm' })
      + 'Fee types this source cannot show</summary>'
      + '<p class="meta">The difference between “we are not charged this” and “we cannot see it” '
      + 'matters, so the second case is listed rather than left blank.</p>'
    ...
    if (zeroAds.length) {
      html += '<div class="note is-warn">' + ico('missing') + '<div><b>Advertising is missing from '
        + 'the settlement in ' + zeroAds.map(m => esc(m.month)).join(', ') + '.</b> '
        + 'That is an absence of a deduction in this export, not an absence of advertising cost. '
        + 'Supply the billing records so the expense lands in the right place in both engines.</div></div>';
    }
    if (ds.forecast.present) {
  ...
  function periodFilter() {
    const f = state.filters;
    const range = state.ledger ? state.ledger.dateRange() : { from: null, to: null };
    return '<section class="card"><div class="frow f3">'
      + '<div class="field" style="margin:0"><label class="lbl" for="f-acct">Account stream</label>'
      + '<select id="f-acct"><option value="">Both streams</option>'
      + accountOptions(f.account) + '</select></div></div>'
      + '<div class="meta" style="margin-top:12px">Source covers ' + esc(range.from || '—') + ' to '
      + esc(range.to || '—') + '. Streams are never combined into one payout schedule.</div></section>'
      + filterHidesDataNotice();
  }
  ...
  screens.profit = function () {
    ...
    if (!ds.actual.present) {
      const f = ds.forecast;
      const costed = new Map(state.productCosts.map(c => [c.sku, c]));
    ...
    const tab = state.tab || 'statement';
    let html = '<div class="subtabs" role="tablist">'
      + '<button data-tab="statement" role="tab" aria-current="' + (tab === 'statement') + '">Statement</button>'
      + '<button data-tab="products" role="tab" aria-current="' + (tab === 'products') + '">Products</button>'
      + '</div>' + periodFilter();

    const f = state.filters;
    const st = Profit.statement(state.ledger, {
      filter: { from: f.from, to: f.to, account: f.account, marketplace: f.marketplace, marketplaces: f.marketplace ? [f.marketplace] : null, currency: f.currency },
      productCosts: state.productCosts.length ? state.productCosts : null,
      operatingCosts: state.operatingCosts.length ? state.operatingCosts : null,
      externalAdvertising: null,
    });

    if (tab === 'statement') {
      html += '<section class="card"><div class="card-h">' + ico('profit') + '<h2>'
      ...
      for (const l of st.lines) {
        html += '<li' + (l.emphasis ? ' class="total"' : '') + '><span class="l">' + esc(l.label)
          + (l.incomplete ? ' <span class="tag is-forecast">' + ico('missing', { size: 'sm' })
            + 'Incomplete</span>' : '')
      ...
      html += '<div class="note is-warn">' + ico('missing') + '<div><b>Cost coverage.</b> '
        + (st.productCost.coverage.total
          ? st.productCost.coverage.covered.toLocaleString() + ' of '
          + st.productCost.coverage.total.toLocaleString() + ' units sold have a recorded product cost'
          : 'No product costs have been supplied')
        + '. ' + (st.coverage.operatingCosts ? '' : 'No operating costs supplied. ')
        + (st.coverage.externalAdvertising ? '' : 'No external advertising billing supplied.')
        + '</div></div>';
      html += '<details><summary>' + ico('info', { size: 'sm' })
      ...
      const sk = Profit.bySku(state.ledger, {
        filter: { from: f.from, to: f.to, account: f.account, marketplace: f.marketplace, marketplaces: f.marketplace ? [f.marketplace] : null, currency: f.currency },
        productCosts: state.productCosts,
      });
      html += '<section class="card"><div class="card-h">' + ico('profit') + '<h2>Every SKU</h2></div>'
        + '<div class="sub">' + sk.costCoverage.covered + ' of ' + sk.costCoverage.total
        + ' SKUs have a recorded product cost.</div>';
      ...
      for (const r of sk.rows.slice(0, 250)) {
        html += '<tr><td>' + esc(r.msku) + '</td>'
          ...
          + '<td class="n">' + amount(r.cogs == null ? null : -r.cogs,
            { missing: 'a product cost for this SKU' }) + '</td>'
          + '<td class="n">' + amount(r.contribution,
            { missing: 'a product cost for this SKU' }) + '</td></tr>';
      }
      html += '</tbody></table></div>';
      html += '<div class="note">' + ico('info') + '<div><b>Unallocated account-level charges: '
        + esc(M.fmt(sk.unallocated.amount)) + '</b> across ' + sk.unallocated.rows.toLocaleString()
        + ' rows. ' + esc(sk.unallocated.note) + '</div></div></section>';
    }
  ...
  screens.plan = function () {
    /* The forecast inflow is knowable from the preview alone; only the
       spendable figure needs bank cash and commitments. */
    let pre = '';
    const dsP = dataset();
    if (dsP.forecast.present) {
      pre += '<section class="card"><div class="card-h">' + ico('forecast')
        + '<h2>Money Amazon expects to owe you</h2></div>'
        + '<div class="sub">' + esc(fmtDay(dsP.forecast.period.from)) + ' – '
        + esc(fmtDay(dsP.forecast.period.to)) + ', from your imported forecast. When it reaches '
        + 'your bank is a separate question and needs deposit history.</div>'
        + '<div class="grid g2">'
        + statCard('forecast', 'Forecast net receivable', dsP.forecast.netReceivable,
          null, 'AMAZON FORECAST')
        + statCard('expenses', 'Forecast period charges',
          (dsP.forecast.storage == null && dsP.forecast.advertising == null) ? null
            : -((dsP.forecast.storage || 0) + (dsP.forecast.advertising || 0)),
          'storage and advertising columns', 'AMAZON FORECAST')
        + '</div></section>';
    }
    return pre + planScreen();
  };

  function planScreen() {
    let html = '<section class="card"><div class="card-h">' + ico('plan')
      + '<h2>Cash available for bills</h2></div>';
    if (state.openingBankCash == null || !state.cashCommitments.length) {
      html += '<div class="sub">A spendable-cash number needs three things this app does not have: '
        + 'what is in the bank now, what is already committed, and the buffer you want to keep. '
        + 'Until then no figure is shown, because a plausible-looking one would be worse than none.</div>'
        + unavailable('opening bank cash and your dated commitments');
    }
    html += '<div class="frow f3" style="margin-top:20px">'
      + '<div class="field"><label class="lbl" for="bank-open">Opening bank cash</label>'
      + '<input type="number" step="0.01" id="bank-open" value="'
      + (state.openingBankCash == null ? '' : (state.openingBankCash / 100).toFixed(2)) + '"></div>'
      + '<div class="field"><label class="lbl" for="bank-buffer">Cash buffer to hold</label>'
      + '<input type="number" step="0.01" id="bank-buffer"></div>'
      + '<div class="field"><label class="lbl">&nbsp;</label>'
      + '<button class="btn" id="savebank">Save</button></div></div>';
    html += '<div class="note">' + ico('info') + '<div>Bank receipts are not reduced by settlement '
      + 'fees again — Amazon\'s fees are already inside the net amounts that released.</div></div>';
    html += '</section>';

    html += '<section class="card"><div class="card-h">' + ico('expenses')
      + '<h2>Committed outflows</h2></div>'
      + '<div class="sub">Bills, cards and inventory purchases, by due date. Inventory purchase cash '
      + 'belongs here, not in the profit statement on top of product cost.</div>';
    if (!state.cashCommitments.length) {
      html += Charts.emptyState('Nothing committed yet',
        'Add the bills, card payments and stock purchases you already know about.');
    } else {
      html += '<div class="tscroll"><table><thead><tr><th>Due</th><th>What</th><th>Kind</th>'
        + '<th class="n">Amount</th><th>Status</th></tr></thead><tbody>';
      for (const c of state.cashCommitments.slice().sort((a, b) => a.due < b.due ? -1 : 1)) {
        html += '<tr><td>' + esc(fmtDay(c.due)) + '</td><td>' + esc(c.label) + '</td>'
          + '<td>' + esc(c.kind || '—') + '</td>'
          + '<td class="n">' + amount(-c.amount) + '</td>'
          + '<td>' + esc(c.confirmed ? 'Confirmed' : 'Estimated') + '</td></tr>';
      }
      html += '</tbody></table></div>';
    }
    html += '<div class="frow f3" style="margin-top:16px">'
      ...
      + '<button class="btn" id="addcommit">Add commitment</button></section>';
    return html;
  };
  ...
  screens.recon = function () {
    ...
    const tab = state.tab || 'settlements';
    let html = '<div class="subtabs" role="tablist">'
      + '<button data-tab="settlements" role="tab" aria-current="' + (tab === 'settlements') + '">Settlements</button>'
      + '<button data-tab="bank" role="tab" aria-current="' + (tab === 'bank') + '">Bank &amp; transfers</button>'
      + '<button data-tab="fva" role="tab" aria-current="' + (tab === 'fva') + '">Forecast vs actual</button>'
      + '<button data-tab="baselines" role="tab" aria-current="' + (tab === 'baselines') + '">Baselines</button>'
      + '</div>';
    ...
    if (tab === 'bank') {
      ...
      const match = Recon.matchBankDeposits(tr, state.bankDeposits);
  ...
  screens.data = function () {
    ...
    html += '<section class="card"><div class="card-h">' + ico('upload')
      + '<h2>Import Amazon reports</h2></div>'
      + '<div class="sub">The Payments date-range transaction CSV, and Fees &amp; Economics Preview '
      + 'CSVs. Files are parsed in this browser and never uploaded.</div>';
    html += '<div id="drop">' + ico('upload', { size: 'lg' })
      + '<h3>Drop CSV files here</h3>'
      + '<div class="meta">or <label style="color:var(--accent);cursor:pointer;text-decoration:underline">'
      + 'choose files<input type="file" id="filepick" multiple accept=".csv" hidden></label></div>'
    ...
    html += '<section class="card"><div class="card-h">' + ico('assumptions')
      + '<h2>Account payout rules</h2></div>'
    ...
    html += '<section class="card"><div class="card-h">' + ico('missing')
      + '<h2>What is still needed</h2></div>'
    ...
    html += '<details class="dl" style="margin:20px 0"><summary>'
      + 'If something is not working</summary><div style="padding-top:12px">'
      + selfTestCard()
      + downloadTestCard()
      + '</div></details>';
  ...
  function buildBrief() {
    ...
    L.push('  bank transit: ' + (state.policy.bankTransitDaysLow == null ? 'unknown'
      : state.policy.bankTransitDaysLow + '-' + state.policy.bankTransitDaysHigh + ' days'));
    return L.join('\n');
  }
  ...
  function askPanel() {
    ...
    if (state.ask.available === false) {
      ...
      body.innerHTML = '<div class="note is-warn">' + ico('missing')
        ...
        + 'answer, the panel stays off.<br><br>Open this app as a published artifact on your own '
        + 'account to enable it.</div></div>'
  ...
  function mirrorPanel() {
    ...
    let html = '<details class="panel" id="mirror"' + (m && m.configured ? '' : '')
      + '><summary>' + ico('marketplace', { size: 'sm' })
      + 'Read your figures from another device</summary><div class="body">';

    html += '<p class="muted" style="margin-top:0">Your figures stay on this '
      + 'computer. This sends a <b>copy</b> of them to a database you own, so a '
      + 'phone or laptop can read them. Only this computer ever writes, so '
      + 'there is nothing to go wrong between two devices editing at once.</p>';
    ...
    if (m && m.configured) {
      ...
      html += '<div class="note"' + '>' + ico('info') + '<div>'
        ...
        + '<b>Not</b> the <code>service_role</code> key \u2014 that one ignores '
        + 'every access rule, and this app will refuse it.</div></div>';
      ...
      html += '<div style="margin-top:12px">'
        ...
        + esc(state.mirrorUrl || '') + '">'
        + '<label class="muted" for="sbkey">anon public key</label>'
  ...
  async function mirrorBoot() {
    if (!state.worker || !state.workerInfo) return;
    try {
      state.mirror = await state.worker.mirror();
      render();
    } catch (e) { /* an older helper has no mirror; the panel simply waits */ }
  }
  ...
  function aboutPanel() {
    ...
    if (sh) {
      html += '<div class="btnrow" style="margin-top:12px">'
        + '<button class="btn sec sm" data-checkupdates="1">'
  ...
  async function syncBoot() {
    state.sync = null;
    state.syncState = { status: 'unavailable', reason: 'Automatic remote sync is disabled: no backend authorization or local-to-artifact bridge has been verified. A storage test does not establish either. Artifact sharing has not been checked.' };
    paintSyncBadge();
    render();
  }
  ...
  const SYNC_WORDS = {
    ...
    saved:      ['is-actual', 'verified', 'Saved to your account'],
    failed:     ['is-forecast', 'missing', 'Not saved to your account'],
    conflict:   ['is-forecast', 'missing', 'Changed on another device'],
    idle:       ['is-assumed', 'asOf', 'Ready'],
    connected:  ['is-actual', 'verified', 'Account connected'],
    unavailable:['is-forecast', 'missing', 'This device only'],
  };
  ...
  function syncBadge() {
    ...
    return '<button class="tag ' + w[0] + '" data-go="data" '
      + 'title="' + esc(st.reason || (st.lastSyncedAt
        ? 'Last saved to your account at ' + new Date(st.lastSyncedAt).toLocaleTimeString()
        : 'Account sync status')) + '" '
  ...
  function syncPanel() {
    ...
    let html = '<section class="card"><div class="card-h">' + ico('asOf')
      + '<h2>Your account</h2></div>'
      + '<div class="sub">Where this data lives, and whether another device can see it.</div>';
    ...
    if (st.status === 'unavailable') {
      ...
      html += 'This app was also changed on another device since this one last synced. '
        + 'Nothing has been overwritten. Choose below which version to keep.';
    } else if (st.status === 'connected') {
      html += 'You are signed in and this app can reach your account, but there is nothing '
        + 'to save yet. Import a report and it will be stored to your account automatically.';
    } else if (ok) {
      html += 'Imported reports, balances and settings are saved to your Claude account and '
        + 'will be there when you sign in on another device.'
        + (st.lastSyncedAt ? ' Last saved ' + esc(new Date(st.lastSyncedAt).toLocaleString()) + '.' : '');
    } else {
    ...
    if (st.status === 'conflict' && st.conflict) {
      ...
      html += '<div class="note is-warn">' + ico('missing') + '<div>'
        ...
        + '<dt>In your account' + (c.remoteAt ? ', saved ' + esc(new Date(c.remoteAt).toLocaleString()) : '')
        ...
        + '<button class="btn sec" data-conflict="theirs">Use the account version</button>'
    ...
    if (pending.length) {
      html += '<div class="note is-warn">' + ico('missing') + '<div><b>' + pending.length
        + ' of your imports are listed in your account but their data has not arrived on this '
        + 'device.</b> They may still be uploading from the device that imported them, or they '
  ...
  function topControls() {
    ...
    if (marketplaces.length) {
      html += '<label class="lbl" for="mkt" style="margin:0 4px 0 0">Marketplace</label>'
        + '<select id="mkt" style="width:auto">'
        + '<option value="">All marketplaces</option>'
        + marketplaces.map(m => '<option value="' + esc(m) + '"'
          + (state.filters.marketplace === m ? ' selected' : '') + '>' + esc(m) + '</option>').join('')
        + '</select>';
    }
    html += '<label class="lbl" for="cur" style="margin:0 4px 0 12px">Currency</label>'
      + '<select id="cur" style="width:auto">' + [...new Set(['USD', ...state.previews.map(p => p.currency), ...state.imports.map(i => i.currency)])].filter(Boolean).map(c => '<option' + (c === state.filters.currency ? ' selected' : '') + '>' + esc(c) + '</option>').join('') + '</select>';
    if (state.workerInfo) html += refreshButton();
    html += '<span id="syncbadge" style="margin-left:12px">' + syncBadge() + '</span>';
    html += '<button class="btn" id="importbtn" style="margin-left:8px">'
      + ico('upload', { size: 'sm' }) + 'Import reports</button>';
    return html;
  }
  ...
  function render() {
    ...
    $('#asof').innerHTML = ico('asOf', { size: 'sm' })
      + (bal ? 'Balances as of ' + esc(fmtDay(bal.observedAt)) + ' · entered by hand'
        : 'No balance snapshot recorded yet');
    ...
    $('#view').innerHTML = dateBasisNote() + body;

    $('#sidenav').innerHTML = SCREENS.map(s =>
      '<button data-go="' + s.id + '"' + (s.id === state.screen ? ' aria-current="page"' : '') + '>'
      + ico(s.icon) + '<span>' + esc(s.name) + '</span></button>').join('');
    ...
    /* A bare count of missing inputs told the reader nothing about what the
       app could already do. Lead with what works. */
    const rd = dataset().readiness;
    const readyN = Dataset.ready(rd).length, blockedN = Dataset.blocking(rd).length;
    $('#sidenote').innerHTML = !dataset().anyData
      ? ico('missing', { size: 'sm' }) + '<span>Nothing imported yet. '
        + '<b>Import a report</b> to begin.</span>'
      : blockedN === 0
        ? ico('verified', { size: 'sm' }) + '<span>Everything this app can calculate is '
          + 'available.</span>'
        : ico('verified', { size: 'sm' }) + '<span><b>' + readyN + ' of ' + rd.length
          + '</b> features working from your imported data. <b>' + blockedN + '</b> need more '
          + 'input — see <b>What is ready</b> on the dashboard.</span>';
    ...
    if (mir) {
      ...
      if (what === 'forget') {
        state.worker.mirrorForget()
          .then(r => { state.mirror = r; state.mirrorSaid = null; })
  ...
  document.addEventListener('change', e => {
    ...
    if (id === 'acctsel' || id === 'f-acct') { state.filters.account = e.target.value || null; render(); }
    ...
    if (id === 'filepick') { (async () => { for (const f of Array.from(e.target.files)) await importFile(f); })(); }
    if (id === 'restorepick') {
