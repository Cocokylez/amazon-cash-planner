...
(function () {
      ...
      + (skuRange === 'Custom date range'
        ? (state.filters.from && state.filters.to
          ? 'The reporting period at the top of this page is typed into the From '
            ...
            + esc(fmtDay(state.filters.from)) + '</b> to <b>'
            + esc(fmtDay(state.filters.to)) + '</b>.'
          : 'No period is chosen at the top of the page, so the forecast’s own '
            + 'window is typed into Amazon’s From and To boxes: <b>'
            + esc(fmtDay(horizonStart())) + '</b> to <b>' + esc(fmtDay(horizonEnd())) + '</b>.')
  ...
  function getWindow() {
    ...
    const txRep = reports.find(r => r.id === 'date-range-transactions');
    const fcReady = !!(fcRep && fcRep.ready);
    const txReady = !!(txRep && txRep.ready);
    const acctTypes = (txRep && txRep.accountTypes) || ['All (Unified Reports)'];
    ...
    let html = '<div class="modal-scrim" data-gwclose="1"></div>'
      ...
      + '<p class="meta" style="margin:0 0 14px">Check the dates and country, then Download. '
    ...
    html += '<section style="border:1px solid var(--line);border-radius:10px;padding:14px 16px;'
      + (txReady ? '' : 'background:var(--surface-alt);') + '">'
      + '<div class="card-h" style="margin:0 0 6px"><h3 style="font-size:var(--t-h3)">Transaction history \u00b7 Payments</h3>'
      + (txReady ? '' : '<span class="tag is-warn">Not set up yet</span>') + '</div>';
    if (!txReady) {
      html += '<div class="meta">This report needs a one-time setup before it can be fetched, so it is '
        + 'skipped this time. Everything that depends on it - actual fees, payout timing, profit - '
        + 'says so until it is set up.</div>'
        + '<div class="btnrow" style="margin-top:10px"><button class="btn sec sm" data-gwsetup="1">'
        + 'Set it up now</button></div>';
    } else {
      html += '<div class="frow f3">'
        + '<div class="field"><label class="lbl" for="gw-acct">Account type</label><select id="gw-acct">'
        + acctTypes.map(t => '<option' + (t === g.acct ? ' selected' : '') + '>' + esc(t) + '</option>').join('')
        + '</select></div>'
        + '<div class="field"><label class="lbl" for="gw-hfrom">From</label>'
        + '<input type="date" id="gw-hfrom" value="' + esc(g.hFrom) + '" max="' + esc(state.today) + '"></div>'
        + '<div class="field"><label class="lbl" for="gw-hto">To</label>'
        + '<input type="date" id="gw-hto" value="' + esc(g.hTo) + '" max="' + esc(state.today) + '"></div></div>'
        + '<div class="btnrow" style="gap:6px;margin-top:4px">'
        + quick('data-gwh', '45', 'Last 45 days') + quick('data-gwh', '90', 'Last 90 days')
        + quick('data-gwh', 'month', 'This month') + quick('data-gwh', 'lastmonth', 'Last month') + '</div>'
        + '<div class="lbl" style="margin-top:12px">Countries <span class="meta" style="font-weight:400">'
        + '- one report per country</span></div>'
        + '<div class="mkts" style="max-height:132px;overflow:auto">'
        + MARKETPLACES.map(m => '<label class="mkt' + (g.mkts.indexOf(m) >= 0 ? ' on' : '') + '">'
          + '<input type="checkbox" data-gwmkt="' + esc(m) + '"' + (g.mkts.indexOf(m) >= 0 ? ' checked' : '')
          + '> ' + esc(m) + '</label>').join('')
        + '</div>'
        + '<div class="meta" style="margin-top:6px">' + days(g.hFrom, g.hTo) + ' days, '
        + g.mkts.length + ' ' + (g.mkts.length === 1 ? 'country' : 'countries') + '.</div>';
    }
    html += '</section>';
    ...
    html += '<div class="btnrow" style="margin-top:16px;justify-content:flex-end">'
      ...
      + '<button class="btn" id="gw-go"' + (fcReady || txReady ? '' : ' disabled') + '>'
  ...
  async function submitGetWindow() {
    ...
    const txReady = reports.some(r => r.id === 'date-range-transactions' && r.ready);
    const fcReady = reports.some(r => r.id === 'fees-preview' && r.ready);
    const tomorrow = horizonStart();
    const problem = !fcReady ? null
      : !g.fFrom || !g.fTo ? 'Choose the forecast dates.'
        : g.fFrom > g.fTo ? 'The forecast ends before it starts.'
          : g.fFrom < tomorrow ? 'The forecast starts tomorrow at the earliest (' + fmtDay(tomorrow) + ').'
            : null;
    const hProblem = !txReady ? null
      : !g.hFrom || !g.hTo ? 'Choose the transaction history dates.'
        : g.hFrom > g.hTo ? 'The history ends before it starts.'
          : g.hTo > state.today ? 'The history cannot reach past today.'
            : !g.mkts.length ? 'Choose at least one country for the transaction history.' : null;
    if (problem || hProblem) { g.error = problem || hProblem; render(); return; }

    /* Remembered for next time, on the helper, like the panel's settings. */
    state.skuMarketplace = g.skuMkt;
    if (txReady) { state.accountType = g.acct; state.marketplaces = g.mkts.slice(); }
    const choice = {
      forecast: fcReady ? { from: g.fFrom, to: g.fTo } : null,
      history: txReady ? { from: g.hFrom, to: g.hTo } : null,
    };
    state.getWin = null;
    render();
  ...
  function downloadsPanel() {
    ...
    /* What the next download will actually ask for. With no period chosen
       each report keeps its own window - this used to print today plus
       thirteen days as if it were the period in use. */
    html += '<div class="meta" style="margin-top:10px">'
      + (state.filters.from && state.filters.to
        ? 'Uses the reporting period at the top of the page: <b>'
          + esc(fmtDay(state.filters.from)) + ' – ' + esc(fmtDay(state.filters.to)) + '</b>.'
        : 'No period chosen, so each report uses its own window: the forecast covers <b>'
          + esc(fmtDay(horizonStart())) + ' – ' + esc(fmtDay(horizonEnd())) + '</b>.')
      + '</div>';
