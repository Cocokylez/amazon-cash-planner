...
(function () {
  ...
  const state = {
    ...
    salesMetric: 'netSales',   // which tile the forecast chart shows
  ...
  const screenById = id => SCREENS.concat(HIDDEN_SCREENS).find(s => s.id === id) || SCREENS[0];
  ...
  screens.sales = function () {
    ...
    /* the dates */
    let html = '<section class="card"><div class="frow f3" style="align-items:end">'
    ...
    /* the totals: each tile picks what the chart below shows */
    const metric = Charts.MEASURES.some(x => x.key === state.salesMetric) ? state.salesMetric : 'netSales';
    const pick = (key, label, cents, meta, org) => '<button type="button" class="tile tile-btn" data-smetric="'
      + key + '" aria-pressed="' + (metric === key) + '" style="--fc:'
      + Charts.MEASURES.find(x => x.key === key).colour + '" title="Show ' + esc(label.toLowerCase())
      + ' on the chart"><span class="tile-l"><i class="fc-key"></i>' + esc(label) + '</span>'
    ...
    /* the chart of the picked tile, read by hovering */
    const measure = Charts.MEASURES.find(x => x.key === metric);
    const mode = state.salesMode === 'weekly' ? 'weekly' : 'running';
    html += '<section class="card"><div class="card-h">' + ico('forecast') + '<h2>' + esc(measure.name)
      ...
      + '</div></div>'
      + Charts.forecast(f.daily, { metric, mode, currency: state.filters.currency || f.currency,
        mondayOf: Simple.mondayOf })
      + '<div class="meta" style="margin-top:10px">Hover over the chart to read any '
      + (mode === 'weekly' ? 'week' : 'day') + '. Click a tile above to chart it. '
    ...
    /* the fees */
    html += '<section class="card"><div class="card-h">' + ico('expenses') + '<h2>Amazon fees</h2></div>'
      + '<div class="tscroll"><table><thead><tr><th>Fee</th><th class="n">Amount</th>'
      + '<th class="n">Share of net sales</th></tr></thead><tbody>';
    /* Fees that come to nothing for these dates are left out: a row of
       $0.00 lines says nothing. */
    for (const x of f.fees.filter(y => y.amount)) {
      html += '<tr><td>' + esc(x.name) + '</td><td class="n">' + amount(-x.amount) + '</td>'
        + '<td class="n">' + pct(x.amount, f.netSales) + '</td></tr>';
    }
    html += '<tr><td><b>All Amazon fees</b></td><td class="n"><b>' + amount(f.feeTotal == null ? null : -f.feeTotal)
      + '</b></td><td class="n"><b>' + pct(f.feeTotal, f.netSales) + '</b></td></tr>'
      + '<tr><td>Advertising (Sponsored Products)</td><td class="n">'
      + amount(f.advertising == null ? null : -f.advertising) + '</td><td class="n">'
      + pct(f.advertising, f.netSales) + '</td></tr>'
    ...
    html += '<section class="card"><div class="card-h">' + ico('profit') + '<h2>By product</h2>'
      ...
      + '<th class="n">Net sales</th><th class="n">Order fees</th><th class="n">Advertising</th>'
      + '<th class="n">After fees &amp; ads</th></tr></thead><tbody>';
    ...
    html += '</tbody></table></div>'
      ...
      + '<div class="meta" style="margin-top:8px">Order fees are the per-sale fees (referral, FBA '
      + 'fulfilment, closing). Storage is charged to the account, so it is in the totals above but not '
      + 'on any one product.</div></section>';
  ...
  function buildBrief() {
    ...
    /* The forecast first: it is what the app is for now. */
    {
      ...
      if (f.coveredDays) {
        ...
        L.push('  top products (net sales, order fees, advertising, after fees and ads): '
          + f.bySku.slice(0, 15).map(r => r.msku + ' ' + M.fmt(r.netSales) + ', ' + M.fmt(r.fees) + ', '
            + M.fmt(r.advertising) + ', ' + M.fmt(r.net)).join('; '));
      } else {
  ...
  function render() {
    ...
    try {
      const ds = dataset();
      body = ds.hiddenByRange && state.screen !== 'data' && state.screen !== 'sales'
        ? rangeEmptyCard(ds)
    ...
    $('#view').innerHTML = (state.screen === 'dashboard' ? setupCard() : '') + body
      + (state.screen === 'sales' || state.screen === 'data' ? '' : dateBasisNote());
    const sm = $('#setupmodal');
