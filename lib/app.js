...
(function () {
  ...
  screens.profit = function () {
    ...
    const sk = Profit.bySku(state.ledger, { filter, productCosts: state.productCosts });
    ...
    html += '<section class="card"><div class="card-h"><h2>Cost coverage</h2>'
      ...
      + '<dt>External advertising</dt><dd>' + (st.coverage.externalAdvertising ? 'Recorded'
        : 'Not supplied') + '</dd></dl>'
      + '<div class="meta" style="margin-top:10px">Revenue less Amazon\u2019s fees is not profit until '
      + 'what you paid for the goods is in.</div>'
      + (skuMissing || !st.coverage.operatingCosts ? '<div class="btnrow" style="margin-top:14px">'
        + '<button class="btn sec sm" data-go="data">' + ico('add', { size: 'sm' })
        + 'Add missing cost</button></div>' : '')
      + '</section>';
    html += '</div>';
  ...
  screens.plan = function () {
    const account = state.filters.account || 'Standard Orders';
    const reqDate = state.requestDate || CSV.addDays(state.today, 1);
    const to = CSV.addDays(state.today, 55);
    const { run } = runFor(account, reqDate);
  ...
  function inputStates(ds) {
    ...
    const skus = ds.forecast.present ? ds.forecast.bySku.length : 0;
    const costed = state.productCosts.length;
    const pick = '<label class="btn sec sm" style="cursor:pointer">{L}<input type="file" '
      + 'data-filepick="1" multiple accept=".csv" hidden></label>';
    const cant = 'This app cannot take this input yet.';
    const list = [
      ...
      { name: 'Bank deposit history', status: state.bankDeposits.length ? 'ready' : 'missing',
        detail: state.bankDeposits.length ? state.bankDeposits.length + ' deposits recorded'
          : 'Dates when a payout really reached the bank, to measure transit time.',
        action: state.bankDeposits.length ? '' : '<span class="meta">' + cant + '</span>' },
      ...
      { name: 'Product costs',
        ...
        action: '<span class="meta">' + cant + '</span>' },
      { name: 'Advertising billing', status: state.advertisingBilling.length ? 'ready' : 'missing',
        detail: state.advertisingBilling.length ? state.advertisingBilling.length + ' billing records'
          : 'Whether ads come out of your payout or go on a card. The cash answers differ.',
        action: state.advertisingBilling.length ? '' : '<span class="meta">' + cant + '</span>' },
    ];
  ...
  document.addEventListener('click', e => {
    ...
    if (jmp) {
      const t = document.getElementById(jmp.getAttribute('data-jump'));
      if (t) { t.scrollIntoView({ behavior: 'smooth', block: 'start' });
        const f = t.querySelector('input, select'); if (f) f.focus({ preventScroll: true }); }
      return;
    }
